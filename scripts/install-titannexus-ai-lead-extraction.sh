...
if [ -f routes/api.php ] && ! grep -q "titannexus_voice_ai.php" routes/api.php; then
    cat >> routes/api.php <<'PHP'

require __DIR__.'/titannexus_voice_ai.php';
PHP
fi
...
echo "PASS70 TitanNexus AI lead manager extraction installed."
echo "Voice webhook routes:"
/usr/local/php84/bin/php artisan route:list | grep "titan-nexus/voice" || true
