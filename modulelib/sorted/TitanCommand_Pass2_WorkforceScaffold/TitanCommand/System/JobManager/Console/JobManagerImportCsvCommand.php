...
class JobManagerImportCsvCommand extends Command
{
    protected $signature = 'jobmanager:import-csv {path} {--dry-run} {--validate-only}';
    protected $description = 'Import Job Manager from a CSV with headers: id,title,status,priority,customer_id,scheduled_at,created_at,updated_at';
    ...
    public function handle(): int
    {
        ...
        foreach ($csv2->getRecords() as $row) {
            ...
            $exists = \App\Extensions\TitanCommand\System\JobManager\Entities\WorkOrder::find($id);
        ...
        if ($errors) {
            ...
            \Illuminate\Support\Facades\Storage::put('exports/jobmanager_import_errors.csv', implode("\n", $errorRows));
            $this->warn('Wrote error CSV to storage/app/exports/jobmanager_import_errors.csv');
