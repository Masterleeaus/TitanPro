...
class TitanCommandChatController extends Controller
{
    ...
    public function index($id = null)
    {
        ...
        $agents = TitanCommand::query()->where('company_id', $companyId)->where('user_id', $userId)->get();
        $defaultAgent = null;
        if ($id) {
        ...
        if (! $defaultAgent) {
            return redirect()->route('dashboard.user.social-media.agent.create')
                ->with([
                    'message' => __('Please create a Titan Command Agent first.'),
                    'type'    => 'info',
                ]);
        }
        ...
        $analysis = TitanCommandAnalysis::query()->where('id', request('analysis'))->where('company_id', $companyId)->where('user_id', $userId)->first();
        ...
        return view('titancommand::chat.index', compact(
            'generators',
            'category',
            'apiSearch',
            'chatbots',
