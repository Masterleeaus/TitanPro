...
>
    <h2 class="mb-4 text-center text-[24px] font-medium leading-[1.2em]">
        <span class="block text-[0.875em] opacity-50">
            @lang('Where would you like to confirm this sub-item?')
        </span>
    ...
    @if ($containers->isEmpty())
        <div class="min-h-[72px] rounded-[10px] bg-foreground/5 px-6 py-[18px] text-center backdrop-blur-xl transition-all">
            <p class="mb-3 text-pretty text-sm font-medium opacity-50">
                @lang('No connected containers found. Please connect your channels accounts first.')
            </p>
            <x-button
                class="underline"
                variant="link"
        ...
        <div class="mb-8">
            ...
            >
                <div
                    ...
                    :class="{ 'rounded-b-none border-foreground/5': dropdownOpen && formData.platform_ids.length < containers.length }"
                ...
                <div
                    ...
                    x-show="dropdownOpen && formData.platform_ids.length < {{ $containers->count() }}"
                    x-transition:enter-start="opacity-0 scale-95 blur-sm"
                    x-transition:enter-end="opacity-100 scale-100 blur-0"
                    x-transition:leave-start="opacity-100 scale-100 blur-0"
                    x-transition:leave-end="opacity-0 scale-95 blur-sm"
                >
                    @foreach ($containers as $container)
                        ...
                        <div
                            ...
                            {{-- @click.prevent="if ( !formData.platform_ids.includes({{ $container->id }}) ) { formData.platform_ids.push({{ $container->id }}) };" --}}
                            @click.prevent="formData.platform_ids = [{{ $container->id }}]; dropdownOpen = false;"
                            x-show="!formData.platform_ids.includes({{ $container->id }})"
                            x-transition:enter-start="opacity-0 scale-95 blur-sm"
                            x-transition:enter-end="opacity-100 scale-100 blur-0"
                            x-transition:leave-start="opacity-100 scale-100 blur-0"
                            x-transition:leave-end="opacity-0 scale-95 blur-sm"
            ...
            <div class="mt-2">
                @include('command-agent::create.step-error', ['step' => 1])
            </div>
        </div>
