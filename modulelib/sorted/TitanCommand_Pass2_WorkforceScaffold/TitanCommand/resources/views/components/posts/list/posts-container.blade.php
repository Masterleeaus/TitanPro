<x-table
    class="relative"
    id="command-agent-sub-items-list"
...
>
    <x-slot:head>
        <th>
            <button
                ...
                @click.prevent="sortPosts('content')"
                type="button"
            >
                @lang('Content')
        ...
        <th>
            <button
                class="flex items-center gap-2"
                @click.prevent="sortPosts('platform_id')"
                type="button"
            >
                @lang('Container')
                <span class="-ms-1 flex flex-col items-center">
                    <x-tabler-chevron-up
                        class="-mb-0.5 h-auto w-3 transition-opacity"
                        ::class="{ 'opacity-30': sort.sortBy === 'platform_id' && sort.sortDirection === 'desc' }"
                        stroke-width="2.5"
                    />
                    <x-tabler-chevron-down
                        class="-mt-0.5 h-auto w-3 transition-opacity"
                        ::class="{ 'opacity-30': sort.sortBy === 'platform_id' && sort.sortDirection === 'asc' }"
                        stroke-width="2.5"
                    />
                </span>
            </button>
    ...
    <x-slot:body>
        @include('command-agent::components.sub-items.list.sub-item-items', ['items' => $sub-items])
    </x-slot:body>
    ...
    >
        @if ($sub-items->hasMorePages())
            <tr>
                <td colspan="5">
                    <div class="command-agent-sub-items-list-load-more-wrap">
                        <a
                            class="command-agent-sub-items-list-load-more group inline-flex w-full items-center justify-center gap-2 text-xs font-medium"
                            href="{{ route('dashboard.user.social-media.agent.sub-item-items', ['page' => 2, 'post_style' => 'list']) }}"
