@php
    use App\Extensions\Titan Command\System\Models\Titan CommandPlatform;

    $new_chat_url = route('dashboard.user.openai.chat.chat');

    if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('command-agent')) {
    ...
    $platforms_list = Titan CommandPlatform::query()
...
@else
    <div class="group relative">
        <x-button href="{{ route('dashboard.user.social-media.agent.sub-items') }}">
            {{ __('New Sub-item') }}
        </x-button>
        ...
        @if ($platforms_list && $platforms_list->count() > 0)
            <div
                ...
                x-data="{}"
            >
                @foreach ($platforms_list as $container)
                    @php
                    ...
                    <a
                        ...
                        @if (request()->route()->getName() === 'dashboard.user.social-media.agent.sub-items') href="#" @click.prevent="$dispatch('create-sub-item-modal-show', {show: true, platformId: {{ $container->id }}})"
						@else href="{{ route('dashboard.user.social-media.agent.sub-items', ['create_post_for' => $container->id]) }}" @endif
                        title="@lang('Compose a sub-item on :container', ['container' => $container->container])"
                    >
                        <img
                            class="size-full dark:hidden"
                            src="{{ $image }}"
                            alt="@lang(':container logo', ['container' => $container->container])"
                        />
                        <img
