...
@section('title', __('Channels Sub-items'))
@section('titlebar_pretitle', '')
@section('titlebar_subtitle', __('Create and edit sub-items'))
...
@section('content')
    <div @class([
        ...
        @if ($theme === 'command-agent-dashboard')
            @include('command-agent::sub-items.titlebar')
        @endif
        ...
        @if (filled($containers))
            <div class="mb-8 grid grid-cols-2 gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                @foreach ($containers as $container)
                    ...
                    >
                        @php
                            $image = asset('vendor/social-media/icons/' . $container->container . '.svg');
                            $image_dark_version = asset('vendor/social-media/icons/' . $container->container . '-mono-light.svg');
                        @endphp
                        ...
                        <p class="mb-0 text-sm font-medium">
                            {{ Str::headline($container->container) }}
                        </p>
                        @if (isset($container->credentials['username']))
                        ...
                        <a
                            ...
                            href="#"
                            @click.prevent="$dispatch('create-sub-item-modal-show', {show: true, platform_id: {{ $container->id }}})"
                        ></a>
                    </x-card>
                @endforeach
            </div>
        ...
        <div x-data="Command AgentPosts">
            <div class="mb-9 flex flex-wrap justify-between gap-5 max-md:gap-y-3 xl:flex-nowrap xl:gap-10">
                <div class="flex grow items-center gap-3 transition max-md:flex-wrap lg:gap-8 lg:border-b">
                    <div
                        ...
                        x-data="{ selectedPlatformLabel: '{{ __('All Containers') }}', dropdownOpen: false }"
                    ...
                    >
                        ...
                        >
                            <button
                                ...
                                @click.prevent="filterPosts({container: ''}); selectedPlatformLabel = '{{ __('All Containers') }}';"
                            >
                                @lang('All')
                            </button>

                            @if (filled($platformEnums))
                                @foreach ($platformEnums as $container)
                                    <button
                                        ...
                                        :class="{ active: filters.container == '{{ $container->name }}' }"
                                        @click.prevent="filterPosts({container: '{{ $container->name }}'}); selectedPlatformLabel = '{{ Str::headline($container->name) }}';"
                                    >
                                        {{ Str::headline($container->name) }}
                                    </button>
                                @endforeach
                            @endif
                        </div>
                    ...
                    <div class="flex gap-4 md:ms-auto">
                        ...
                        <div class="group relative flex">
                            ...
                            >
                                ...
                                >
                                    @foreach ($platforms_with_image as $container)
                                        @php
                                            $image = asset('vendor/social-media/icons/' . $container->container . '.svg');
                                            $image_dark_version = asset('vendor/social-media/icons/' . $container->container . '-mono-light.svg');
                                        @endphp
                                        ...
                                        <x-button
                                            ...
                                            ::class="{ 'active': filters.platform_id == {{ $container->id }} }"
                                            @click.prevent="filterPosts({platform_id: {{ $container->id }}})"
                                            variant="ghost"
                                        >
                                            <figure class="m-0 shrink-0">
                                                <img
            ...
            @include('command-agent::sub-items.create-sub-item-modal', [
                'containers' => $platforms_with_image,
                'agents' => $agents ?? collect(),
            ])

            @include('command-agent::components.sub-items.list.sub-items-container', ['sub-items' => $sub-items, 'show_heading_and_arrows' => false])
