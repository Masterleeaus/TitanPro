<template id="command-agent-chat-sub-item-card-template">
    <template x-if="sub-item">
        <div class="flex grow flex-col rounded-[10px] border p-3.5 transition">
            <div class="mb-2.5 flex items-center justify-between gap-1">
                <div class="flex items-center gap-1 text-2xs text-heading-foreground">
                    <img
                        ...
                        width="20"
                        height="20"
                        :src="`/vendor/social-media/icons/${sub-item.container?.container}.svg`"
                    >
                    <img
                        class="m-0 hidden dark:block"
                        width="20"
                        height="20"
                        :src="`/vendor/social-media/icons/${sub-item.container?.container}-mono-light.svg`"
                    >
                    <span x-text="sub-item.container?.credentials?.username"></span>
                </div>
                <span class="text-3xs opacity-55">
                    <span
                        class="capitalize"
                        x-text="sub-item.post_type"
                    ></span>
                    <span class="mx-1 align-text-top text-xl leading-[0]">.</span>
                    <span
                        class="capitalize"
                        x-text="sub-item.status"
                    ></span>
                </span>
            </div>

            <template x-if="sub-item.media_urls.length">
                <figure
                    ...
                    x-data="{
                        totalSlides: sub-item.media_urls?.length ?? 0,
                        currentIndex: 0,
                        prev() {
                ...
                >
                    ...
                    <template x-for="(media_url, index) in sub-item.media_urls">
                        <img
                    ...
                    <template x-if="sub-item.media_urls.length >= 2">
                        <div>
                            <button
            ...
            <p
                ...
                x-html="sub-item.content"
            ></p>

            <template x-if="sub-item.scheduled_at">
                <p class="mb-4 text-3xs opacity-55">
                    <span
                        x-text="new Date(sub-item.scheduled_at).toLocaleString(navigator.languages.length ? navigator.languages[0] : 'en-US', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).replace(',', '')"
                    ></span>
                </p>
            </template>
            ...
            <div class="mt-auto">
                <x-button
                    ...
                    @click.prevent="openEditSidedrawer({ query: `id=${sub-item.id}`, taskKey: `fetchingPost-${sub-item.id}`, autoNavigateOnPostUpdates: false })"
    ...
    <template x-if="!currentTasks.has('fetchingPost') && !sub-item">
        <div class="flex grow flex-col rounded-[10px] border p-3.5 transition">
            <p class="m-0 text-xs font-medium">
                @lang('Sub-item was removed')
            </p>
        </div>
    </template>
</template>
