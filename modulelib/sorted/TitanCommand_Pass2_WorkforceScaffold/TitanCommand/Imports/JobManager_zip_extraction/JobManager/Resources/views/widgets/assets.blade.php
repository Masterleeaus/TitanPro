...
<div class="card mb-3">
  <div class="card-header d-flex justify-content-between align-items-center">
    ...
    <form method="post" action="{{ route('jobmanager.assets.store', $work_order_id) }}" class="d-flex gap-2">
      @csrf
    ...
    <table class="table table-sm mb-0">
      ...
      @forelse($rows as $r)
        <tr>
          <td>#{{ $r->asset_id }}</td>
          <td>
            <form method="post" action="{{ route('jobmanager.assets.destroy', [$work_order_id, $r->id]) }}">
              @csrf @method('DELETE')
