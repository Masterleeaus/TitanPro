...
use App\Extensions\TitanLeads\System\Http\Resources\MarketingConversationResource;
use App\Extensions\TitanLeads\System\Http\Resources\MarketingMessageResource;
...
use App\Extensions\TitanLeads\System\Services\InboxService;
use App\Extensions\TitanLeads\System\Services\Email\EmailSenderService;
...
use App\Extensions\TitanLeads\System\Services\Whatsapp\WhatsappSenderService;
...
class InboxController extends Controller
{
    ...
    {
        return view('titan-leads::inbox.index');
    }
    ...
    public function store(Request $request): ChatbotHistoryResource|JsonResponse
    {
        ...
        if ($marketingConversation->getAttribute('type') === 'sms') {
            try {
                $this->smsSenderService
                    ->setSmsChannel(Auth::id())
                    ->sendText(
                        receiver: $marketingConversation->getAttribute('session_id'),
                        message: $request['message']
                    );
        ...
        if ($marketingConversation->getAttribute('type') === 'email') {
            try {
                $this->emailSenderService
                    ->setEmailChannel(Auth::id())
                    ->send(
                        to: $marketingConversation->getAttribute('session_id'),
                        subject: 'Reply',
