<x-button
    ...
    size="none"
    href="{{ route('dashboard.user.titan-leads.inbox.index') }}"
...
@push('script')
    @if (config('titan-leads.notification_enabled', true))
        <script>
            $(document).ready(function() {
                // Function to fetch the notification count
                function fetchNotificationCount() {
                    $.ajax({
                        url: '{{ route('dashboard.user.titan-leads.inbox.notification.count') }}',
                        type: 'GET',
