...
@section('titlebar_actions')
    ...
    <div class="flex gap-4 lg:justify-end">
        <x-button href="{{ route('dashboard.user.titan-leads.telegram-campaign.create') }}">
                ...
                @foreach ($items as $item)
                    <tr>
                        ...
                        <td class="whitespace-nowrap text-end">
                            ...
                            @else
                                {{--								<x-button --}}
                                {{--									href="{{ route('dashboard.user.titan-leads.train.index', $item->id) }}" --}}
                                {{--									class="size-9" --}}
                                {{--									variant="ghost-shadow" --}}
                                {{--									size="none" --}}
                                {{--									title="{{ __('Training') }}" --}}
                                ...
                                <x-button
                                    class="size-9"
                                    href="{{ route('dashboard.user.titan-leads.telegram-campaign.edit', $item->id) }}"
                                    variant="ghost-shadow"
                                    size="none"
                                    title="{{ __('Edit') }}"
                                >
                                ...
                                <x-button
                                    class="size-9"
                                    data-delete="delete"
                                    data-delete-link="{{ route('dashboard.user.titan-leads.telegram-campaign.destroy', $item->id) }}"
                                    variant="ghost-shadow"
                                    hover-variant="danger"
                                    size="none"
                                    title="{{ __('Delete') }}"
