			...
			>
				...
				<div class="lqd-ext-chatbot-training mt-16 flex flex-col justify-center gap-9">
					...
					<div class="lqd-ext-chatbot-training-content grid">
						@include('titan-leads::training.training-tabs.training-tab-website')
						@include('titan-leads::training.training-tabs.training-tab-pdf')
						@include('titan-leads::training.training-tabs.training-tab-text')
						@include('titan-leads::training.training-tabs.training-tab-qa')
					</div>
				</div>
			</div>
		</div>
			...
			document.addEventListener('alpine:init', () => {
				Alpine.data('marketingCampaignTraining', () => ({
					...
					async fetchEmbeddings() {
						...
						const res = await fetch(`{{ route('dashboard.user.titan-leads.train.data') }}?id={{$marketingCampaign->id}}`);
					...
					async deleteEmbedding(embeddingId) {
						...
						const res = await fetch('{{ route('dashboard.user.titan-leads.train.delete') }}', {
							method: 'POST',
							headers: {
								'Content-Type': 'application/json',
								'Accept': 'application/json',
