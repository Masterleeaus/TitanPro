                ...
                <tr>
                    ...
                    <td>
                        <img
                            class="size-6"
                            alt="{{ $item->type->name }}"
                            src="{{ asset('vendor/titan-leads/images/' . $item->type->value . '.png') }}"
                        />
                    </td>
                    <td>
                    ...
                    <td class="whitespace-nowrap text-end">
                        ...
                        @else
                            <x-button
                                class="size-9"
                                href="{{ route('dashboard.user.titan-leads.' . $item->type->value . '-campaign.edit', $item->id) }}"
                                variant="ghost-shadow"
                                size="none"
                                title="{{ __('Delete') }}"
                            >
                            ...
                            <x-button
                                class="size-9"
                                data-delete="delete"
                                data-delete-link="{{ route('dashboard.user.titan-leads.' . $item->type->value . '-campaign.destroy', $item->id) }}"
                                variant="ghost-shadow"
                                hover-variant="danger"
                                size="none"
                                title="{{ __('Delete') }}"
