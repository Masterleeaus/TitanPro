...
@section('content')
    <div>
        ...
        >
            ...
            >
                @includeIf('titan-leads::inbox.particles.conversations-filter')
                ...
                >
                    @includeIf('titan-leads::inbox.particles.conversations-channel-filter')

                    @include('titan-leads::inbox.particles.conversations-list')
                </div>
            </div>
            ...
            >
                @include('titan-leads::inbox.particles.messages-header')
                ...
                >
                    <div class="mt-auto space-y-2 px-4 xl:px-10">
                        @include('titan-leads::inbox.particles.messages-list')
                    </div>

                    @include('titan-leads::inbox.particles.messages-form')
                </div>
            </div>
            ...
            >
                @include('titan-leads::inbox.particles.contact-info-head')

                <div class="grid grow grid-cols-1 place-items-start overflow-y-auto">
                    @include('titan-leads::inbox.particles.contact-info-tab-details')
                </div>
            </div>
        </div>
    </div>
                ...
                Alpine.data('externalChatbotHistory', () => ({
                    ...
                    async nameUpdate(element) {
                        ...
                        const res = await fetch(
                            '{{ route('dashboard.user.titan-leads.inbox.conversations.name.update') }}', {
                                method: 'POST',
                                headers: {
                    ...
                    async onSendMessage() {
                        ...
                        const res = await fetch(
                            '{{ route('dashboard.user.titan-leads.inbox.history') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                    ...
                    async fetchHistories(id) {
                        const res = await fetch(
                            '{{ route('dashboard.user.titan-leads.inbox.history') }}?conversation_id=' + id, {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                    ...
                    async handleConversationsSearch() {
                        ...
                        const res = await fetch(
                            '{{ route('dashboard.user.titan-leads.inbox.conversations.search') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                    ...
                    async handleDelete() {
                        ...
                        const res = await fetch(
                            '{{ route('dashboard.user.titan-leads.inbox.destroy') }}', {
                                method: 'DELETE',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
