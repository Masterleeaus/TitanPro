...
{
    protected ?TitanClient $client = null;
    ...
    protected function getProfileNameByCategory($categoryId)
    {
        ...
        $map = config('aidocument.titan_core.category_profiles', []);
    ...
    {
        $config = config('aidocument.titan_core.profiles');
    ...
    {
        // Titan Core may be disabled / not installed in some tenants.
        $this->client = app()->bound(TitanClient::class) ? app(TitanClient::class) : null;
    }
        ...
        if(\Auth::user()->isAbleTo('ai document create'))
        {
            if(isset($request->template_id) && !empty($request->template_id))
            {
                ...
                if($template)
                {
                    ...
                    $document_store->titan_model = $profileConfig['model'] ?? config('aidocument.titan_core.default_model', 'gpt-5-mini');
    ...
    public function AiGenerate(Request $request)
    {
        if ($request->ajax())
        {
            ...
            if(\Auth::user()->isAbleTo('ai document generate'))
            {
                ...
                if (!$this->client) {
            return response()->json(['status' => 'fail', 'message' => 'Titan Core is not available.'], 503);
        }
        ...
        $response = $this->client->chat([
                ...
                if (isset($response['choices']))
                {
                    ...
                    $prompt_store->titan_model = $profileConfig['model'] ?? config('aidocument.titan_core.default_model', 'gpt-5-mini');
                    ...
                    $content->titan_model = $profileConfig['model'] ?? config('aidocument.titan_core.default_model', 'gpt-5-mini');
