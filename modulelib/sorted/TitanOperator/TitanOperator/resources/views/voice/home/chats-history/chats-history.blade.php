<div
    class="lqd-ext-titan_operator-history invisible fixed end-0 start-0 top-0 z-10 flex h-screen bg-background opacity-0 transition-all max-sm:block lg:start-[--navbar-width] [&.lqd-open]:visible [&.lqd-open]:opacity-100"
    x-data="externalTitanOperatorHistory"
    :class="{ 'lqd-open': open }"
    @keydown.window.escape="setOpen(false)"
>
    <div
        class="lqd-ext-titan_operator-history-sidebar group/history-sidebar relative w-full shrink-0 space-y-5 border-e px-6 py-7 sm:h-full sm:w-[440px] sm:overflow-y-auto"
    ...
    >
        ...
        <div
            ...
            @include('titan_operator-voice::home.chats-history.chats-list')

            <div
                class="lqd-ext-titan_operator-history-load-wrap grid place-items-center font-medium text-heading-foreground"
                x-ref="loadMoreWrap"
            >
                <x-button
                    class="lqd-ext-titan_operator-history-load-more col-start-1 col-end-1 row-start-1 row-end-1 w-full"
                    variant="link"
                    href="{{ route('dashboard.titan_operator-voice.conversation.with.paginate', ['page' => 1]) }}"
                    x-ref="loadMore"
                ...
                <span
                    class="lqd-ext-titan_operator-history-loading col-start-1 col-end-1 row-start-1 row-end-1 inline-flex gap-2"
                    x-show="!allLoaded && fetching"
                    x-ref="loading"
                >
                    {{ __('Loading') }}
                ...
                <span
                    class="lqd-ext-titan_operator-history-all-loaded col-start-1 col-end-1 row-start-1 row-end-1 inline-flex gap-2"
                    x-ref="allLoaded"
                    x-show="allLoaded"
                >
                    {{ __('All Items Loaded') }}
    ...
    <div class="lqd-ext-titan_operator-history-content-wrap flex grow flex-col max-sm:h-[70vh]">
        <div class="lqd-ext-titan_operator-history-head hidden h-[--header-height] w-full justify-between gap-4 border-b px-8 py-5">
            <form
        ...
        <div class="lqd-ext-titan_operator-history-messages-wrap grow overflow-hidden">
            <div
                class="lqd-ext-titan_operator-history-messages flex h-full flex-col gap-2 overflow-y-auto p-10"
                x-ref="historyMessages"
            >
                @include('titan_operator-voice::home.chats-history.chat-messages')
            </div>
        </div>
