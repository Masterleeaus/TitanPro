<template x-for="(message, index) in chatsList.find(c => c.id == activeChat)?.chat_histories">
    <div
        class="lqd-ext-titan_operator-history-message flex gap-2"
        data-sender="message.role"
        :class="{ 'flex-row-reverse': message.role === 'agent' }"
        :key="'message-' + index"
    >
        <div class="lqd-ext-titan_operator-history-message-content-wrap space-y-2 lg:max-w-[50%]">
            <div
                class="lqd-ext-titan_operator-history-message-content rounded-xl p-3"
                :class="{
            ...
            <div class="lqd-ext-titan_operator-history-message-time text-[11px] opacity-50">
                <p
                    class="m-0"
