<div class="lqd-ext-titan_operator-window-welcome-banner">
    <div class="mb-28 flex items-center gap-2.5 text-white">
        @if ($is_editor)
            <img
                class="lqd-ext-titan_operator-window-head-logo"
                :src="activeTitanOperator.logo"
                x-show="activeTitanOperator.show_logo && activeTitanOperator.logo"
                width="25"
                height="25"
            />
            <svg
            ...
            <h4
                class="lqd-ext-titan_operator-window-head-title text-current"
                x-text="activeTitanOperator.title"
            ></h4>
        @else
            @if ($titan_operator['logo'] && $titan_operator['show_logo'])
                <img
                    class="lqd-ext-titan_operator-window-head-logo"
                    alt="{{ $titan_operator['title'] }}"
                    src="{{ $titan_operator['logo'] }}"
                    width="25"
                    height="25"
                />
            @endif
            ...
            @endif
            <h4 class="lqd-ext-titan_operator-window-head-title">
    ...
    <button
        class="lqd-ext-titan_operator-window-welcome-button relative flex w-full items-center gap-3.5 rounded-full bg-white/25 p-4 text-start text-white shadow-2xl backdrop-blur-md transition-all before:pointer-events-none before:absolute before:-inset-px before:z-0 before:rounded-full before:border before:border-white before:opacity-35 before:[mask-image:linear-gradient(to_bottom,black,transparent)] hover:-translate-y-1 hover:bg-white hover:text-black hover:shadow-white/20 sm:px-[22px] sm:py-[18px]"
