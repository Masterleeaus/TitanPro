@php
    $channels = [
        [
            ...
            'href' => $titan_operator->whatsapp_link ?? '#',
        ],
        [
            'id' => 'telegram',
            'title' => 'Telegram',
            ...
            'href' => $titan_operator->telegram_link ?? '#',
        ],
        [
            ...
            'href' => $titan_operator->watch_product_tour_link ?? '#',
        ],
    ];

    $channels_has_actual_links = isset($titan_operator) && collect($channels)->some(fn($channel) => !empty($channel['href']) && $channel['href'] !== '#');
@endphp
...
@if ($is_editor || (isset($titan_operator) && ($titan_operator->is_links || $titan_operator->is_contact)))
    ...
    >
        @if ($is_editor || $channels_has_actual_links)
            <div
                class="space-y-2"
                @if ($is_editor) x-show="activeTitanOperator.is_links && [activeTitanOperator.whatsapp_link, activeTitanOperator.telegram_link, activeTitanOperator.watch_product_tour_link].some(link => link?.trim() && link !== '#')" @endif
            >
                @foreach ($channels as $channel)
                    @continue(isset($titan_operator) && (empty($channel['href']) || $channel['href'] === '#'))
                    <a
						...
						@if ($is_editor)
							:href="activeTitanOperator.{{ $channel['id'] . '_link' }}"
							x-show="activeTitanOperator.{{ $channel['id'] . '_link' }}?.trim()"
						@else
							href="{{ $channel['href'] }}"
						@endif
						target="_blank"
        ...
        @if ($is_editor || (isset($titan_operator) && $titan_operator->is_contact))
            <a
                ...
                @if ($is_editor) x-show="activeTitanOperator.is_contact" @endif
                href="#"
