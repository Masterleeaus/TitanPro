<script>
    (() => {
        ...
        soundEnabled: {{ isset($titan_operator) ? ($titan_operator->getAttribute('enabled_sound') ?? 'true') : 'true' }},
                ...
                init() {
                    ...
                    @else
                        ...
                        if (window.self !== window.top) {
                            ...
                            document.documentElement.classList.add('lqd-ext-titan_operator-embedded');
                        }
                    @endif
                ...
                connectToAgent() {
                    @if (isset($titan_operator) && isset($session) && \App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator_agent'))
                ...
                handleWindowMessages(event) {
                    switch (event.data.type) {
                        case 'lqd-ext-titan_operator-request-styling':
                ...
                handleStylingResponse(event) {
                    const operatorElStyles = getComputedStyle(this.$el);
                    const styles = {};
                    const attrs = {};
                    [
                    ...
                    ].forEach(attr => styles[attr] = operatorElStyles.getPropertyValue(attr) || '');

                    ['data-pos-x', 'data-pos-y'].forEach(attr => attrs[attr] = this.$el.getAttribute(attr));

                    event.source.postMessage({
                            type: 'lqd-ext-titan_operator-response-styling',
                            data: {
                                styles,
                                attrs
                            },
                ...
                async getSession() {
                    ...
                    if (this.conversations.length) {
                        @if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator_agent') && isset($titan_operator) && isset($session))
                            await this.initAbly();
                        @endif
                    }
