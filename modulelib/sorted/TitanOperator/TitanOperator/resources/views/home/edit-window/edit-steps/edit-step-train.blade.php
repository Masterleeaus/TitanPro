...
<div
    ...
    data-step="3"
    x-data="externalTitanOperatorTraining"
...
>
    <h2 class="mb-3.5">
        @lang('Titan Operator Training')
    </h2>
    <p class="mb-3 text-xs/5 opacity-60 lg:max-w-[360px]">
        @lang('This step is optional but highly recommended to personalize your titan_operator experience.')
    </p>
    <x-button
    ...
    @endphp
    <div class="lqd-ext-titan_operator-training mt-16 flex flex-col justify-center gap-9">
        ...
        <div class="lqd-ext-titan_operator-training-content grid">
            @include('titan_operator::home.training.training-tabs.training-tab-website')
            @include('titan_operator::home.training.training-tabs.training-tab-pdf')
            @include('titan_operator::home.training.training-tabs.training-tab-text')
    ...
    <script>
        (() => {
            document.addEventListener('alpine:init', () => {
                Alpine.data('externalTitanOperatorTraining', () => ({
                    activeTab: 'website',
                    fetching: false,
                    uploading: false,
                    embeddings: [],
                    editingItem: {},
                    init() {
                        this.$data.externalTitanOperatorTraining = this;
                    },
                    setActiveTab(tab) {
                    ...
                    async fetchEmbeddings() {
                        if (!this.activeTitanOperator || this.fetching) return;

                        this.fetching = true;

                        const res = await fetch(`{{ route('dashboard.titan_operator.train.data') }}?id=${this.activeTitanOperator.id}`);
                    ...
                    async trainEmbeddings(event) {
                        ...
                        const embdeddingData = {
                            id: this.activeTitanOperator.id,
                    ...
                    async deleteEmbedding(embeddingId) {
                        ...
                        const res = await fetch('{{ route('dashboard.titan_operator.train.delete') }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                    ...
                    async addUrl(event) {
                        ...
                        formData.append('id', this.activeTitanOperator.id);
