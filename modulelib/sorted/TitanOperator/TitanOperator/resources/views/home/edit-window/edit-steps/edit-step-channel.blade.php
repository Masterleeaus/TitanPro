...
>
	<h2 class="mb-3.5">
		@lang('Titan Operator Channel')
	</h2>
	<p class="mb-3 text-xs/5 opacity-60 lg:max-w-[360px]">
		@lang('This step is optional, but you can enhance your titan_operator experience through different channels.')
	</p>
			...
			document.addEventListener('alpine:init', () => {
				Alpine.data('externalTitanOperatorChannel', () => ({
					...
					openInstagramOauth() {
						...
						@if(\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator_instagram'))
							if (!this.activeTitanOperator || !this.activeTitanOperator.id) {
								toastr.error('{{ __('First, you must select a titan_operator.') }}');
							...
							url.searchParams.set('operator_id', this.activeTitanOperator.id);
					...
					async deleteToClipboard(id) {
						...
						if(confirm('If you delete the channel, all conversations linked to it will stop receiving messages. This may lead to some issues.')) {
							...
							fetch('{{ route('dashboard.titan_operator_multi_channel.delete') }}' , {
								method: 'POST',
								headers: {
								...
								if (data.status === 'success') {
									...
									this.operatorChannels = this.operatorChannels.filter(channel => channel.id !== id);
					...
					async fetchChannels() {
						...
						formData.append('operator_id', this.activeTitanOperator.id)

						fetch('{{ route('dashboard.titan_operator_multi_channel.index') }}', {
							method: 'POST',
							headers: {
								'Accept': 'application/json',
								'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
							...
							if (data.status === 'success') {
								...
								this.operatorChannels = data.data;
					...
					storeChannel(id) {
						...
						fetch(form.action, {
							...
							if (data.status === 'success') {
								...
								this.operatorChannels.push(data.data);
