...
namespace App\Extensions\TitanOperator\System\Generators;
...
use App\Extensions\TitanOperator\System\Tools\KnowledgeBase;
...
class OpenAIGenerator extends Generator
{
        ...
        if ($tools) {
            ...
            foreach ($calls as $call) {
                ...
                $embeding = app(KnowledgeBase::class)
                    ...
                    ->call(
                        EntityEnum::from($this->titan_operator->getAttribute('ai_embedding_model')),
                        $arguments['query'],
                        $this->embeddings
                    );
    ...
    {
        if ($this->titan_operator->instructions) {
            if ($this->titan_operator->do_not_go_beyond_instructions) {
                $this->titan_operator->instructions .= " 
            ...
            array_unshift($histories, [
                'role'    => 'system',
                'content' => $this->titan_operator->instructions,
            ]);
        }

        $this->embeddings = $this->titan_operator->embeddings()->whereNotNull('embedding')->get();
        ...
        if ($this->titan_operator->getAttribute('interaction_type') === InteractionType::SMART_SWITCH) {
            $histories[] = [
                'role'    => 'system',
    ...
    {
        if ($this->titan_operator->human_agent_conditions) {
            return $this->titan_operator->human_agent_conditions;
        }
