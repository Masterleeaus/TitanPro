<script>
    (() => {
        document.addEventListener('alpine:init', () => {
            Alpine.data('externalTitanOperator', (isEditor = false) => ({
                conversations: [],
                ...
                ablyInstance: null,
                ablyChannel: null,
                articles: {!! $is_editor ? '[]' : $titan_operator->articles()->toJson() !!},
                ...
                soundEnabled: {{ isset($titan_operator) ? $titan_operator->getAttribute('enable_sound') ?? 'true' : 'true' }},
                ...
                init() {
                    ...
                    @if ($is_editor)
                        this.$data.externalTitanOperator = this;
                    ...
                    @else
                        ...
                        if (window.self !== window.top) {
                            ...
                            document.documentElement.classList.add('lqd-ext-titan-operator-embedded');
                        }
                    @endif
                ...
                connectToAgent() {
                    @if (isset($titan_operator) && isset($session) && \App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator-agent'))
                        let route = '{{ route('api.v2.titan_operator.conversion.connect.support', [$titan_operator->uuid, $session]) }}';
                ...
                handleWindowMessages(event) {
                    switch (event.data.type) {
                        case 'lqd-ext-titan-operator-request-styling':
                ...
                handleStylingResponse(event) {
                    const titan_operatorElStyles = getComputedStyle(this.$el);
                    const styles = {};
                    const attrs = {};
                    [
                    ...
                    ].forEach(attr => styles[attr] = titan_operatorElStyles.getPropertyValue(attr) || '');

                    ['data-pos-x', 'data-pos-y'].forEach(attr => attrs[attr] = this.$el.getAttribute(attr));

                    event.source.postMessage({
                            type: 'lqd-ext-titan-operator-response-styling',
                            data: {
                                styles,
                                attrs
                            },
                ...
                async getSession() {
                    ...
                    this.activeTitanOperator = data.data;

                    if (this.conversations.length) {
                        @if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator-agent') && isset($titan_operator) && isset($session))
                            await this.initAbly();
                        @endif
                    }
                ...
                async startNewConversation() {
                    ...
                    @else
                        ...
                        const res = await fetch(`{{ isset($routes) ? $routes['conversations'] : '' }}`, {
                            ...
                            body: JSON.stringify({
                                prompt: this.activeTitanOperator.bubble_message
                            })
                        });
                ...
                async onSendMessage() {
                    ...
                    @else
                        ...
                        @if (isset($titan_operator))
                            let condition = '{{ $titan_operator->interaction_type->value }}';

                            if (
                                conversation.connect_agent_at == null &&
                                condition === '{{ \App\Extensions\TitanOperator\System\Enums\InteractionType::SMART_SWITCH }}'
                            ) {
                                this.showConnectButtons = true;
                ...
                onTypingDone(event) {
                    ...
                    if (
                        ...
                        userMessages.length &&
                        this.activeTitanOperator.showCollectEmail
                    ) {
                        this.messages.push({
                            id: 'collect-email',
                            role: 'collect-email',
                        ...
                        this.activeTitanOperator.showCollectEmail = false;
                ...
                async onReceiveMessage(data, loaderMessage) {
                    ...
                    if (data.collect_email) {
                        this.activeTitanOperator.showCollectEmail = true;
                    }
                ...
                async collectEmail(event) {
                    ...
                    if (this.activeTitanOperator.showCollectEmail) return;
					...
					console.log(this.activeTitanOperator);
