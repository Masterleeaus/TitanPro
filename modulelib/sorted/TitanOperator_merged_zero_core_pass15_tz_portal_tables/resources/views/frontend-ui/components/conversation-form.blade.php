<form
    class="lqd-ext-titan-operator-window-form relative min-h-full w-full"
...
>
    ...
    <div class="pointer-events-none absolute end-2 top-[18px] flex gap-3">
        @if ($is_editor || (isset($titan_operator) && $titan_operator->is_attachment))
            <template x-if="@if ($is_editor) true @else activeConversationData?.connect_agent_at @endif">
                <div
                    ...
                    @if ($is_editor) x-show="activeTitanOperator.is_attachment" @endif
                >
        ...
        @endif
        @if ($is_editor || (isset($titan_operator) && $titan_operator->is_emoji))
            <button
                class="pointer-events-auto inline-grid size-[18px] place-items-center p-0"
                @if ($is_editor) x-show="activeTitanOperator.is_emoji" @endif
    ...
    @if ($is_editor || (isset($titan_operator) && $titan_operator->is_emoji))
        <div
            class="lqd-ext-titan-operator-emoji-picker pointer-events-auto absolute -inset-x-4 bottom-full"
            x-ref="emojiPicker"
