...
<template @if (!isset($take)) x-for="article in articles"@else x-for="article in articles.slice(0, {{ $take }})" @endif>
    <div class="lqd-ext-titan-operator-window-articles-list-item group relative py-4">
        <h4
            class="lqd-ext-titan-operator-window-articles-list-item-info-name m-0 mb-2 text-2xs font-semibold text-black"
            x-text="article.title"
        ></h4>
        <p
            class="lqd-ext-titan-operator-window-articles-list-item-info-last-message m-0 line-clamp-2 w-full overflow-hidden text-ellipsis text-2xs opacity-70"
...
<h4
    class="lqd-ext-titan-operator-window-articles-list-no-articles mb-0 py-4 text-sm font-semibold"
    x-show="!articles.length"
    x-cloak
>
    {{ __('No articles yet.') }}
