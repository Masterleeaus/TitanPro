@php
    $connect_agent_enabled =
        isset($titan_operator) &&
        $titan_operator->getAttribute('interaction_type') === \App\Extensions\TitanOperator\System\Enums\InteractionType::SMART_SWITCH &&
        \App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator-agent');
@endphp
...
<template x-for="(message, index) in messages">
    <div>
        ...
        <template x-if="message.role !== 'connecting-to-agent'">
            <div
                class="lqd-ext-titan-operator-window-conversation-message"
                :data-type="message.role"
                :data-id="message.id"
            >
                <figure
                    class="lqd-ext-titan-operator-window-conversation-message-avatar"
                ...
                <div class="lqd-ext-titan-operator-window-conversation-message-content-wrap">
                    <div class="lqd-ext-titan-operator-window-conversation-message-content text-xs/5">
                        <pre
                            class="peer"
                        ...
                        <template x-if="message.role === 'loader'">
                            <span class="lqd-ext-titan-operator-window-conversation-message-loader inline-flex items-center gap-1">
                    ...
                    @if (isset($titan_operator) && $titan_operator['show_date_and_time'])
                        <div
                            class="lqd-ext-titan-operator-window-conversation-message-time"
                    ...
                    @if (!isset($titan_operator))
                        <div
                            class="lqd-ext-titan-operator-window-conversation-message-time"
                            x-show="activeTitanOperator?.show_date_and_time && message.role !== 'collect-email'"
                            x-text="getTimeLabel(message.created_at ?? new Date())"
                        ></div>
                    @endif
                </div>
                ...
                >
                    ...
                    <span class="flex items-center gap-1">
                        ...
                        <template x-if="message.role === 'connect-agent' && connectingToAgent">
                            <span class="lqd-ext-titan-operator-window-conversation-message-loader inline-flex items-center gap-0.5">
