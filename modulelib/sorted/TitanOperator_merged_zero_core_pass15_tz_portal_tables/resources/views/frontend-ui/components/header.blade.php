<div
    class="lqd-ext-titan-operator-header sticky top-0 z-2 bg-cover bg-center p-2"
    {{-- blade-formatter-disable --}}
	@if($is_editor)
	:style="{
		backgroundColor: activeTitanOperator.header_bg_type === 'color' ? activeTitanOperator.header_bg_color : '',
		backgroundImage: activeTitanOperator.header_bg_type === 'gradient' ? activeTitanOperator.header_bg_gradient : activeTitanOperator.header_bg_type === 'image' ? `url(${activeTitanOperator.header_bg_image})` : '',
	}"
	@else
	style="
		@if(isset($titan_operator['header_bg_type']) && $titan_operator['header_bg_type'] === 'color' && isset($titan_operator['header_bg_color']))
			background-color: {{ $titan_operator['header_bg_color'] }};
		@elseif(isset($titan_operator['header_bg_type']) && $titan_operator['header_bg_type'] === 'gradient' && isset($titan_operator['header_bg_gradient']))
			background-image: {{ $titan_operator['header_bg_gradient'] }};
		@elseif(isset($titan_operator['header_bg_type']) && $titan_operator['header_bg_type'] === 'image' && isset($titan_operator['header_bg_image']))
			background-image: url({{ $titan_operator['header_bg_image'] }});
		@endif
	"
	@endif
        ...
        >
            ...
            <div class="grid place-items-center text-xs font-semibold">
                ...
                >
                    ...
                    <span @if ($is_editor) x-text="activeTitanOperator?.title" @endif>
                        @if (isset($titan_operator))
                            {{ $titan_operator['title'] }}
                        @endif
                    </span>
                </div>
                <p
...
{{--                            for="titan_operator-options-sound"--}}
{{--                        >--}}
{{--                            {{ __('Enable Sounds') }}--}}
{{--                            <input--}}
...
{{--                            @if (isset($titan_operator)) :href="`/api/v2/titan_operator/{{ $titan_operator->getAttribute('uuid') }}/session/{{ $session }}/conversation/${activeConversation}/export`"--}}
{{--							@else--}}
