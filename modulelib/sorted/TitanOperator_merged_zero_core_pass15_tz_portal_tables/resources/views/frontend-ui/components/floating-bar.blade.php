<div
    class="lqd-ext-titan-operator-floating-bar absolute inset-x-[18px] bottom-14 z-10 grid rounded-full border px-3 py-1 shadow-2xl backdrop-blur-md transition-all before:pointer-events-none before:absolute before:-inset-px before:rounded-[inherit] before:border before:border-black/5 before:opacity-0 before:transition-opacity before:[mask-image:linear-gradient(to_bottom,black,transparent)] sm:px-4"
...
>
    <div
        @class([
            ...
            'justify-between' => isset($titan_operator) && $titan_operator->is_articles,
            'justify-around' => isset($titan_operator) && !$titan_operator->is_articles,
        ])
        @if ($is_editor) :class="{'justify-between': activeTitanOperator.is_articles, 'justify-around': !activeTitanOperator.is_articles}" @endif
    ...
    <div
        ...
        x-transition
    >
        @include('titan_operator::frontend-ui.components.conversation-form')
    </div>
</div>
