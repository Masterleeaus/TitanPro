...
<div
	...
	>
		...
		@includeIf('titanzero-chat::temp.desc', ['compact' => true, 'tempChat' => $tempChat])
		...
		<div
			...
			<div class="relative flex grow items-center max-md:px-2">
				@php
					$fileChatAllowed = ((int) setting('chatpro_file_chat_allowed', 1) === 1)
				@endphp
				<input
					id="selectImageInput"
					type="file"
				...
				<div
					...
					>
						...
						@includeIf('titanzero-chat::temp.button', ['compact' => true, 'tempChat' => $tempChat, 'category' => $category])
						...
						@if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('canvas') && (bool) setting('ai_chat_pro_canvas', 1))
							@includeIf('titanzero-chat::includes.canvas-button')
						@endif
		...
		/>
		<input
			id="titan_operator_id"
			type="hidden"
			value="{{ $category->titan_operator_id }}"
		/>
		<input
			id="category_id"
			type="hidden"
