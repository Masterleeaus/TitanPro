				...
				<div class="col-md-5 mx-auto">
					...
					>
						...
						<div class="mb-[20px]">
							<label class="form-label">
								{{__('Human Name')}}
								<x-info-tooltip text="{{__('Define a human name for the titan_operator to give it more personality.')}}" />
							</label>
						...
						<div class="mb-[20px]">
							<label class="form-label">
								{{__('Template Role')}}
								<x-info-tooltip text="{{__('A role for the titan_operator that can define what it can help with. For example Finance Expert.')}}" />
							</label>
						...
						<div class="mb-[20px]">
							<label class="form-label">
								{{__('Helps With')}}
								<x-info-tooltip text="{{__('Describe what this titan_operator can help with. It shows when starting a conversation and the titan_operator introducing itself.')}}" />
							</label>
						...
						<div class="mb-[20px]">
							<label class="form-label" for="titan_operator_id">
								{{__('TitanOperator Training')}}
								<x-info-tooltip text="{{__('Choose any trained titan_operator. If you need to train a new titan_operator, visit the TitanOperator Training')}}" />
							</label>
							<select name="titan_operator_id" id="titan_operator_id" class="form-control">
								<option value="0">Select TitanOperator</option>
								@foreach($titan_operators as $titan_operator)
									<option {{ $template?->titan_operator_id == $titan_operator->id ? 'selected': '' }} value="{{ $titan_operator->id }}" > {{ $titan_operator->title }} </option>
								@endforeach
							</select>
						</div>
						<div class="mb-[20px]">
							<label class="form-label" for="assistant">
								{{__('AI Assistants')}}
								<x-info-tooltip text="{{__('Choose your Openai Assistant. If you need to train a new titan_operator, visit the TitanOperator Training')}}" />
							</label>
						...
						{{--						<div id="titan_operator_training_json" class="mb-[20px] {{ is_numeric($template?->titan_operator_id) && $template?->titan_operator_id != '0' ? 'd-none' : '' }}" >--}}
						{{--							<label class="form-label">--}}
						{{--								{{__('TitanOperator Training')}}--}}
...
@push('script')
	<script>
		$(document).ready(function() {
			$('#titan_operator_id').on('change', function () {

				let value = $(this).val();

				if(value == '0')
				{
					$('#titan_operator_training_json').removeClass('d-none');
				}else{
					$('#titan_operator_training_json').addClass('d-none');
				}
			});
		});
	</script>
