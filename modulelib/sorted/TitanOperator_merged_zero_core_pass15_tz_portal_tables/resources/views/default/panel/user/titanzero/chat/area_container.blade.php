...
>
	...
	>
						...
						<div class="lqd-marquee-viewport relative flex w-full overflow-hidden">
							<div class="lqd-marquee-slider flex w-full gap-4 py-2 lg:px-14">
								@foreach ($example_prompts ?? [] as $prompt)
									...
									>
                                        <span
											class="bg-gradient-to-r from-gradient-from via-gradient-via to-gradient-to bg-clip-text text-transparent">
                                            {{ __($prompt?->name) }}
                                        </span>
									</button>
								@endforeach
		...
		@if (view()->hasSection('chat_form'))
			@yield('chat_form')
		@else
			@include('titanzero-chat::includes.chat_form')
		@endif
	</div>
	<div
		...
		document.addEventListener('alpine:init', () => {
				...
				async setCanvasActive(active = true) {
					...
					if (active) {
						...
						if (window.confirm('{{ __('Do you want to save?') }}')) {
							...
							try {
								const res = await fetch('{{ route('user.tzc.canvas.content.store') }}', {
									method: 'post',
									headers: {
			...
			Alpine.data('aiActions', () => ({
				...
				rewrite() {
					...
					$.ajax({
						type: 'post',
						url: '{{ route('dashboard.user.titanzero.api.update-writing') }}',
						data: formData,
						contentType: false,
						processData: false,
