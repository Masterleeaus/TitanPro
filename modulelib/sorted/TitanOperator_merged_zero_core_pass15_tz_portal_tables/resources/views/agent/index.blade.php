@php
    ...
    if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator_telegram')) {
        $channel_filters['telegram'] = [
            'label' => __('Telegram'),
        ];
    }

    if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator_whatsapp')) {
        $channel_filters['whatsapp'] = [
            'label' => __('Whatsapp'),
        ];
    }

    if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator_messenger')) {
...
@push('css')
    ...
    />
    <style>
        .lqd-titan_operator-emoji .picmo__picker.picmo__picker {
        ...
        .lqd-titan_operator-emoji .picmo__picker .picmo__searchContainer .picmo__searchField {
            border-radius: 8px;
            font-size: 14px;
        }

        .lqd-titan_operator-emoji .picmo__picker .picmo__emojiButton {
            border-radius: 6px;
        }

        .lqd-titan_operator-emoji .picmo__picker .picmo__preview {
            display: none;
        }
...
@section('content')
    <div
        class="lqd-ext-titan_operator-history flex h-screen flex-col max-lg:[--header-height:65px] lg:flex-row"
        x-data="operatorAgentMessages"
    >
        <div
            class="lqd-ext-titan_operator-history-sidebar group/history-sidebar relative flex shrink-0 flex-col bg-foreground/[3%] lg:w-[clamp(250px,27%,400px)]"
            :class="{ 'mobile-dropdown-open': mobileDropdownOpen }"
        >
            @includeIf('titan_operator_agent::particles.conversations-filter')
            ...
            >
                @includeIf('titan_operator_agent::particles.conversations-sort')

                @includeIf('titan_operator_agent::particles.conversations-channel-filter')

                @include('titan_operator_agent::particles.conversations-list')
            </div>
        </div>

        <div
            class="lqd-ext-titan_operator-history-content-wrap flex h-full grow flex-col overflow-y-auto lg:w-1/2"
            x-ref="historyContentWrap"
        >
            @include('titan_operator_agent::particles.messages-header')

            <div
                class="lqd-ext-titan_operator-history-messages relative flex h-full flex-col gap-2 pt-10"
                x-ref="historyMessages"
            >
                <div class="mt-auto space-y-2 px-4 xl:px-10">
                    @include('titan_operator_agent::particles.messages-list')
                </div>

                @include('titan_operator_agent::particles.messages-form')
            </div>
        </div>

        <div
            class="lqd-ext-titan_operator-contact-info flex flex-col border-s transition-all max-lg:fixed max-lg:bottom-0 max-lg:top-[calc(var(--header-height)+3.5rem)] max-lg:z-10 max-lg:h-0 max-lg:max-h-[calc(100%-var(--header-height)-3.5rem)] max-lg:w-full max-lg:overflow-hidden max-lg:bg-background/90 max-lg:backdrop-blur-lg lg:w-[clamp(250px,27%,400px)] max-lg:[&.active]:h-full"
            :class="{ 'active': mobile.contactInfoVisible }"
        >
            @include('titan_operator_agent::particles.contact-info-head')

            <div class="grid grow grid-cols-1 place-items-start overflow-y-auto">
                @include('titan_operator_agent::particles.contact-info-tab-details')
                @include('titan_operator_agent::particles.contact-info-tab-history')
            </div>
        </div>
    </div>
@endsection
    ...
    <script>
        (() => {
            document.addEventListener('alpine:init', () => {
                Alpine.data('operatorAgentMessages', () => ({
                    selectedStatus: {
                        label: '{{ __('All') }}',
                        count: 0,
                        status: 'all'
                    ...
                    async init() {
                        ...
                        Alpine.store('operatorAgentMessages', this);
                    ...
                    async updateConversationDetails({
                        ...
                        @else
                            ...
                            const res = await fetch('{{ route('dashboard.titan_operator_agent.conversations.update') }}', {
                                method: 'PUT',
                                headers: {
                    ...
                    async initAbly() {
						...
						await channel.subscribe("conversation", async message => {
							const newMessageData = message.data.operatorConversation;
                    ...
                    async onSendMessage() {
                        ...
                        @else
                            ...
                            const sendFirstMessage = async () => {
                                ...
                                const res = await fetch('{{ route('dashboard.titan_operator_agent.history') }}', {
                                    method: 'POST',
                                    headers: {
                                        'Accept': 'application/json',
                                    },
                    ...
                    async fetchHistories(id) {
                        const res = await fetch('{{ route('dashboard.titan_operator_agent.history') }}?conversation_id=' + id, {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                    ...
                    async fetchChats(opts = {}) {
                        ...
                        const res = await fetch(
                            `${this.$refs.loadMore.href}&agentFilter=${this.filters.agent}&operator_channel=${this.filters.channel}&status=${this.filters.status}&unread=${this.filters.unreadsOnly}&sort=${this.filters.sort}`, {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                    ...
                    async handleConversationsSearch() {
                        ...
                        const res = await fetch('{{ route('dashboard.titan_operator_agent.conversations.search') }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                    ...
                    async exportHistory(conversationId) {
                        ...
                        @else
                            ...
                            fetch('{{ route('dashboard.titan_operator_agent.history') }}?conversation_id=' + conversationId)
                    ...
                    async closeConversation(conversationId) {
                        ...
                        const res = await fetch('{{ route('dashboard.titan_operator_agent.conversations.closed') }}?conversation_id=' + conversationId, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                    ...
                    async pinConversation(conversationId) {
                        ...
                        const res = await fetch('{{ route('dashboard.titan_operator_agent.conversations.pinned') }}?conversation_id=' + conversationId, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                    ...
                    async getConversationsHistory(sessionId) {
                        ...
                        const res = await fetch(
                            `{{ route('dashboard.titan_operator_agent.conversations.history.session') }}?sessionId=${sessionId}`, {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                    ...
                    async deleteConversation(conversationId) {
                        ...
                        @else
                            ...
                            const res = await fetch('{{ route('dashboard.titan_operator_agent.destroy') }}', {
                                method: 'DELETE',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
