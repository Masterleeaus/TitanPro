<div>
    <x-forms.input
        ...
        size="lg"
        type="select"
        x-model="activeTitanOperator.interaction_type"
    >
        <option value="{{ \App\Extensions\TitanOperator\System\Enums\InteractionType::SMART_SWITCH->value }}">
            @lang('AI & Human Agent')
        </option>
        <option value="{{ \App\Extensions\TitanOperator\System\Enums\InteractionType::AUTOMATIC_RESPONSE->value }}">
            @lang('Only AI')
        </option>
        <option value="{{ \App\Extensions\TitanOperator\System\Enums\InteractionType::HUMAN_SUPPORT->value }}">
...
<div
    ...
    x-cloak
    x-show="activeTitanOperator.interaction_type === 'smart_switch'"
>
            ...
            <div class="mb-8 space-y-3">
                @foreach ($human_agent_conditions as $condition)
                    <x-forms.input
                        ...
                        type="checkbox"
                        custom
                        ::checked="activeTitanOperator.human_agent_conditions?.includes($el.getAttribute('data-condition'))"
    ...
    <select
        ...
        multiple
        x-model="activeTitanOperator.human_agent_conditions"
    >
        @foreach ($human_agent_conditions as $condition)
...
<div>
    <x-forms.input
        ...
        size="lg"
        x-model="activeTitanOperator.connect_message"
