...
>
    <span
        ...
        id="inbox-notification-titan-operator-agent"
    >0</span>
    <x-tabler-inbox
...
@push('script')
    @if (config('titan_operator.notification_enabled', true))
        <script>
            $(document).ready(function() {
                // Function to fetch the notification count
                function fetchNotificationCount() {
                    $.ajax({
                        url: '{{ route('dashboard.titan_operator_agent.notification.count') }}',
                        type: 'GET',
                        success: function(data) {
                            let inboxNotification = $('#inbox-notification-titan-operator-agent');
