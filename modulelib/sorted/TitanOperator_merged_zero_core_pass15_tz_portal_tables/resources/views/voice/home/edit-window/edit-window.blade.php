...
<template x-teleport="body">
    <div
        class="lqd-titan_operator-edit-window fixed bottom-0 end-0 start-0 top-0 z-[100] overflow-y-auto bg-background lg:start-[--navbar-width]"
        x-show="activeTitanOperator.id"
        :class="{ active: activeTitanOperator }"
    >
        {{-- Edit Window Header --}}
        <div
            class="lqd-titan_operator-edit-window-header sticky top-0 z-2 border-b bg-background/60 backdrop-blur-lg backdrop-saturate-150">
            <div class="flex flex-wrap items-center justify-between gap-4 px-3 py-3 lg:h-[--header-height] lg:px-12">
                ...
                <div class="lqd-steps hidden flex-col gap-1 lg:flex">
                    <div class="lqd-steps-steps flex items-center justify-between gap-1 lg:gap-3">
                        @foreach (\App\Extensions\TitanOperatorVoice\System\Enums\StepEnum::toArray() as $step)
                            <button
        ...
        <div class="lqd-titan_operator-edit-window-content py-8 lg:py-0">
            <div class="container flex flex-wrap justify-center gap-y-5">
                {{-- Options Container --}}
                <div
                    class="lqd-titan_operator-edit-window-options grid w-full lg:min-h-[calc(100vh-var(--header-height))] lg:w-[550px] lg:py-16">
                    <input
                        type="hidden"
                        name="id"
                        x-model="activeTitanOperator.id"
                    >
                    @include('titan_operator-voice::home.edit-window.edit-steps.edit-step-configure')
                    @include('titan_operator-voice::home.edit-window.edit-steps.edit-step-customize', [
                        'avatars' => $avatars,
                    ])
                    @include('titan_operator-voice::home.edit-window.edit-steps.edit-step-train')
                    @include('titan_operator-voice::home.edit-window.edit-steps.edit-step-embed')
                    <div
            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('customAvatar', () => ({
                    ...
                    async onPickerChange() {
                        ...
                        await fetch('{{ route('dashboard.titan_operator-voice.upload.avatar') }}', {
                                method: 'POST',
                                body: formData,
                            })
                            .then(response => {
