...
@section('title', 'Titan Operator Automation')
        ...
        <div class="row row-cards">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h3 class="card-title">
                            Automation for: {{ $titan_operator->title ?? $titan_operator->name ?? $titan_operator->uuid }}
                        </h3>
                        <p class="text-muted mb-3">
                            Manage workflows, tools, and the external tool gateway for this titan_operator. Execution is webhook-first.
                        </p>
                        ...
                        <form method="POST" action="{{ route('dashboard.titan_operator.automation.update', $titan_operator) }}" class="mt-3">
                            @csrf
                                ...
                                <div class="tab-pane" id="tab-gateway">
                                    <div class="row g-3">
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control" name="gateway[external_endpoint_url]" value="{{ old('gateway.external_endpoint_url', $titan_operator->external_endpoint_url ?? '') }}" placeholder="https://your-saas.com/api/tool-gateway">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select class="form-select" name="gateway[external_auth_type]">
                                                @php($auth = old('gateway.external_auth_type', $titan_operator->external_auth_type ?? ''))
                                        ...
                                        <div class="col-md-8">
                                            ...
                                            <input type="text" class="form-control" name="gateway[external_auth_token]" value="{{ old('gateway.external_auth_token', $titan_operator->external_auth_token ?? '') }}" placeholder="token">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label">HMAC Signing Secret</label>
                                            <input type="text" class="form-control" name="gateway[external_signing_secret]" value="{{ old('gateway.external_signing_secret', $titan_operator->external_signing_secret ?? '') }}" placeholder="optional">
                                            <div class="form-hint">If set, requests include X-Workflow-Signature.</div>
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label">Timeout (ms)</label>
                                            <input type="number" class="form-control" name="gateway[external_timeout_ms]" value="{{ old('gateway.external_timeout_ms', $titan_operator->external_timeout_ms ?? 15000) }}" min="1000" step="500">
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label">Rate limit / min</label>
                                            <input type="number" class="form-control" name="gateway[limit_per_minute]" value="{{ old('gateway.limit_per_minute', $titan_operator->limit_per_minute ?? 60) }}" min="1" step="1">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="d-flex gap-2 mt-3">
                                ...
                                <a class="btn btn-outline-secondary" href="{{ route('dashboard.titan_operator.index') }}">Back</a>
                                <a class="btn btn-outline-secondary" href="{{ route('dashboard.titan_operator.workflows.index', $titan_operator) }}">Legacy Workflows Page</a>
                            </div>
                        </form>
                    </div>
                </div>
