<div class="h-full overflow-y-auto">
    <ul class="lqd-ext-titan-operator-history-list">
        <template
            x-for="(chatItem, index) in chatsList"
            :key="chatItem.id"
        >
            <li
                class="lqd-ext-titan-operator-history-list-item group/chat-item relative border-b px-6 py-4 before:absolute before:inset-x-2.5 before:inset-y-1.5 before:z-0 before:scale-95 before:rounded-xl before:bg-accent/10 before:opacity-0 before:transition [&.active]:before:scale-100 [&.active]:before:opacity-100"
            ...
            >
                <div class="relative z-1 flex gap-2.5">
                    <figure class="inline-grid size-8 shrink-0 place-items-center rounded-full font-heading text-xs font-semibold uppercase text-white">
                        <template x-if="chatItem?.titan_operator_channel === 'frame'">
                            <svg
                        ...
                        <template x-if="chatItem?.titan_operator_channel === 'whatsapp'">
                            <svg
                                width="31"
                                height="31"
                                viewBox="0 0 31 31"
                        ...
                        <template x-if="chatItem?.titan_operator_channel === 'telegram'">
                            <svg
                                width="32"
                                height="31"
                                viewBox="0 0 32 31"
                    ...
                    <div class="flex w-10/12 grow gap-1">
                        <div class="max-w-full grow overflow-hidden text-start">
                            ...
                            <p
                                ...
                                x-text="`@${chatItem.titan_operator_channel === 'frame' ? '{{ __('livechat') }}' : chatItem.titan_operator_channel}`"
                            ></p>
                            <p
                ...
                <a
                    class="lqd-ext-titan-operator-history-list-item-trigger absolute start-0 top-0 z-2 inline-block h-full w-full"
                    href="#"
    ...
    <div
        class="lqd-ext-titan-operator-history-load-wrap grid place-items-center p-6 font-medium text-heading-foreground"
        x-ref="loadMoreWrap"
    >
        <x-button
            class="lqd-ext-titan-operator-history-load-more col-start-1 col-end-1 row-start-1 row-end-1 w-full"
            variant="link"
            href="{{ route('dashboard.titan_operator.conversations.with.paginate', ['page' => 1]) }}"
            x-ref="loadMore"
        ...
        <span
            class="lqd-ext-titan-operator-history-loading col-start-1 col-end-1 row-start-1 row-end-1 inline-flex gap-2"
            x-show="!allLoaded && fetching"
            x-ref="loading"
        >
            {{ __('Loading') }}
        ...
        <span
            class="lqd-ext-titan-operator-history-all-loaded col-start-1 col-end-1 row-start-1 row-end-1 inline-flex gap-2"
            x-ref="allLoaded"
