@php
    ...
    if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator-telegram')) {
        $channel_filters['telegram'] = [
            'label' => __('Telegram'),
        ];
    }

    if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator-whatsapp')) {
...
@endphp
<div
    class="lqd-ext-titan-operator-history invisible fixed end-0 start-0 top-0 z-10 flex h-screen bg-background opacity-0 transition-all max-sm:block lg:start-[--navbar-width] lg:group-[&.focus-mode]/body:start-0 [&.lqd-open]:visible [&.lqd-open]:opacity-100"
    x-data="externalTitanOperatorHistory"
    :class="{ 'lqd-open': open }"
    @keydown.window.escape="setOpen(false)"
>
    <div
        class="lqd-ext-titan-operator-history-sidebar group/history-sidebar relative flex shrink-0 flex-col bg-foreground/[3%] lg:w-[clamp(250px,27%,400px)]"
        :class="{ 'mobile-dropdown-open': mobileDropdownOpen }"
    >
        @includeIf('titan_operator::home.chats-history.particles.conversations-filter')
        ...
        >
            @includeIf('titan_operator-agent::particles.conversations-sort')

            @includeIf('titan_operator::home.chats-history.particles.conversations-channel-filter')

            @include('titan_operator::home.chats-history.particles.conversations-list')
        </div>
    </div>

    <div
        class="lqd-ext-titan-operator-history-content-wrap flex h-full grow flex-col overflow-y-auto lg:w-1/2"
        x-ref="historyContentWrap"
    >
        @include('titan_operator::home.chats-history.particles.messages-header')

        <div
            class="lqd-ext-titan-operator-history-messages relative flex h-full flex-col gap-2 py-10"
            x-ref="historyMessages"
        >
            <div class="mt-auto space-y-2 px-4 xl:px-10">
                @include('titan_operator::home.chats-history.particles.messages-list')
            </div>
        </div>
    </div>

    <div
        class="lqd-ext-titan-operator-contact-info flex flex-col border-s transition-all max-lg:fixed max-lg:bottom-0 max-lg:top-[calc(var(--header-height)+3.5rem)] max-lg:z-10 max-lg:h-0 max-lg:max-h-[calc(100%-var(--header-height)-3.5rem)] max-lg:w-full max-lg:overflow-hidden max-lg:bg-background/90 max-lg:backdrop-blur-lg lg:w-[clamp(250px,27%,400px)] max-lg:[&.active]:h-full"
        :class="{ 'active': mobile.contactInfoVisible }"
    >
        @include('titan_operator::home.chats-history.particles.contact-info-head')

        <div class="grid grow grid-cols-1 place-items-start overflow-y-auto">
            @include('titan_operator::home.chats-history.particles.contact-info-tab-details')
            {{-- @include('titan_operator::home.chats-history.particles.contact-info-tab-history') --}}
        </div>
    </div>
</div>
    ...
    <script>
        (() => {
            document.addEventListener('alpine:init', () => {
                Alpine.data('externalTitanOperatorHistory', () => ({
                    filters: {
                    ...
                    async init() {
                        ...
                        Alpine.store('externalTitanOperatorHistory', this);
                    },
                    async loadMore() {
                    ...
                    async fetchChats(opts = {}) {
                        ...
                        const res = await fetch(
                            `${this.$refs.loadMore.href}&agentFilter=${this.filters.agent}&titan_operator_channel=${this.filters.channel}&status=${this.filters.status}&unread=${this.filters.unreadsOnly}&sort=${this.filters.sort}`, {
                                method: 'GET',
                                headers: {
                    ...
                    async handleConversationsSearch() {
                        ...
                        const res = await fetch('{{ route('dashboard.titan_operator.conversations.search') }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
