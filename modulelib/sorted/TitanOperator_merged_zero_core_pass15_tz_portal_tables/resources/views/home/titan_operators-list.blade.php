...
<div class="py-14">
    <h2 class="mb-9">
        @lang('Active TitanOperators')
    </h2>

    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        {{-- Newly added items --}}
        <template
            x-for="titan_operator in titan_operators.data.filter(c => c.id !== 'new_titan_operator')"
            :key="titan_operator.id"
        >
            <x-card size="md">
                <x-slot:head
                ...
                >
                    <figure>
                        <img
                            ...
                            width="40"
                            height="40"
                            :src="`${window.location.origin}/${titan_operator.avatar}`"
                            ...
                            @php
                                $dropdown_items = [
                                    [
                                        ...
                                        'link' => '#',
                                        'attrs' => [
                                            '@click.prevent' => 'setActiveTitanOperator(titan_operator.id, 1, true); toggle("collapse")',
                                        ],
                                    ],
                                    [
                                        'label' => __('Customize'),
                                        'link' => '#',
                                        'attrs' => [
                                            '@click.prevent' => 'setActiveTitanOperator(titan_operator.id, 2, true); toggle("collapse")',
                                        ],
                                    ],
                                    [
                                        'label' => __('Train'),
                                        'link' => '#',
                                        'attrs' => [
                                            '@click.prevent' => 'setActiveTitanOperator(titan_operator.id, 3); toggle("collapse")',
                                        ],
                                    ],
                                    [
                                        'label' => __('Test & Embed'),
                                        'link' => '#',
                                        'attrs' => [
                                            '@click.prevent' => 'setActiveTitanOperator(titan_operator.id, 4, true); toggle("collapse")',
                                        ],
                                    ],
                                ];
                                ...
                                if (\App\Extensions\TitanOperator\System\Helpers\TitanOperatorHelper::existChannels()) {
                                    $dropdown_items[] = [
                                        ...
                                        'link' => '#',
                                        'attrs' => [
                                            '@click.prevent' => 'setActiveTitanOperator(titan_operator.id, 5, true); toggle("collapse")',
                                        ],
                                    ];
                                }
                            ...
                            <ul class="py-1 text-xs font-medium">
                                ...
                                <li :class="{ 'opacity-50': submittingData, 'pointer-events-none': submittingData }">
                                    <x-forms.input
                                        ...
                                        type="checkbox"
                                        switcher
                                        ::id="`active-titan_operator-${titan_operator.id}`"
                                        ::checked="titan_operator.active"
                                        @change="toggleTitanOperatorActivation(titan_operator.id);"
                                        x-model="titan_operator.active"
                                        x-init="$el.closest('label').setAttribute('for', `active-titan_operator-${titan_operator.id}`)"
                                    />
                                </li>
                                <li :class="{ 'opacity-50': submittingData, 'pointer-events-none': submittingData }">
                                    <form
                                        action="{{ route('dashboard.titan_operator.delete') }}"
                                        @submit.prevent="deleteTitanOperator"
                                    >
                                        <input
                                            type="hidden"
                                            :value="titan_operator.id"
                                            name="id"
                                        >
                                        <x-button
                ...
                <h3
                    class="mb-2.5"
                    x-text="titan_operator.title"
                ></h3>
                <p
                    class="mb-2.5 text-sm font-medium text-heading-foreground/50"
                    x-data="{ diff: Math.floor((new Date() - new Date(titan_operator.created_at)) / 1000) }"
                    x-init="if (Math.floor((new Date() - new Date(titan_operator.created_at)) / 1000) < 60) { setInterval(() => { diff = Math.floor((new Date() - new Date(titan_operator.created_at)) / 1000); }, 1000); }"
                >
                    @lang('Created')
                    <span
                        x-text="
                ...
                <div class="flex justify-between">
                    <div
                        ...
                        :class="{
                            'lqd-active': titan_operator.active,
                            'lqd-passive': !titan_operator.active
                        }"
                    >
                        <x-tabler-check
                            class="size-4"
                            ::class="{ hidden: !titan_operator.active }"
                        />
                        <span
                    ...
                    <div class="flex gap-1">
                        <template x-if="titan_operator?.channels?.some(c => c.channel === 'messenger')">
                            <x-tabler-brand-messenger class="size-5" />
                        </template>
                        <template x-if="titan_operator?.channels?.some(c => c.channel === 'whatsapp')">
                            <x-tabler-brand-whatsapp class="size-5" />
                        </template>
                        <template x-if="titan_operator?.channels?.some(c => c.channel === 'telegram')">
