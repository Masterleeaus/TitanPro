...
>
    ...
    <p class="text-xs/5 opacity-60 lg:max-w-[360px]">
        @lang('Create and configure a client portal that interacts with your users, captures enquiries, and delivers accurate information.')
    </p>
    ...
    <div class="flex flex-col gap-7 pt-9">
        <div x-data="logoPreviewHandler">
            <x-forms.input
                ...
                size="lg"
                x-bind="logoPicker"
                @change="externalTitanOperator && externalTitanOperator.toggleWindowState('open');"
            />
            <input
                type="hidden"
                x-model="uploadedLogo"
                x-modelable="activeTitanOperator.logo"
            >
        ...
        <div>
            ...
            <p class="mb-5 w-full text-sm">
                @lang('Select an avatar for your titan_operator.')
            </p>
            <div class="grid grid-cols-3 gap-x-2 gap-y-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
                <div class="lqd-titan_operator-avatar-list contents">
                    @forelse ($avatars as $avatar)
                        <div class="lqd-titan_operator-avatar-list-item relative flex justify-center">
                            <input
                                ...
                                name="avatar"
                                value="{{ $avatar['avatar'] }}"
                                x-model="activeTitanOperator.avatar"
                            />
                            <label
                    ...
                    @empty
                        <div class="lqd-titan_operator-avatar-list-item relative flex hidden justify-center">
                            <input
                                class="peer invisible size-0"
                                type="radio"
                                name="avatar"
                ...
                @foreach ($predefined_colors as $color)
                    <div class="relative flex justify-center">
                        <input
                            ...
                            name="color"
                            value="#{{ $color }}"
                            x-model="activeTitanOperator.color"
                        />
                        <label
        ...
        <div>
            ...
            <div class="lqd-tabs">
                <div class="lqd-tabs-triggers mb-3.5 flex gap-2 rounded-lg bg-heading-foreground/5 p-1">
                    <button
                        ...
                        type="button"
                        :class="{ 'active': activeTitanOperator.header_bg_type === 'color' }"
                        @click.prevent="activeTitanOperator.header_bg_type = 'color'"
                    >
                        @lang('Solid Color')
                    </button>
                    ...
                    <button
                        ...
                        type="button"
                        :class="{ 'active': activeTitanOperator.header_bg_type === 'gradient' }"
                        @click.prevent="activeTitanOperator.header_bg_type = 'gradient'"
                    >
                        @lang('Gradient')
                    </button>
                    ...
                    <button
                        class="grow justify-center rounded p-1 text-center text-2xs font-medium [&.active]:bg-background [&.active]:text-heading-foreground [&.active]:shadow-lg [&.active]:shadow-black/5"
                        type="button"
                        :class="{ 'active': activeTitanOperator.header_bg_type === 'image' }"
                        @click.prevent="activeTitanOperator.header_bg_type = 'image'"
                    >
                        @lang('Image')
                    </button>
                </div>
                ...
                <div class="lqd-tabs-contents">
                    ...
                    >
                        <div
                            ...
                            x-data="liquidColorPicker({ colorVal: activeTitanOperator.header_bg_color })"
                            x-effect="picker && picker.setColor(activeTitanOperator.header_bg_color)"
                        >
                            <span
                            ...
                            <input
                                ...
                                type="text"
                                x-model="activeTitanOperator.header_bg_color"
                                x-ref="colorInput"
                    ...
                    >
                        <div
                            class="space-y-2"
                            x-data="gradientPicker({ gradient: activeTitanOperator.header_bg_gradient })"
                            x-init="$watch('activeTitanOperator.id', (value) => {
                                gradient = activeTitanOperator.header_bg_gradient ?? '';
                        ...
                        >
                            ...
                            <input
                                ...
                                x-model="gradient"
                                x-modelable="activeTitanOperator.header_bg_gradient"
                            >
                        </div>
                    </div>
                    <div
                    ...
                    >
                        <x-forms.input
                            ...
                            size="lg"
                            @change="$event.target.files[0] && (activeTitanOperator.header_bg_image_blob = $event.target.files[0]) && (activeTitanOperator.header_bg_image = URL.createObjectURL($event.target.files[0]))"
                        />
                    </div>
                </div>
            </div>
            ...
            <input
                type="hidden"
                name="header_bg_type"
                x-model="activeTitanOperator.header_bg_type"
            >
        </div>
        ...
        <div>
            <x-forms.input
                ...
                ::checked="activeTitanOperator.is_email_collect === 1"
                x-model.boolean="activeTitanOperator.is_email_collect"
                @change="externalTitanOperator.toggleWindowState('open')"
            />
            <template
        ...
        <div>
            <x-forms.input
                ...
                switcher
                name="is_contact"
                ::checked="activeTitanOperator.is_contact === 1"
                x-model.boolean="activeTitanOperator.is_contact"
                @change="externalTitanOperator.toggleWindowState('open')"
            />
            <template
        ...
        <div>
            <x-forms.input
                ...
                switcher
                name="is_emoji"
                ::checked="activeTitanOperator.is_emoji === 1"
                x-model.boolean="activeTitanOperator.is_emoji"
                @change="externalTitanOperator.toggleWindowState('open')"
            />
            <template
        ...
        <div>
            <x-forms.input
                ...
                switcher
                name="is_links"
                ::checked="activeTitanOperator.is_links === 1"
                x-model.boolean="activeTitanOperator.is_links"
                @change="externalTitanOperator.toggleWindowState('open')"
            />
            <template
        ...
        <div>
            <x-forms.input
                ...
                switcher
                name="is_contact"
                ::checked="activeTitanOperator.is_articles === 1"
                x-model.boolean="activeTitanOperator.is_articles"
                @change="externalTitanOperator.toggleWindowState('open')"
            />
            <template
        ...
        <div>
            <x-forms.input
                ...
                switcher
                name="show_logo"
                ::checked="activeTitanOperator.show_logo === 1"
                x-model.boolean="activeTitanOperator.show_logo"
                @change="externalTitanOperator.toggleWindowState('open')"
            />
            <template
        ...
        <div>
            <x-forms.input
                ...
                ::checked="activeTitanOperator.show_date_and_time === 1"
                x-model.boolean="activeTitanOperator.show_date_and_time"
                @change="if ( externalTitanOperator ) { externalTitanOperator.toggleWindowState('open'); externalTitanOperator.toggleView('conversation-messages') }"
            />
        ...
        <div>
            <x-forms.input
                ...
                ::checked="activeTitanOperator.trigger_background === 'transparent'"
                @change="
                    $event.target.checked ? activeTitanOperator.trigger_background = 'transparent' : activeTitanOperator.trigger_background = '';
                    externalTitanOperator && externalTitanOperator.toggleWindowState('close');
                "
                x-model="activeTitanOperator.trigger_background"
            />
        ...
        <div
            ...
            x-data="{ currentVal: `${parseInt(activeTitanOperator.trigger_avatar_size || 60, 10)}px` }"
        >
            <label
            ...
            <div class="flex items-center gap-3">
                <input
                    ...
                    @input="
                        ...
                        activeTitanOperator.trigger_avatar_size = currentVal;
                        externalTitanOperator && externalTitanOperator.toggleWindowState('close');
                    "
        ...
        <div>
            ...
            <div class="flex flex-wrap justify-between gap-2">
                @foreach (\App\Extensions\TitanOperator\System\Enums\PositionEnum::toArray() as $position)
                    <div class="relative text-center">
                        <input
                            ...
                            name="position"
                            value="{{ $position }}"
                            x-model="activeTitanOperator.position"
                        />
                        <label
                            @class([
        ...
        <div class="">
            ...
            <div>
                <x-forms.input
                    ...
                    name="footer_link"
                    size="lg"
                    x-model="activeTitanOperator.footer_link"
                    @input.throttle.250ms="externalTitanOperator && externalTitanOperator.toggleWindowState('open'); externalTitanOperator.toggleView('welcome')"
                />
            ...
            <div class="mt-3">
                <x-forms.input
                    ...
                    name="whatsapp_link"
                    size="lg"
                    x-model="activeTitanOperator.whatsapp_link"
                    @input.throttle.250ms="externalTitanOperator && externalTitanOperator.toggleWindowState('open'); externalTitanOperator.toggleView('welcome')"
                />
            ...
            <div class="mt-3">
                <x-forms.input
                    ...
                    name="telegram_link"
                    size="lg"
                    x-model="activeTitanOperator.telegram_link"
                    @input.throttle.250ms="externalTitanOperator && externalTitanOperator.toggleWindowState('open'); externalTitanOperator.toggleView('welcome')"
                />
            ...
            <div class="mt-3">
                <x-forms.input
                    ...
                    size="lg"
                    x-model="activeTitanOperator.watch_product_tour_link"
                    @input.throttle.250ms="externalTitanOperator && externalTitanOperator.toggleWindowState('open'); externalTitanOperator.toggleView('welcome')"
                />
