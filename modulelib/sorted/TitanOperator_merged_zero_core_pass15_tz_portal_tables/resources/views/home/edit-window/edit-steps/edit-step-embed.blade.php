...
>
    ...
    <p class="mb-14 text-xs/5 opacity-60 lg:max-w-[360px]">
        @lang('Your client portal has been configured successfully. You can now embed it into your website and start engaging with your audience.')
    </p>
    ...
    <div class="mb-14">
        ...
        >
            &lt;script
            defer
            src="{{ url('vendor/titan_operator/js/external-titan_operator.js') }}"
            data-titan_operator-uuid="<span x-text="activeTitanOperator.uuid"></span>"
    ...
    <div class="mt-8 rounded-2xl border border-heading-foreground/10 bg-heading-foreground/5 p-5">
        <label class="mb-3 block text-xs font-semibold text-heading-foreground">
            @lang('Portal Launch Links')
        </label>
        <div class="space-y-3 text-2xs">
            <div class="rounded-xl bg-background p-3">
                <div class="mb-1 font-medium">@lang('Direct Launch URL')</div>
                <div class="break-all opacity-70">{{ url('titan_operator') }}/<span x-text="activeTitanOperator.uuid"></span>/frame</div>
            </div>
            <div class="rounded-xl bg-background p-3">
                <div class="mb-1 font-medium">@lang('PWA Runtime')</div>
                <div class="break-all opacity-70">{{ url('vendor/titan-operator-zero/manifest.webmanifest') }}</div>
            </div>
        </div>
        <p class="mt-3 text-xs/5 opacity-60">@lang('Use the direct launch URL for browser embeds, and publish the runtime manifest if you want an installable app-like portal experience.') </p>
    </div>
    ...
    <div>
        ...
        <p class="mb-3 text-xs/5 opacity-60 lg:max-w-[360px]">
            @lang('Paste this code just before the closing &lt;/body&gt; tag in your HTML file, then save the changes. Refresh your site to ensure your client portal loads correctly.')
        </p>
    </div>
</div>
