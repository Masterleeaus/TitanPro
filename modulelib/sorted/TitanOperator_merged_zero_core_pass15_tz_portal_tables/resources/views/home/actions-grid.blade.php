        ...
        <div class="flex flex-col gap-y-4 md:flex-row">
			...
			@if(\App\Helpers\Classes\MarketplaceHelper::isRegistered('titan_operator-agent'))
				<a
					...
					href="{{ route('dashboard.titan_operator-agent.index') }}"
				>
...
<div class="grid grid-cols-1 gap-5 lg:grid-cols-2 lg:gap-11">
    ...
    >
        <figure class="mx-auto mb-6 inline-grid size-40 place-items-center rounded-full bg-heading-foreground/[3%]">
            <img
                src="{{ asset('vendor/titan_operator/images/titan_operator-create.png') }}"
                alt="Add New TitanOperator"
            >
        </figure>
        <p class="mx-auto mb-6 max-w-[370px] font-heading text-xl font-semibold leading-[1.3em] text-heading-foreground">
            @lang('Create and configure a titan_operator that interacts with your users.')
        </p>
        <x-button
            @click.prevent="setActiveTitanOperator('new_titan_operator', 1, true);"
            variant="ghost-shadow"
            href="#"
        >
            <x-tabler-plus class="size-4" />
            @lang('Add New TitanOperator')
        </x-button>
    </x-card>
    ...
    >
        <figure class="mx-auto mb-6 inline-grid size-40 place-items-center rounded-full bg-heading-foreground/[3%]">
            <img
                src="{{ asset('vendor/titan_operator/images/titan_operator-history.png') }}"
