(async function () {
	...
	const widgetMarkup = `
<div id="lqd-ext-titan-operator-wrap" data-ready="false" data-window-open="false">
    <style>
        #lqd-ext-titan-operator-wrap {
        ...
        #lqd-ext-titan-operator-wrap .lqd-ext-titan-operator-trigger {
            display: inline-grid;
            place-items: center;
        ...
        #lqd-ext-titan-operator-wrap .lqd-ext-titan-operator-trigger:before {
            content: '';
        ...
        #lqd-ext-titan-operator-wrap .lqd-ext-titan-operator-trigger-mobile {
            display: inline-grid;
			place-items: center;
			width: 24px;
			height: 24px;
        ...
        #lqd-ext-titan-operator-wrap .lqd-ext-titan-operator-trigger-mobile:before {
			content: none;
		}
        #lqd-ext-titan-operator-wrap #lqd-ext-titan-operator-trigger-img,
        #lqd-ext-titan-operator-wrap #lqd-ext-titan-operator-trigger-icon {
            grid-row: 1 / 1;
            grid-column: 1 / 1;
            transition: all 0.15s;
        }
        ...
        #lqd-ext-titan-operator-wrap .lqd-ext-titan-operator-trigger:active {
            transform: scale(0.9);
        }

        #lqd-ext-titan-operator-iframe-wrap {
        ...
        #lqd-ext-titan-operator-iframe {
            width: 100%;
            height: 100%;
        }

        #lqd-ext-titan-operator-trigger-bubble {
            padding: 12px 16px;
            border-radius: 12px;
            position: absolute;
         ...
         #lqd-ext-titan-operator-trigger-bubble:before {
            content: '';
            display: inline-block;
            width: 100%;
            height: 100%;
        ...
        #lqd-ext-titan-operator-trigger-bubble p {
            position: relative;
            z-index: 1;
            margin: 0;
        }

        .lqd-ext-titan-operator-not-loaded {
            margin: 0;
            padding: 1rem;
        }

        #lqd-ext-titan-operator-wrap[data-ready=true] .lqd-ext-titan-operator-trigger:not(.lqd-ext-titan-operator-trigger-mobile),
        #lqd-ext-titan-operator-wrap[data-ready=true] #lqd-ext-titan-operator-trigger-bubble {
            opacity: 1;
            visibility: visible;
            transform: translateY(0);
        }

        #lqd-ext-titan-operator-wrap[data-window-state=open] #lqd-ext-titan-operator-iframe-wrap {
        ...
        #lqd-ext-titan-operator-wrap[data-window-state=open] .lqd-ext-titan-operator-trigger:before {
            transform: translateY(0);
            opacity: 1;
        }
        #lqd-ext-titan-operator-wrap[data-window-state=open] #lqd-ext-titan-operator-trigger-icon {
            opacity: 1;
            transform: translateY(0);
        }
        #lqd-ext-titan-operator-wrap[data-window-state=open] #lqd-ext-titan-operator-trigger-img {
            opacity: 0;
            transform: translateY(-3px);
        }
        #lqd-ext-titan-operator-wrap[data-window-state=open] #lqd-ext-titan-operator-trigger-bubble {
            transform: scale(0.95);
            opacity: 0;
            visibility: hidden;
        }

        html[dir=rtl] #lqd-ext-titan-operator-wrap[data-pos-x=left] {
            align-items: end;
        }

        #lqd-ext-titan-operator-wrap[data-pos-x=right] {
            left: auto;
            right: var(--lqd-ext-chat-offset-x, 30px);
            align-items: end;
        }

        #lqd-ext-titan-operator-wrap[data-pos-x=right] #lqd-ext-titan-operator-iframe-wrap {
            transform-origin: bottom right;
        }

        #lqd-ext-titan-operator-wrap[data-pos-x=right] #lqd-ext-titan-operator-trigger-bubble {
            left: auto;
            right: 0;
        }

		html[dir=rtl] #lqd-ext-titan-operator-wrap[data-pos-x=right] {
            align-items: start;
        }

        #lqd-ext-titan-operator-wrap[data-pos-y=top] {
            bottom: auto;
            top: var(--lqd-ext-chat-offset-y, 30px);
            flex-direction: column-reverse;
        }

        #lqd-ext-titan-operator-wrap[data-pos-y=top] #lqd-ext-titan-operator-trigger-bubble {
            bottom: auto;
		...
		@media (max-width: 768px) {
			...
			#lqd-ext-titan-operator-wrap .lqd-ext-titan-operator-iframe-wrap,
			#lqd-ext-titan-operator-wrap .lqd-ext-titan-operator-trigger {
				pointer-events: auto;
			}
			#lqd-ext-titan-operator-iframe-wrap {
			...
			#lqd-ext-titan-operator-wrap[data-window-state="open"] .lqd-ext-titan-operator-trigger-mobile {
				opacity: 1;
				visibility: visible;
			}
		}
    </style>
    <div id="lqd-ext-titan-operator-iframe-wrap">
        ${iFrameUrl ? `
            <iframe
                src="${iFrameUrl}"
                title="${config.title}"
                ...
                allowfullscreen
                allowtransparency
                id="lqd-ext-titan-operator-iframe"
                name="lqd-ext-titan-operator-iframe"
                crossOrigin="anonymous"
                onload="
                    const wrapper = document.querySelector('#lqd-ext-titan-operator-wrap');
                    window.addEventListener('message', event => {
                        if ( event.origin !== '${operatorHostOrigin}' || event.data.type !== 'lqd-ext-titan-operator-response-styling' || !wrapper ) return;
                    ...
                    this.contentWindow.postMessage({
                        type: 'lqd-ext-titan-operator-request-styling',
                    }, '${operatorHostOrigin}');
                "
            ></iframe>` : `
        <p class="lqd-ext-titan-operator-not-loaded">Could not setup the titan_operator</p>
        `}
    </div>
    ${config.bubble_message && config.bubble_message !== '' ?
		`<div id="lqd-ext-titan-operator-trigger-bubble">
                <p>
    ...
    <button
        class="lqd-ext-titan-operator-trigger"
        type="button"
    >
        <img
            id="lqd-ext-titan-operator-trigger-img"
            src="${operatorHostOrigin}${config.avatar}"
            alt="${config.title}"
            width="60"
            height="60"
        />
        <span id="lqd-ext-titan-operator-trigger-icon">
            <svg
                width="16"
                height="10"
                viewBox="0 0 16 10"
    ...
    <button
        class="lqd-ext-titan-operator-trigger lqd-ext-titan-operator-trigger-mobile"
        type="button"
    >
	...
	const titan_operatorWrap = document.querySelector('#lqd-ext-titan-operator-wrap');
	const triggers = document.querySelectorAll('.lqd-ext-titan-operator-trigger');
	let open = false;
	...
	triggers.forEach(trigger => {
		trigger.addEventListener('click', ev => {
			ev.preventDefault();
			open = !open;
			titan_operatorWrap.setAttribute('data-window-state', open ? 'open' : 'close');
		});
	});
})();
