...
use App\Extensions\TitanOperator\System\ZeroCore\Models\TitanZero\AiChatProFolder;
        ...
        $query = UserOpenaiChat::where('user_id', Auth::id())
            ...
            ->where(function ($q) {
                $q->whereNull('chat_type')
                    ->orWhereIn('chat_type', ['chatpro', 'chatpro-temp', 'chatPro', 'job-chat', 'client-chat']);
            })
            ->select(['id', 'title', 'created_at', 'is_pinned', 'updated_at', 'reference_url', 'doc_name', 'website_url', 'folder_id'])
            ->orderByDesc('is_pinned')
            ->orderByDesc('updated_at');

        if ((int) Helper::setting('ai_image_chat_enabled', 0) === 1) {
