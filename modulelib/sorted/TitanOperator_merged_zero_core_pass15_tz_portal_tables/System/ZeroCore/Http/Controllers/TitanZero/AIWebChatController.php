...
class AIWebChatController extends Controller
{
    ...
    public function openAIGeneratorWorkbook()
    {
        ...
        $streamUrl = route('user.tzc.webchat.stream');
        ...
        return view('titanzero-chat::webchat.index', compact(
            'category',
            'apiSearch',
            'generators',
            'apiSearchId',
    ...
    public function openChatAreaContainer(Request $request): JsonResponse
    {
        ...
        $html = view('titanzero-chat::webchat.area_container', [
            'chat'        => $chat,
    ...
    public function startNewChat(Request $request)
    {
        ...
        if ($url) {
            ...
            // TitanZero domain restriction
            // Restricts URL crawling to the tenant's registered business website.
            // Falls through silently when team or website_domain is not yet configured.
            $this->enforceDomainRestriction($url);
    ...
    // ─────────────────────────────────────────────────────────────────
    // TitanZero Domain Restriction (Gap #4)
    // ─────────────────────────────────────────────────────────────────

    /**
     * Enforce that the submitted URL belongs to the tenant's registered
     * business website. Aborts 403 if the domain doesn't match.
     *
     * Silently passes through when:
     *  - The user has no team relation
     *  - The team has no website_domain configured
     *  - The TZC_WEBCHAT_DOMAIN_RESTRICT env var is not set to 'true'
     *
     * To activate fully:
     *  - Ensure team model has a 'website_domain' attribute (e.g. "example.com")
     *  - Set TZC_WEBCHAT_DOMAIN_RESTRICT=true in .env
     */
    protected function enforceDomainRestriction(string $url): void
    {
        // Feature gate — opt-in via env to avoid breaking existing installs
        if (!config('titan_operator.zero-chat.webchat.domain_restrict', false)) {
            return;
        }
        ...
        // Resolve team via relation or team_id attribute
        $team = $user->getAttribute('team')
             ?? $user->getAttribute('myCreatedTeam')
             ?? null;
        ...
        $allowedDomain = $team->website_domain ?? $team->domain ?? null;

        if (!$allowedDomain) {
            return;
        }

        // Normalise — strip protocol, www, trailing slash
        $allowed  = strtolower(preg_replace('/^www\./', '', parse_url(trim($allowedDomain), PHP_URL_HOST) ?? $allowedDomain));
        $submitted = strtolower(preg_replace('/^www\./', '', parse_url($url, PHP_URL_HOST) ?? ''));

        if ($submitted !== $allowed && !str_ends_with($submitted, '.' . $allowed)) {
            abort(403, __('Domain not permitted. Only your registered business website can be crawled. Allowed: ') . $allowedDomain);
        }
    }
