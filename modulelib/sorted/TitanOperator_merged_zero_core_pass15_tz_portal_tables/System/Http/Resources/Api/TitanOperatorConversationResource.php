...
namespace App\Extensions\TitanOperator\System\Http\Resources\Api;
...
class TitanOperatorConversationResource extends JsonResource
{
    public function toArray(Request $request): array|Arrayable|JsonSerializable
    {
        return [
            ...
            'routes'     => [
                'messages' => route('api.v2.titan_operator.conversion.messages', [
                    'titan_operator'             => $request->route('titan_operator')?->getAttribute('uuid'),
                    'sessionId'           => $request->route('sessionId'),
                    'operatorConversation' => $this->getAttribute('id'),
                ]),
            ],
        ];
    }
