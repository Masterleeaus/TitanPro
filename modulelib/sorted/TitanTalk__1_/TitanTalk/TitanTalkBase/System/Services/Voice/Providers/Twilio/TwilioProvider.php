...
namespace App\Extensions\MarketingBot\System\Services\Voice\Providers\Twilio;

use App\Extensions\MarketingBot\System\Services\Voice\Providers\PhoneProviderInterface;
...
class TwilioProvider implements PhoneProviderInterface
{
    ...
    {
        $token = (string) config('titantalk.twilio.auth_token');
        $sig = null;
    ...
    {
        $from = $fromNumber ?: (string) config('titantalk.twilio.from_number');
        $twimlUrl = $options['twimlUrl'] ?? (string) config('titantalk.twilio.outbound_twiml_url');

        // fallback: use built-in outbound twiml endpoint if set
        if (!$twimlUrl) {
            $twimlUrl = route('titantalk.webhooks.voice.outbound_twiml');
        }
