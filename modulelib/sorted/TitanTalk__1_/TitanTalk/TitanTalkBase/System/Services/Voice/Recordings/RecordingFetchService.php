...
class RecordingFetchService
{
    public function fetch(CallRecording $rec): CallRecording
    {
        ...
        $disk = config('titantalk.recordings.disk', 'local');
        $dir = trim((string)config('titantalk.recordings.path', 'titantalk/recordings'), '/');
        $ext = 'mp3';
        ...
        $authUser = config('titantalk.twilio.account_sid');
        $authPass = config('titantalk.twilio.auth_token');
