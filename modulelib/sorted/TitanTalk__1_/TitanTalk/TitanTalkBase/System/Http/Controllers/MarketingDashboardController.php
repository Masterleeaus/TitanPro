...
class MarketingDashboardController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        return view('marketing-bot::dashboard.index', [
            ...
            'campaignFocus'          => $campaignList->first(),
            'campaignMetrics'        => $this->campaignMetrics($campaignList),
            'messagingStats'         => $this->messagingStats(),
            'quickPresets'           => app(\App\Extensions\MarketingBot\System\Services\TitanTalk\PresetMessageService::class)->presets(),
            'quickConversations'     => $this->quickConversations(),
    ...
    public function messagingStats(): array
    {
        $sentToday = \App\Extensions\MarketingBot\System\Models\MarketingMessageHistory::query()
            ->whereHas('conversation', fn ($query) => $query->where('user_id', Auth::id()))
            ->whereDate('created_at', Carbon::today())
            ->where('role', 'assistant')
            ->count();

        $repliesToday = \App\Extensions\MarketingBot\System\Models\MarketingMessageHistory::query()
            ->whereHas('conversation', fn ($query) => $query->where('user_id', Auth::id()))
            ->whereDate('created_at', Carbon::today())
            ->where('role', 'user')
            ->count();

        $channels = MarketingConversation::query()
            ->where('user_id', Auth::id())
        ...
        return [
            ...
            'pending_followups' => max(($sentToday - $repliesToday), 0),
            'channels' => $channels,
        ];
    }

    public function quickConversations()
    {
        return MarketingConversation::query()
            ->where('user_id', Auth::id())
            ->whereIn('type', ['whatsapp', 'telegram'])
            ->orderByDesc('last_activity_at')
            ->limit(30)
            ->get(['id', 'conversation_name', 'type', 'session_id', 'last_activity_at']);
    }

    public function campaignMetrics($campaignList): array
    {
        $campaign = $campaignList->first();
        if (! $campaign) {
            return ['sent' => 0, 'replies' => 0, 'conversions' => 0];
        }

        $sent = (int) count($campaign->contacts ?? []);
        $replies = MarketingConversation::query()
            ->where('user_id', Auth::id())
            ->whereDate('updated_at', '>=', Carbon::now()->subDays(30))
            ->whereIn('type', [$campaign->type->value ?? $campaign->type])
            ->whereHas('histories', fn ($query) => $query->where('role', 'user'))
            ->count();

        return [
            'sent' => $sent,
            'replies' => $replies,
            'conversions' => min($replies, $sent),
        ];
    }
