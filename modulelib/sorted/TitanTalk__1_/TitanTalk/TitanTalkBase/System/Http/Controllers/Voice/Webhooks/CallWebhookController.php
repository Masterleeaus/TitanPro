...
namespace App\Extensions\MarketingBot\System\Http\Controllers\Voice\Webhooks;

use Illuminate\Http\Request;
use Illuminate\Routing\Controller;
use App\Extensions\MarketingBot\System\Services\Voice\Calls\CallIngestService;
use App\Extensions\MarketingBot\System\Services\Voice\Providers\Twilio\TwilioProvider;
use App\Extensions\MarketingBot\System\Services\Voice\Routing\InboundRoutingService;
use App\Extensions\MarketingBot\System\Services\Voice\Callbacks\CallOutcomeResolver;
...
class CallWebhookController extends Controller
{
    public function inbound(Request $request, CallIngestService $ingest, TwilioProvider $provider, InboundRoutingService $routing)
    {
        try {
            ...
            if (!$ok && config('titantalk.twilio.require_signature', false)) {
        ...
        $twiml = $routing->buildInboundResponse($companyId ?: 0, (string)($attrs['to_number'] ?? ''), preg_replace('#/inbound$#', '', rtrim(route('dashboard.user.marketing-bot.voice.webhooks.voice.inbound'), '/')));
    ...
    public function ivrSelect(Request $request, int $menuId, InboundRoutingService $routing)
    {
        ...
        $twiml = $routing->buildIvrSelectionResponse($companyId, $menuId, $digit, preg_replace('#/inbound$#', '', rtrim(route('dashboard.user.marketing-bot.voice.webhooks.voice.inbound'), '/')));
...
if (($rec->kind ?? 'call') === 'voicemail') {
    ...
    if (config('titantalk.voicemail.request_titan_zero_summary', false)) {
        try {
