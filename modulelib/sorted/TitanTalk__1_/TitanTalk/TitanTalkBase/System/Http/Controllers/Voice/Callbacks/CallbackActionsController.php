...
use App\Extensions\MarketingBot\System\Services\Voice\Callbacks\CallbackService;
...
class CallbackActionsController extends Controller
{
    ...
    public function done(Request $request, int $id, CallbackService $callbacks)
    {
        ...
        return redirect()->route('dashboard.user.marketing-bot.voice.callbacks.index', ['company_id' => $companyId])->with('status', 'Callback marked done.');
    }
    ...
    public function cancel(Request $request, int $id, CallbackService $callbacks)
    {
        ...
        return redirect()->route('dashboard.user.marketing-bot.voice.callbacks.index', ['company_id' => $companyId])->with('status', 'Callback cancelled.');
    }
}
