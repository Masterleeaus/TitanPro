...
class OpenAIGenerator extends Generator
{
    ...
    public function buildHistories(array $histories): array
    {
        ...
        $histories[] = [
            'role'    => 'system',
            'content' => $this->marketingCampaign->instruction ?: 'Always consult your training documents and knowledge base first. When a request needs a real business action, explain the next step clearly and do not pretend the action is already complete unless the system has actually processed it.',
        ];

        if (! empty($this->additionalSystemContext)) {
            $histories[] = [
                'role'    => 'system',
                'content' => $this->additionalSystemContext,
            ];
        }
