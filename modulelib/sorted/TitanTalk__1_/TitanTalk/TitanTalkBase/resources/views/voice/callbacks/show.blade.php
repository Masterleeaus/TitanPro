...
<div class="container">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a class="btn btn-outline-secondary" href="{{ route('dashboard.user.marketing-bot.voice.callbacks.index', ['company_id'=>request('company_id')]) }}">Back</a>
  </div>
  ...
  <div class="row g-3">
    <div class="col-lg-6">
      ...
      <div class="card card-body mt-3">
        <h6>Update</h6>
        <form method="post" action="{{ route('dashboard.user.marketing-bot.voice.callbacks.assign', ['company_id'=>request('company_id'),'id'=>$cb->id]) }}">
          @csrf
          <div class="mb-2">
        ...
        <form method="post" action="{{ route('dashboard.user.marketing-bot.voice.callbacks.due', ['company_id'=>request('company_id'),'id'=>$cb->id]) }}">
          @csrf
          <div class="row g-2">
            <div class="col-md-6">
        ...
        <div class="d-flex gap-2">
          <form method="post" action="{{ route('dashboard.user.marketing-bot.voice.callbacks.done', ['company_id'=>request('company_id'),'id'=>$cb->id]) }}">
            @csrf
            <button class="btn btn-success btn-sm" @disabled($cb->status!=='open')>Mark done</button>
          </form>
          <form method="post" action="{{ route('dashboard.user.marketing-bot.voice.callbacks.cancel', ['company_id'=>request('company_id'),'id'=>$cb->id]) }}">
            @csrf
    ...
    <div class="col-lg-6">
      <div class="card card-body">
        ...
        @if($cb->call)
          ...
          <div class="mt-2">
            <a class="btn btn-outline-primary btn-sm" href="{{ route('dashboard.user.marketing-bot.voice.calls.show', ['company_id'=>request('company_id'),'id'=>$cb->call->id]) }}">Open call</a>
          </div>
        @else
