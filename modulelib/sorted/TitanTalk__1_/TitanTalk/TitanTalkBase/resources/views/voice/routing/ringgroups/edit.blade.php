...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <a href="{{ route('dashboard.user.marketing-bot.voice.routing.ringgroups.index') }}" class="btn btn-light">Back</a>
</div>

@include('marketing-bot::voice.partials.flash')

<form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.routing.ringgroups.update', $group->id) }}" class="card card-body mb-3">
  @csrf
  @include('marketing-bot::voice.routing/ringgroups/form', ['group' => $group])
  <div class="mt-3">
...
<div class="card">
  ...
  <div class="card-body">
    <form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.routing.ringgroups.members.add', $group->id) }}" class="row g-2">
      @csrf
    ...
    <table class="table">
      ...
      @foreach($members as $m)
        <tr>
          ...
          <td class="text-end">
            <form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.routing.ringgroups.members.delete', [$group->id, $m->id]) }}">
              @csrf
