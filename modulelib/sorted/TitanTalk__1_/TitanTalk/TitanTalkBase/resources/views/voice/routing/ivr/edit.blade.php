...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h4>Edit IVR Menu</h4>
  <a href="{{ route('dashboard.user.marketing-bot.voice.routing.ivr.index') }}" class="btn btn-light">Back</a>
</div>

@include('marketing-bot::voice.partials.flash')

<form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.routing.ivr.update', $menu->id) }}" class="card card-body mb-3">
  @csrf
  @include('marketing-bot::voice.routing/ivr/form', ['menu' => $menu])
  <div class="mt-3">
...
<div class="card">
  ...
  <div class="card-body">
    <form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.routing.ivr.options.add', $menu->id) }}" class="row g-2">
      @csrf
    ...
    <table class="table">
      ...
      @foreach($options as $o)
        <tr>
          ...
          <td class="text-end">
            <form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.routing.ivr.options.delete', [$menu->id, $o->id]) }}">
              @csrf
