...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <a href="{{ route('dashboard.user.marketing-bot.voice.campaigns.index') }}" class="btn btn-light">Back</a>
</div>

@include('marketing-bot::voice.partials.flash')

<form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.campaigns.update', $campaign->id) }}" class="card card-body mb-3">
  @csrf
  @include('marketing-bot::voice.campaigns/form', ['campaign' => $campaign])
  <div class="mt-3">
...
<div class="card mb-3">
  <div class="card-header d-flex justify-content-between">
    ...
    <form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.campaigns.run', $campaign->id) }}">
      @csrf
      <button class="btn btn-sm btn-outline-primary">Run Batch (10)</button>
    </form>
  </div>
  <div class="card-body">
    <form method="POST" action="{{ route('dashboard.user.marketing-bot.voice.campaigns.contacts.add', $campaign->id) }}" class="row g-2 mb-3">
      @csrf
