@php
    ...
    $plan_type = 'regular';
    $reportsUrl = \Illuminate\Support\Facades\Route::has('dashboard.user.analytical-engine.index')
        ? route('dashboard.user.analytical-engine.index')
        : '#';

    $boardActionUrl = function (string $key, string $mode = 'modal') {
        if (\Illuminate\Support\Facades\Route::has('dashboard.user.board.action')) {
    ...
    $activeHub = 'service_console';
    $hubManifest = file_exists(base_path('dashboard_support/service_console/manifest.php'))
        ? include base_path('dashboard_support/service_console/manifest.php')
        : [];
    $hubKpis = file_exists(base_path('dashboard_support/service_console/kpis.php'))
        ? include base_path('dashboard_support/service_console/kpis.php')
        : [];
    $hubActions = file_exists(base_path('dashboard_support/service_console/actions.php'))
        ? include base_path('dashboard_support/service_console/actions.php')
        : [];
    $hubTabs = file_exists(base_path('dashboard_support/service_console/tabs.php'))
        ? include base_path('dashboard_support/service_console/tabs.php')
        : [];
    $hubAssistants = file_exists(base_path('dashboard_support/service_console/assistants.php'))
        ? include base_path('dashboard_support/service_console/assistants.php')
        : [];
    $hubLifecycles = file_exists(base_path('dashboard_support/service_console/lifecycles.php'))
        ? include base_path('dashboard_support/service_console/lifecycles.php')
        : [];

    $hubTitle = $hubManifest['title'] ?? 'Service Console';
    $hubPurpose = $hubManifest['purpose'] ?? 'Quotes, proposals, bookings, packages, client portal, and invoice creation before Money processing.';


$titlebar_links = [
    ['label' => 'Command Centre', 'link' => url('/dashboard?hub=home'), 'key' => 'home'],
    ['label' => 'Connect Core', 'link' => url('/dashboard?hub=connect'), 'key' => 'connect'],
    ['label' => 'Service Console', 'link' => url('/dashboard?hub=service_console'), 'key' => 'service_console'],
    ['label' => 'Work Hub', 'link' => url('/dashboard?hub=work'), 'key' => 'work'],
    ['label' => 'Team Portal', 'link' => url('/dashboard?hub=team'), 'key' => 'team'],
    ['label' => 'Supply Depot', 'link' => url('/dashboard?hub=supply'), 'key' => 'supply'],
    ['label' => 'Money Manager', 'link' => url('/dashboard?hub=money'), 'key' => 'money'],
    ['label' => 'Trust Vault', 'link' => url('/dashboard?hub=trust_vault'), 'key' => 'trust_vault'],
];
...
/* --- BOS Hub Standardization Pass 1 --- */
.bos-hub-hero{display:flex;flex-wrap:wrap;align-items:flex-start;justify-content:space-between;gap:14px;margin:0 0 16px}
.bos-hub-hero-copy h1,.bos-hub-hero-copy h2{margin:0;font-weight:700;letter-spacing:-.02em}
.bos-hub-hero-copy p{margin:6px 0 0;opacity:.72;max-width:72ch}
.bos-hub-badge{display:inline-flex;align-items:center;gap:8px;padding:7px 12px;border-radius:999px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);font-size:12px}
.bos-hub-shell{display:flex;flex-direction:column;gap:18px}
.bos-slim-rail{display:flex;align-items:center;gap:8px;overflow:auto;padding:4px 0 10px}
.bos-slim-rail a{display:inline-flex;align-items:center;gap:8px;white-space:nowrap;padding:8px 12px;border-radius:999px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.02);font-size:12px;text-decoration:none}
.bos-slim-rail a.is-active{background:rgba(99,102,241,.14);border-color:rgba(99,102,241,.32)}
    ...
    <script>
        ...
        window.addEventListener('titan:action', function(e) {
            ...
            const routes = {
                ...
                customer_intake: '{{ \Illuminate\Support\Facades\Route::has('dashboard.user.customers.index') ? route('dashboard.user.customers.index') : '#' }}',
...
<div class="hub-metadata-strip mb-3 hidden" data-hub="{{ $activeHub }}" data-kpi-count="{{ count($hubKpis ?? []) }}" data-action-count="{{ count($hubActions ?? []) }}" data-tab-count="{{ count($hubTabs ?? []) }}" data-assistant-count="{{ count($hubAssistants ?? []) }}" data-lifecycle-count="{{ count($hubLifecycles ?? []) }}"></div>
        ...
        >
            ...
            >
                <h3 class="mb-6 flex items-center gap-3">
                    ...
                    @lang('What needs doing today?')
                </h3>
                <p class="mb-3 text-sm text-heading-foreground/70">{{ __('Type a command or launch a guided workflow.') }}</p>
                <x-header-search
                    class="mb-2 w-full"
                    ...
                    size="lg"
                    placeholder="{{ __('Ask Work AI to create, assign, schedule, or review jobs') }}"
                    :show-arrow=false
                    :show-icon=false
                    :show-kbd=false
                    :outline-glow=true
...
@php
    $tenantUserId = \Illuminate\Support\Facades\Auth::id();
    $tenantTeamId = optional(\Illuminate\Support\Facades\Auth::user())->team_id;
    $companyId = optional(\Illuminate\Support\Facades\Auth::user())->company_id ?? 1;
    ...
    try {
        ...
        if ($hasTable('tz_invoices')) {
            $q = \Illuminate\Support\Facades\DB::table('tz_invoices');
            ...
            $q2 = \Illuminate\Support\Facades\DB::table('tz_invoices');
        ...
        if ($hasTable('tz_jobs')) {
            $q = \Illuminate\Support\Facades\DB::table('tz_jobs');
        ...
        if ($hasTable('tz_employee_timesheets') && $hasCol('tz_employee_timesheets','total_minutes')) {
            $q = \Illuminate\Support\Facades\DB::table('tz_employee_timesheets');
        ...
        if ($hasTable('tz_employee_profiles') && $hasCol('tz_employee_profiles','is_active')) {
            $q = \Illuminate\Support\Facades\DB::table('tz_employee_profiles')->where('is_active', 1);
        ...
        if ($hasTable('tz_subscriptions') && $hasCol('tz_subscriptions','status')) {
            $qAll = \Illuminate\Support\Facades\DB::table('tz_subscriptions');
            ...
            $qCan = \Illuminate\Support\Facades\DB::table('tz_subscriptions')->whereIn('status', ['cancelled','canceled','ended','inactive']);
...
<x-card class="w-full mb-6" size="md" id="command">
    <x-slot:head class="border-0 pb-0 pt-5">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <h4 class="m-0 text-[17px]">{{ __('Work Snapshot') }}</h4>
                <p class="mt-1 text-xs opacity-60">{{ __('Jobs, staffing, delays, and the next best operational move.') }}</p>
            </div>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <x-button variant="primary" href="{{ $boardActionUrl('wizard.command.next', 'modal') }}">
                    ...
                    <span class="font-bold">{{ __('Ask Work AI') }}</span>
                </x-button>
            </div>
        </div>
        ...
        @php
            $alerts = [];
            if (($tz['jobs_overdue'] ?? 0) > 0) $alerts[] = ['label' => __('Overdue jobs: ') . ($tz['jobs_overdue'] ?? 0), 'href' => url('/account/jobs?filter=overdue')];
            if (($tz['jobs_unassigned'] ?? 0) > 0) $alerts[] = ['label' => __('Unassigned jobs: ') . ($tz['jobs_unassigned'] ?? 0), 'href' => url('/account/jobs?date=today&filter=unassigned')];
            if (($tz['jobs_today'] ?? 0) === 0) $alerts[] = ['label' => __('No jobs today — create or dispatch work'), 'href' => $boardActionUrl('wizard.pipeline.fill', 'modal')];
        @endphp
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
        <a class="rounded-xl bg-background/40 p-4 transition hover:bg-background/60" href="{{ url('/account/jobs?date=today') }}">
            <div class="text-xs font-medium opacity-60">{{ __('Jobs Today') }}</div>
        ...
        <a class="rounded-xl bg-background/40 p-4 transition hover:bg-background/60" href="{{ url('/account/jobs?filter=overdue') }}">
            <div class="text-xs font-medium opacity-60">{{ __('Overdue') }}</div>
        ...
        <a class="rounded-xl bg-background/40 p-4 transition hover:bg-background/60" href="{{ url('/account/jobs?date=today&status=completed') }}">
            <div class="text-xs font-medium opacity-60">{{ __('Completed') }}</div>
        ...
        <a class="rounded-xl bg-background/40 p-4 transition hover:bg-background/60" href="{{ $boardActionUrl('wizard.work.next', 'modal') }}">
            <div class="text-xs font-medium opacity-60">{{ __('Next Best Move') }}</div>
            <div class="mt-1 text-2xl font-bold">⚡</div>
            <div class="mt-1 text-xs opacity-70">{{ __('Auto-plan dispatch') }}</div>
        </a>
    </div>
</x-card>
...
<div class="mb-4 flex items-center justify-between gap-4">
    <div>
        <h4 class="m-0 text-[17px]">{{ __('Command Tools') }}</h4>
        <p class="mt-1 text-xs opacity-60">{{ __('Each card is a tool. Click to drill down, or launch a multi-step AI wizard.') }}</p>
    </div>
    <div class="flex flex-wrap gap-2">
        <x-button variant="outline" href="{{ $boardActionUrl('wizard.invoices.followup', 'modal') }}">
            <x-tabler-send class="size-4" />
            {{ __('Follow-up Invoices') }}
        </x-button>
...
<div class="w-full mb-6">
    <div class="tz-kpi-grid">
        <div class="tz-kpi-card">
            <div class="tz-kpi-title">Revenue (30d)</div>
            <div class="tz-kpi-value">{{ $kpi['revenue_30d_fmt'] }}</div>
            <div class="tz-kpi-sub">Paid invoices</div>
...
@endphp
    ...
    @endphp
        ...
        <div class="tz-kpi-card" style="color:#7C9CFF">
            <div class="tz-kpi-title">Efficiency</div>
            <div class="tz-kpi-value">{{ $kpi['efficiency_pct'] ?? 0 }}%</div>
            <div class="tz-kpi-sub">Target 95%+</div>
            @php
        ...
        <div class="tz-kpi-card" style="color:#8BE3B3">
            <div class="tz-kpi-title">Utilization</div>
            <div class="tz-kpi-value">{{ $kpi['utilization_pct'] ?? 0 }}%</div>
            <div class="tz-kpi-sub">Target 80%+</div>
            @php
        ...
        <div class="tz-kpi-card">
            <div class="tz-kpi-title">Revenue / Labor Hour</div>
...
@endphp
    ...
    @endphp
        ...
        <div class="tz-kpi-card" style="color:#F5C26B">
            <div class="tz-kpi-title">Churn (30d)</div>
            <div class="tz-kpi-value">{{ $kpi['churn_pct'] ?? 0 }}%</div>
            <div class="tz-kpi-sub">Recurring customers lost</div>
            @php
...
try {
    ...
    if (\Illuminate\Support\Facades\Schema::hasTable('tz_job_schedule') && \Illuminate\Support\Facades\Schema::hasTable('tz_jobs')) {
        $schedBase = \Illuminate\Support\Facades\DB::table('tz_job_schedule')
            ->join('tz_jobs', 'tz_jobs.id', '=', 'tz_job_schedule.job_id');

        if (\Illuminate\Support\Facades\Schema::hasColumn('tz_jobs', 'user_id')) {
            $schedBase->where('tz_jobs.user_id', auth()->id());
        }
        if (\Illuminate\Support\Facades\Schema::hasColumn('tz_jobs', 'team_id') && auth()->user()?->team_id) {
        ...
        if (\Illuminate\Support\Facades\Schema::hasColumn('tz_jobs', 'assigned_user_id')) {
            $tz['jobs_unassigned'] = (int) (clone $sched)->whereNull('tz_jobs.assigned_user_id')->count();
        } elseif (\Illuminate\Support\Facades\Schema::hasColumn('tz_jobs', 'assigned_to')) {
        ...
        if (\Illuminate\Support\Facades\Schema::hasTable('tz_job_states') && \Illuminate\Support\Facades\Schema::hasColumn('tz_job_states', 'job_id')) {
            $completed = \Illuminate\Support\Facades\DB::table('tz_job_states')
                ->whereIn('job_id', (clone $sched)->select('tz_jobs.id'));

            $completedJobIds = \Illuminate\Support\Facades\DB::table('tz_job_states');
            if (\Illuminate\Support\Facades\Schema::hasColumn('tz_job_states', 'state')) {
                $completed->where('state', 'completed');
                $completedJobIds->where('state', 'completed');
            } elseif (\Illuminate\Support\Facades\Schema::hasColumn('tz_job_states', 'state_type')) {
        ...
        } elseif (\Illuminate\Support\Facades\Schema::hasColumn('tz_jobs', 'status')) {
    ...
    if (\Illuminate\Support\Facades\Schema::hasTable('tz_invoices')) {
        $inv = \Illuminate\Support\Facades\DB::table('tz_invoices');
        if (\Illuminate\Support\Facades\Schema::hasColumn('tz_invoices', 'user_id')) $inv->where('user_id', auth()->id());
        if (\Illuminate\Support\Facades\Schema::hasColumn('tz_invoices', 'team_id') && auth()->user()?->team_id) $inv->where('team_id', auth()->user()->team_id);

        $dueCol = \Illuminate\Support\Facades\Schema::hasColumn('tz_invoices', 'due_date') ? 'due_date' : (\Illuminate\Support\Facades\Schema::hasColumn('tz_invoices', 'due_at') ? 'due_at' : null);
        $statusCol = \Illuminate\Support\Facades\Schema::hasColumn('tz_invoices', 'status') ? 'status' : null;
    ...
    // Customers
    if (\Illuminate\Support\Facades\Schema::hasTable('tz_customers')) {
        $cust = \Illuminate\Support\Facades\DB::table('tz_customers');
        if (\Illuminate\Support\Facades\Schema::hasColumn('tz_customers', 'user_id')) $cust->where('user_id', auth()->id());
        if (\Illuminate\Support\Facades\Schema::hasColumn('tz_customers', 'team_id') && auth()->user()?->team_id) $cust->where('team_id', auth()->user()->team_id);
...
<div class="flex items-start justify-between gap-4">
    ...
    <div class="flex gap-2">
        ...
        <x-button type="button" @click="window.dispatchEvent(new CustomEvent('titan:action',{detail:{intent:'dispatch_plan'}}))">{{ __('Dispatch') }}</x-button>
    </div>
</div>
...
>
    @php
        $calendarUrl = '#';
        try {
            if (\Illuminate\Support\Facades\Route::has('dashboard.user.bookings.index')) {
                $calendarUrl = route('dashboard.user.bookings.index');
            } elseif (\Illuminate\Support\Facades\Route::has('dashboard.user.schedule.index')) {
    ...
    <div class="tz-premcal">
        <div class="tz-premcal-top">
            ...
            <a class="tz-premcal-link" href="{{ $calendarUrl }}">{{ __('Open Calendar') }} ›</a>
        </div>
            ...
            <div class="grid w-full grid-cols-1 justify-between gap-4 sm:grid-cols-2">
                @if (\App\Helpers\Classes\MarketplaceHelper::isRegistered('chatbot') || $app_is_demo)
                    ...
                    >
                        ...
                        <a
                            ...
                            href="{{ (\App\Helpers\Classes\MarketplaceHelper::isRegistered('chatbot') && \Illuminate\Support\Facades\Route::has('dashboard.chatbot.index')) ? route('dashboard.chatbot.index') : '#' }}"
                        ></a>
                    </x-card>
                ...
                @php
                    ...
                    if (class_exists(\Illuminate\Support\Facades\Route::class)) {
                        try {
                            if (\Illuminate\Support\Facades\Route::has('dashboard.user.jobs.map.index')) {
                                $tz_map_href = route('dashboard.user.jobs.map.index');
                            } elseif (\Illuminate\Support\Facades\Route::has('dashboard.user.map.index')) {
...
@php
    ...
    $fetch = function (?string $table, int $limit = 10, ?callable $mutator = null) use ($tzUserId, $tzTeamId) {
        ...
        try {
            ...
            if (\Illuminate\Support\Facades\Schema::hasColumn($table, 'user_id')) $q->where('user_id', $tzUserId);
            if ($tzTeamId && \Illuminate\Support\Facades\Schema::hasColumn($table, 'team_id')) $q->where('team_id', $tzTeamId);
            if ($mutator) $mutator($q, $table);
            if (\Illuminate\Support\Facades\Schema::hasColumn($table, 'scheduled_for')) {
                $q->orderBy('scheduled_for', 'asc');
            } elseif (\Illuminate\Support\Facades\Schema::hasColumn($table, 'scheduled_at')) {
                $q->orderBy('scheduled_at', 'asc');
            } elseif (\Illuminate\Support\Facades\Schema::hasColumn($table, 'updated_at')) {
    ...
    $rowsOverdue = $fetch($tJobs, 12, function ($q, $table) {
        ...
        if (\Illuminate\Support\Facades\Schema::hasColumn($table, 'due_at')) {
            $q->where('due_at', '<', $now);
        } elseif (\Illuminate\Support\Facades\Schema::hasColumn($table, 'scheduled_at')) {
    ...
    <x-card class="w-full" size="md" x-data="{ tab: localStorage.getItem('titan-work-tab') || 'jobs' }" x-init="$watch('tab', value => localStorage.setItem('titan-work-tab', value))">
        ...
        <div class="mt-4 rounded-2xl border border-white/5 bg-background/20">
            <div class="flex items-center justify-between gap-3 border-b border-white/5 px-4 py-3 text-xs">
                ...
                <template x-for="(label, key) in {jobs:'{{ __('Jobs') }}',tasks:'{{ __('Tasks') }}',schedule:'{{ __('Schedule') }}',overdue:'{{ __('Overdue') }}',history:'{{ __('History') }}',agents:'{{ __('Agents') }}',activity:'{{ __('Activity') }}'}" :key="key">
                    <a x-show="tab === key"
                    ...
                    <table class="w-full text-sm">
                        <thead class="sticky top-0 z-10 bg-background/95 text-xs uppercase opacity-60 backdrop-blur">
                            <tr class="border-b border-white/5">
                                <th class="px-4 py-3 text-start">{{ __('Job') }}</th>
                                <th class="px-4 py-3 text-start">{{ __('Customer/Site') }}</th>
                                <th class="px-4 py-3 text-start">{{ __('Completed') }}</th>
                                <th class="px-4 py-3 text-start">{{ __('Status') }}</th>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            @forelse ($rowsAgents as $r)
                                ...
                                @endphp
                                <tr class="cursor-pointer border-b border-white/5 hover:bg-background/30" @if($href !== '#') onclick="window.location='{{ $href }}'" @endif>
                                    <td class="px-4 py-3 font-semibold">{{ \Illuminate\Support\Str::headline((string) $agent) }}</td>
                                    <td class="px-4 py-3">{{ $job }}</td>
                                    <td class="px-4 py-3">{{ $status }}</td>
                                    <td class="px-4 py-3 opacity-70">{{ $updated }}</td>
