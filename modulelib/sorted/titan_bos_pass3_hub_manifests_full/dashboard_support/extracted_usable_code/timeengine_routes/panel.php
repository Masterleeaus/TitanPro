...
Route::middleware(['auth', 'updateUserActivity'])
    ...
    ->group(static function () {
        ...
        Route::delete('/api/search/delete-search-key/{key}', [SearchController::class, 'deleteSearchkey']);
                ...
                // Titan Calendar System
                Route::prefix('calendar')
                    ->name('calendar.')
                    ->group(function () {
                        Route::get('/', [\App\Http\Controllers\Calendar\TitanCalendarController::class, 'index'])->name('index');
                        Route::get('/settings', [\App\Http\Controllers\Calendar\TitanCalendarSettingsController::class, 'index'])->name('settings');
                        Route::get('/feed', \App\Http\Controllers\Calendar\TitanCalendarFeedController::class)->name('feed');
                        Route::post('/events', [\App\Http\Controllers\Calendar\TitanCalendarEventController::class, 'store'])->name('store');
                        Route::post('/events/move', [\App\Http\Controllers\Calendar\CalendarInteractionController::class, 'move'])->name('events.move');
                        Route::get('/scheduled-tasks', [\App\Http\Controllers\Calendar\TitanScheduledTaskController::class, 'index'])->name('scheduled-tasks');
                        Route::post('/scheduled-tasks', [\App\Http\Controllers\Calendar\TitanScheduledTaskController::class, 'store'])->name('scheduled-tasks.store');
                        Route::post('/scheduled-tasks/{task}/run', [\App\Http\Controllers\Calendar\TitanScheduledTaskController::class, 'run'])->name('scheduled-tasks.run');
                    });
    });
