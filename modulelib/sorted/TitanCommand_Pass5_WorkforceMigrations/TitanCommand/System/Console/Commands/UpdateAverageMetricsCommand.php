...
{
    protected $signature = 'titancommand:update-average-metrics {agent_id? : Limit the update to a single agent ID}';
    ...
    public function handle(): int
    {
        ...
        Log::info('titancommand:update-average-metrics started', [
        ...
        TitanCommand::query()
            ...
            ->chunkById(50, function ($agents) use (&$updated, &$attempted) {
                foreach ($agents as $agent) {
                    ...
                    if ($averages === null) {
                        Log::info('titancommand:update-average-metrics skipped agent', [
                            'agent_id' => $agent->id,
                            'reason'   => 'no_metrics',
                        ]);
                    ...
                    $updated++;
                    Log::info('titancommand:update-average-metrics updated agent', [
                        'agent_id'           => $agent->id,
        ...
        Log::info('titancommand:update-average-metrics finished', [
            'agent_option'     => $agentId,
    ...
    private function calculateAverages(TitanCommand $agent): ?array
    {
        $metrics = TitanCommandPostDailyMetric::query()
    ...
    private function extractImpressions(TitanCommandPostDailyMetric $metric): ?float
    {
    ...
    private function extractEngagement(TitanCommandPostDailyMetric $metric): ?float
    {
