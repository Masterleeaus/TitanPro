...
class IcsExportController extends Controller{
  ...
  public function workOrder($id){
    ...
    $ics=$this->H()."BEGIN:VEVENT\n".$this->L('UID','wo-'.$wo->id.'@fieldservicesuite').$this->L('DTSTAMP',now()->format('Ymd\THis\Z')).$this->L('DTSTART',date('Ymd\THis',strtotime($dt))).$this->L('SUMMARY','Job #'.$wo->id)."END:VEVENT\n".$this->F();
