...
namespace App\Extensions\TitanCommand\System\JobManager\Http\Controllers; use Illuminate\Routing\Controller; use Illuminate\Http\Request; use Illuminate\Support\Facades\DB; use Illuminate\Support\Facades\Storage;
class ClientSignController extends Controller{protected function clientId(){return auth('client')->user()->client_id ?? auth('client')->id();}
  public function form($id){ $wo=DB::table('work_orders')->where('id',$id)->where('client_id',$this->clientId())->first(); abort_if(!$wo,404); return view('jobmanager::client/sign', compact('wo')); }
  public function submit(Request $r,$id){
    ...
    return redirect()->route('client.jobmanager.show',$id)->with('status','Thanks, your sign-off has been recorded.');
  }
}
