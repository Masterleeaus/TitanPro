...
namespace App\Extensions\TitanCommand\System\Services\Chat;
...
class TitanCommandChatService
{
    ...
    public static function generatePost(?string $argsString): string
    {
        ...
        $rateLimiter = new RateLimiter('titancommand-chat', 5);
        ...
        try {
            ...
            $response_md = '::: titancommand-chat-post-card {data-platform-id=' . $post->platform->id . ' data-post-id=' . $post->id . '}<br>:::';

            // return json_encode($response, JSON_THROW_ON_ERROR);
            return $response_md;
        } catch (JsonException|Throwable $e) {
            Log::error('TitanCommandChatService::generatePost error: ' . $e->getMessage());
