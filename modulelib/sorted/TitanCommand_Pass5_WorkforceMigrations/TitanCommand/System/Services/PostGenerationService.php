...
class PostGenerationService
{
     ...
     */
    public function generatePost(TitanCommand|Builder|Model $agent, array $options = []): array
    {
        try {
     ...
     */
    public function generateBulkPosts(TitanCommand $agent, int $count = 5): array
    {
        $posts = [];
    ...
    public function generateOnePost(TitanCommand $agent, ?string $argsString = null): ?TitanCommandPost
    {
        $payload = [];
     ...
     */
    protected function buildPrompt(TitanCommand $agent, array $options = [], array $generationSettings = []): string
    {
...
PROMPT;
     ...
     * Get system prompt
     */
    protected function getSystemPrompt(TitanCommand $agent, array $generationSettings = []): string
    {
        $tone = $generationSettings['tone'] ?? $agent->tone;
        $language = $generationSettings['language'] ?? $agent->language;
...
SYSTEM;
     ...
     */
    protected function parsePostContent(string $content, TitanCommand $agent, array $generationSettings = []): array
    {
        try {
            // Clean the response
    ...
    protected function buildIndustryGuidance(TitanCommand $agent): ?string
    {
        $contextSegments = [];
    ...
    protected function resolvePlatformId(TitanCommand $agent, ?string $requestedPlatform): ?int
    {
        $platformIds = $agent->platform_ids ?? [];

        $query = CommandPlatform::query()
    ...
    public function getNextPostTypeForAgent(TitanCommand $agent, ?array $overridePostTypes = null): ?string
    {
     ...
     */
    public function generateWithImagePrompt(TitanCommand $agent, array $options = []): array
    {
