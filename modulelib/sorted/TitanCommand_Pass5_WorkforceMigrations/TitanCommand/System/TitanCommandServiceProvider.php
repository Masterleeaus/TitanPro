...
class TitanCommandServiceProvider extends ServiceProvider implements ExtensionRegisterKeyProviderInterface, UninstallExtensionServiceProviderInterface
{
    ...
    protected function registerCommands(): static
    {
        if ($this->app->runningInConsole()) {
            ...
            // exec('php artisan titancommand:generate-posts 1');
            // Schedule tasks
            $this->app->booted(function () {
                $schedule = $this->app->make(Schedule::class);
                $schedule->command('titancommand:generate-posts')->everyTwoMinutes();
                $schedule->command('titancommand:check-pending-images')->everyFiveMinutes();
                $schedule->command('titancommand:post-metrics-analyzer')->cron('10 1 */3 * *');
                $schedule->command('titancommand:post-performance-advisor')->cron('25 1 */3 * *');
                $schedule->command('titancommand:trend-finder')->cron('40 1 */3 * *');
                $schedule->command('titancommand:weekly-social-trends')->cron('0 2 */3 * *');
    ...
    {
        $this->publishes([
            __DIR__ . '/../resources/assets/images' => public_path('vendor/titancommand/images'),
            __DIR__ . '/../resources/assets/videos' => public_path('vendor/titancommand/videos'),
        ], 'extension');
    ...
    protected function registerRoutes(): void
    {
        ...
        $this->router()
            ->group([
                ...
                Route::post('titancommand/fal-webhook', [TitanCommandController::class, 'falWebhook'])->name('dashboard.user.social-media.agent.fal-webhook');
            });
    }
