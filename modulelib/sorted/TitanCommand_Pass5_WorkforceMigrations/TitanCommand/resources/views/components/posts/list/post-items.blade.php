...
@forelse($sub-items as $sub-item)
    ...
    <tr
        class="command-agent-sub-item-item"
        ...
        x-data='Command AgentPostItem({ postId: {{ $sub-item->id }}, status: "{{ $sub-item->status }}", media_urls: @json($sub-item->media_urls), platform_id: {{ $sub-item->platform_id }}, container: @json($sub-item->container), post_type: "{{ $sub-item->post_type }}", scheduled_at: "{{ $sub-item->scheduled_at }}", published_at: "{{ $sub-item->published_at }}", content: @json($sub-item->content), editSidedrawerId: "#command-agent-sidedrawer" })'
    >
        <td>
        ...
        <td class="min-w-40">
            <p
                @class([
                    ...
                    'text-blue-600' => $sub-item->status === 'draft',
                    'text-yellow-700 dark:text-yellow-600' => $sub-item->status === 'scheduled',
                    'text-green-600' => $sub-item->status === 'published',
                    'text-red-600' => $sub-item->status === 'failed',
                ])
                :class="{
        ...
        <td class="min-w-36">
            @php
                $date_to_show = $sub-item->status === 'published' && $sub-item->published_at ? $sub-item->published_at : $sub-item->scheduled_at ?? '';
        ...
        <td>
            <div class="flex items-center gap-1.5 text-xs">
                ...
                <img
                    ...
                    alt="{{ $sub-item->content }}"
                >
                <span x-text="container.credentials.username">
                    {{ $platform_username }}
                </span>
            </div>
        </td>
...
@empty
    <tr>
        <td colspan="5">
            <h4 class="command-agent-sub-item-item group m-0 flex w-full flex-shrink-0 flex-grow-0 basis-auto text-lg">
