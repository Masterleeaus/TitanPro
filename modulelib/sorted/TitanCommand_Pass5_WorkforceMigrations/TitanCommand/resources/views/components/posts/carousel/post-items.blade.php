...
@forelse($sub-items as $sub-item)
    @php
        $platform_enum = $sub-item->getPlatformEnum();
        ...
        $platform_username = $sub-item->container?->username();

        $social_post = $sub-item->socialPost;
        ...
        $published_link = $social_post?->link ?: ($social_post?->id ? route('dashboard.user.social-media.sub-item.show', $social_post->id) : null);
    @endphp

    <div
        class="command-agent-sub-item-item group flex w-full flex-shrink-0 flex-grow-0 basis-auto flex-col px-5 py-2 sm:w-1/2 lg:w-1/4"
        data-sub-item-id="{{ $sub-item->id }}"
        :class="{ 'animate-pulse pointer-events-none': currentTasks.has('approvePost') || currentTasks.has('rejectPost') }"
        x-data='Command AgentPostItem({ postId: {{ $sub-item->id }}, status: "{{ $sub-item->status }}", media_urls: @json($sub-item->media_urls), platform_id: {{ $sub-item->platform_id }}, container: @json($sub-item->container), post_type: "{{ $sub-item->post_type }}", scheduled_at: "{{ $sub-item->scheduled_at }}", content: @json($sub-item->content), editSidedrawerId: "#command-agent-sidedrawer" })'
    >
        <div class="flex grow flex-col rounded-[10px] border p-3.5 transition">
            <div class="mb-2.5 flex items-center justify-between gap-1">
                <div class="flex items-center gap-1 overflow-hidden text-2xs text-heading-foreground">
                    <img
                        ...
                        height="20"
                        src="{{ $platform_image }}"
                        :src="`/vendor/social-media/icons/${container.container}.svg`"
                        alt="{{ $sub-item->content }}"
                    >
                    <img
                        ...
                        :src="`/vendor/social-media/icons/${container.container}-mono-light.svg`"
                        alt="{{ $sub-item->content }}"
                    >
                    <span
                        class="w-full truncate"
                        x-text="container.credentials.username"
                ...
                <span class="whitespace-nowrap text-3xs opacity-55">
                    <span
                        ...
                        x-text="post_type"
                    >
                        {{ $sub-item->post_type ? Str::headline($sub-item->post_type) : __('Sub-item') }}
                    </span>
                    <span class="mx-1 align-text-top text-xl leading-[0]">.</span>
                    <span x-text="statusState.label">
                        {{ Str::headline($sub-item->status) }}
                    </span>
                </span>
            </div>

            {{-- placeholder --}}
            @if (is_array($sub-item->media_urls) && count($sub-item->media_urls) > 0)
                <div
            ...
            <p
                @class([
                    ...
                    'bg-blue-500/15 text-blue-700 dark:text-blue-300' =>
                        $sub-item->status === 'draft',
                    'bg-yellow-500/15 text-yellow-700 dark:text-yellow-300' =>
                        $sub-item->status === 'scheduled',
                    'bg-green-500/15 text-green-700 dark:text-green-300' =>
                        $sub-item->status === 'published',
                    'bg-red-500/15 text-red-700 dark:text-red-300' =>
                        $sub-item->status === 'failed',
                ])
            ...
            @if ($sub-item->status === 'published' && $is_trending)
            ...
            <div class="mb-5 flex items-center gap-1">
                @if ($sub-item->status === 'draft')
                    <x-button
                        ...
                        size="none"
                        title="{{ __('Approve') }}"
                        @click.prevent="editingPostInitialStatus = 'draft'; approvePost({{ $sub-item->id }})"
                ...
                @if ($sub-item->status !== 'published')
                    <x-button
                        ...
                        size="none"
                        @click.prevent="regeneratePostContent({{ $sub-item->id }})"
                        title="{{ __('Regenerate Sub-item Content') }}"
                    >
                        <x-tabler-loader-2
                ...
                <x-dropdown.dropdown offsetY="10px">
                    ...
                    >
                        @if ($sub-item->status === 'published' && $published_link)
                            <a
                        ...
                        @if ($sub-item->status !== 'published')
                            <a
                                class="block rounded border-b px-2 py-1.5 text-2xs font-medium text-heading-foreground transition-colors last:border-b-0 hover:bg-foreground/5"
                                href="#"
                                @click.prevent="$dispatch('command-agent:duplicate-sub-item', { id: {{ $sub-item->id }} })"
                            >
                                @lang('Duplicate')
                            </a>
                        @endif
                        @if ($sub-item->status !== 'published')
                            <a
                                ...
                                href="#"
                                @click.prevent="rejectPost({{ $sub-item->id }}); toggle('collapse');"
                            >
                                @lang('Delete')
                            </a>
                        @endif
            ...
            <div class="mt-auto pb-4">
                ...
                @else
                    <x-button
                        ...
                        variant="outline"
                        hover-variant="primary"
                        @click.prevent="openEditSidedrawer({ query: 'id={{ $sub-item->id }}', taskKey: 'fetchingPost-{{ $sub-item->id }}' })"
                        ::class="{ 'opacity-50 pointer-events-none': currentTasks.has('fetchingPost-{{ $sub-item->id }}') }"
                    >
                        <x-tabler-loader-2
                            class="size-4 animate-spin"
                            x-show="currentTasks.has('fetchingPost-{{ $sub-item->id }}')"
                            x-cloak
                        />
                        @lang('Edit')
                    </x-button>
...
@empty
    <h4 class="command-agent-sub-item-item group flex w-full flex-shrink-0 flex-grow-0 basis-auto px-5 py-2 text-lg">
        @lang('No sub-items were found.')
    </h4>
@endforelse

@if ($sub-items->hasMorePages())
    <span
        class="hidden"
        data-next-page-url="{{ $sub-items->nextPageUrl() }}"
        aria-hidden="true"
    ></span>
@endif
    ...
    <script>
        (() => {
            document.addEventListener('alpine:init', () => {
                Alpine.data('Command AgentPostItem', ({
                    postId,
                    status,
                    media_urls,
                    platform_id,
