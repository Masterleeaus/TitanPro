@php
	...
	if ($app_is_demo) {
		...
		$chartData[] = [
			...
			'chart_series' => [
				'name' => 'all',
				'data' => array_map(fn() => rand(5000, 25000), range(0, 11)),
			],
			'today_impressions' => rand(800, 3000),
		];

		// Generate data for each container
		foreach ($demo_platforms as $container) {
			$chartData[] = [
				'label' => __(ucfirst($container)),
				'id' => $container,
				'chart_series' => [
					'name' => $container,
					'data' => array_map(fn() => rand(1000, 8000), range(0, 11)),
				],
				'today_impressions' => rand(200, 1000),
			];
		}
	}
	...
	if ($app_is_demo) {
		$facebook = collect($chartData)->firstWhere('id', 'facebook');

		$initialFilter = $facebook['chart_series']['name'] ?? 'facebook';
		$initialLabel = $facebook['label'] ?? __('Facebook');
		$initialToday = $facebook['today_impressions'] ?? 0;
	} else {
		$initialFilter = $chartData[0]['chart_series']['name'] ?? 'all';
		$initialLabel = $chartData[0]['label'] ?? __('All');
		$initialToday = $chartData[0]['today_impressions'] ?? 0;
	}
@endphp

<div x-data='Command AgentImpressionsChart'>
	<x-card class:body="px-6">
		...
		<div
			class="min-h-56"
			id="command-agent-impressions-chart"
		></div>
	</x-card>
</div>
...
@push('script')
	<script>
		document.addEventListener('alpine:init', () => {
			Alpine.data('Command AgentImpressionsChart', () => ({
				chartData: @json($chartData),
				activeFilter: "{{ $initialFilter }}",
				chart: null,
				chartEl: document.querySelector('#command-agent-impressions-chart'),
				counterEl: document.querySelector('#impressions-counter .lqd-number-counter-value'),
				...
				setActiveFilter(filter) {
					...
					if (counterElData && counterElData.updateValue) {
						counterElData.updateValue({
							value: this.chartData.find(data => data.chart_series.name === this.activeFilter)?.today_impressions ?? 0
						});
					}
				}
			}));
