...
class RewindFixService
{
    public function proposeFix(RewindCase $case, array $proposal, array $proposedBy, bool $requiresConfirmation = true): RewindFix
    {
        return RewindFix::query()->create([
            ...
            'case_id' => $case->id,
            'fix_type' => $proposal['fix_type'] ?? 'manual',
    ...
    public function applyFix(RewindFix $fix, array $actor): RewindFix
    {
        if ($fix->requires_confirmation && $fix->status !== 'confirmed') {
            throw new \RuntimeException('Fix must be confirmed before applying.');
        }
    ...
    private function applyByType(RewindFix $fix): array
    {
        ...
        if ($type === 'metadata_update') {
            ...
            if (!$targetTable || !$targetId || !$metaKey) {
                throw new \InvalidArgumentException('metadata_update missing required fields.');
            }

            // TEMPLATE allowlist: only titan_rewind_cases
            if ($targetTable !== 'titan_rewind_cases') {
                throw new \RuntimeException('Target table not allowlisted in template.');
            }

            $row = DB::table($targetTable)
                ->where('id', $targetId)
                ->where('company_id', $fix->company_id)
                ->where('user_id', $fix->user_id)
                ->first();
            ...
            DB::table($targetTable)
                ...
                ->where('company_id', $fix->company_id)
                ->where('user_id', $fix->user_id)
                ->update(['meta_json' => json_encode($after)]);

            return [
                'target_type' => $targetTable,
                'target_id' => $targetId,
                'before' => ['meta_json' => $before],
                'after' => ['meta_json' => $after],
            ];
        }

        throw new \RuntimeException("Unknown fix_type: {$type}");
    }
}
