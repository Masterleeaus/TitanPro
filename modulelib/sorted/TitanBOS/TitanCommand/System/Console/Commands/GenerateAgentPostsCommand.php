...
use App\Extensions\TitanCommand\System\Notifications\PostGenerationCompletedNotification;
...
class GenerateAgentPostsCommand extends Command
{
     ...
     */
    protected $signature = 'titancommand:generate-posts
    ...
    public function handle(): int
    {
        ...
        $this->info('🤖 Starting AI Titan Command Agent post generation...');

        $agentId = $this->option('agent');
        $force = $this->option('force');

        Log::info('titancommand:generate-posts started', [
        ...
        if ($agents->isEmpty()) {
            ...
            Log::info('titancommand:generate-posts finished', [
                'status'        => 'no_agents',
                'agents_found'  => 0,
                'posts_created' => 0,
            ]);
        ...
        foreach ($agents as $agent) {
            ...
            try {
                ...
                Log::info('titancommand:generate-posts agent finished', [
                    'agent_id'         => $agent->id,
    ...
    {
        $query = TitanCommand::query()->active();
     ...
     */
    protected function processAgent(TitanCommand $agent, bool $force = false): int
    {
        ...
        if (! $force && ! $this->needsMorePosts($agent, $targetPerPlatform)) {
            Log::info('titancommand:generate-posts skipped agent due to sufficient buffer', [
                'agent_id' => $agent->id,
            ]);
        ...
        if (empty($postsToGeneratePerPlatform)) {
            Log::info('titancommand:generate-posts no posts needed', [
                'agent_id'         => $agent->id,
                'calculated_posts' => 0,
            ]);
     ...
     */
    protected function updateGenerationStatus(TitanCommand $agent, string $status, array $data = []): void
    {
        ...
        if (in_array($status, ['completed', 'failed'], true)) {
            ...
            TitanCommandGenerationCache::mark($agent, $status, $payload);
        }
    ...
    protected function buildScheduleContext(TitanCommand $agent): array
    {
    ...
    protected function calculateTargetPostsPerPlatform(TitanCommand $agent, array $scheduleDays): int
    {
        $dailyPostCount = max(1, (int) $agent->daily_post_count);
     ...
     */
    protected function needsMorePosts(TitanCommand $agent, int $targetPerPlatform): bool
    {
     ...
     */
    protected function calculatePostsNeededByPlatform(TitanCommand $agent, int $targetPerPlatform): array
    {
        $needs = [];
    ...
    protected function countExistingPostsForPlatform(TitanCommand $agent, int $platformId): int
    {
        return $agent->posts()
    ...
    protected function getInitialScheduleCursor(TitanCommand $agent): ?Carbon
    {
    ...
    protected function determineTimeSlots(TitanCommand $agent, int $dailyPostCount): array
    {
