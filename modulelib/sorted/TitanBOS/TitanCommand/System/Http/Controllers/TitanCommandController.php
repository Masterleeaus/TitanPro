...
use App\Extensions\TitanCommand\System\Enums\StatusEnum;
use App\Extensions\TitanCommand\System\Models\CommandPlatform;
use App\Extensions\TitanCommand\System\Models\TitanCommandPost;
use App\Extensions\TitanCommand\System\Models\TitanCommandPostDailyMetric;
use App\Extensions\TitanCommand\System\Services\TitanCommandFollowerService;
use App\Extensions\TitanCommand\System\Models\TitanCommand;
use App\Extensions\TitanCommand\System\Services\ImageGenerationService;
use App\Extensions\TitanCommand\System\Services\PostGenerationService;
use App\Extensions\TitanCommand\System\Services\TargetAudienceService;
use App\Extensions\TitanCommand\System\Services\WebScrapingService;
use App\Extensions\TitanCommand\System\Support\TitanCommandGenerationCache;
...
class TitanCommandController extends Controller
{
    ...
    private string $scrapedContentCachePrefix = 'titancommand:scrape:';
    ...
    public function index(): View
    {
        ...
        $postsQuery = TitanCommandPost::query()
        ...
        $pending_posts_count = (clone $postsQuery)
            ->where('status', TitanCommandPost::STATUS_DRAFT)
            ->count();

        $scheduled_posts_count = (clone $postsQuery)
            ->where('status', TitanCommandPost::STATUS_SCHEDULED)
            ->count();

        $total_posts_count = $postsQuery->count();

        // Get paginated posts
        $posts = TitanCommandPost::query()
            ->whereHas('agent', function ($query) use ($userId) {
                $query->where('user_id', $userId);
            })
        ...
        $platforms = CommandPlatform::query()
            ->where('company_id', $companyId)->where('user_id', $userId)
            ->get();

        $agentsQuery = TitanCommand::query()
        ...
        if ($agentIds->isNotEmpty()) {
            $new_impressions = TitanCommandPostDailyMetric::query()
                ->whereIn('agent_id', $agentIds)
                ->whereDate('date', '>=', now()->subDay()->toDateString())
                ->sum('view_count');
        }

        $generationStatus = $defaultAgent ? TitanCommandGenerationCache::currentStatus($defaultAgent) : ['status' => 'idle'];

        return view('titancommand::dashboard.index', [
    ...
    public function postItems(Request $request): View
    {
        ...
        $baseQuery = fn () => TitanCommandPost::query()
        ...
        $view = 'titancommand::components.posts.carousel.post-items';

        if (! empty($postStyle)) {
            $view = 'titancommand::components.posts.' . $postStyle . '.post-items';
        }
    ...
    public function calendar(): View
    {
        ...
        $agents = TitanCommand::query()
            ->where('company_id', $companyId)->where('user_id', $userId)
            ->with('posts')
            ->latest()
            ->get();

        return view('titancommand::calendar.index', [
            'agents' => $agents,
        ]);
    }
    ...
    public function posts(): View
    {
        ...
        return view('titancommand::posts.index', [
            'posts'         => $posts,
     ...
     * Analytics page
     */
    public function analytics(TitanCommandFollowerService $followerService): View
    {
        $userId = Auth::id();

        $total_posts = TitanCommandPost::whereHas('agent', function ($q) use ($userId) {
            $q->where('user_id', $userId);
        })->count();

        $published_posts = TitanCommandPost::whereHas('agent', function ($q) use ($userId) {
            $q->where('user_id', $userId);
        ...
        $averageEngagement = (float) TitanCommandPost::query()
            ->where('user_id', $userId)
            ->whereNotNull('post_engagement_rate')
            ->avg('post_engagement_rate');

        $totalFollowers = $platforms->sum(function (CommandPlatform $platform) {
        ...
        return view('titancommand::analytics.index', [
            'stats'                => $stats,
            'platforms'            => $platforms,
            'news'                 => $newsFeed,
    ...
    private function syncPlatformFollowers(Collection $platforms, TitanCommandFollowerService $followerService): void
    {
        $platforms->each(function (CommandPlatform $platform) use ($followerService) {
    ...
    private function buildAnalyticsNews(int $userId, array $stats): array
    {
        ...
        $recentPost = TitanCommandPost::query()
            ->whereHas('agent', fn ($q) => $q->where('user_id', $userId))
            ->latest()
            ->first();
    ...
    private function buildImpressionsChartData(int $userId, $platforms, array $months): array
    {
        ...
        $records = TitanCommandPostDailyMetric::query()
        ...
        $todayTotals = TitanCommandPostDailyMetric::query()
    ...
    private function buildPublishedPostsChartData(int $userId, $platforms, array $months): array
    {
        ...
        $records = TitanCommandPost::query()
            ->where('user_id', $userId)
        ...
        $todayTotals = TitanCommandPost::query()
            ->where('user_id', $userId)
            ->where('status', StatusEnum::published)
            ->whereNotNull('social_media_platform_id')
    ...
    {
        $userPlatforms = CommandPlatform::query()
            ->where('company_id', $companyId)->where('user_id', $userId)
            ->get();
        ...
        return view('titancommand::accounts.index', [
    ...
    {
        return view('titancommand::chat.index', [
        ]);
    }
    ...
    public function create(): View
    {
        ...
        return view('titancommand::create.index', [
            'platforms' => $platforms,
        ]);
    }
    ...
    public function agents(): View
    {
        ...
        return view('titancommand::agents.index', [
            'agents'                => $agents,
            'platforms'             => $platforms,
     ...
     * Show edit form
     */
    public function edit(TitanCommand $agent): View
    {
        ...
        return view('titancommand::agent.edit', [
            'agent'     => $agent,
            'platforms' => $platforms,
        ]);
    }

    protected function determineAgentOfMonth(Collection $agents): ?TitanCommand
    {
        if ($agents->isEmpty()) {
            return null;
        }
        ...
        return $pool
            ->sortByDesc(function (TitanCommand $agent) {
     ...
     * Update agent
     */
    public function update(Request $request, TitanCommand $agent): RedirectResponse
    {
        $this->authorize('update', $agent);
     ...
     * Delete agent
     */
    public function destroy(TitanCommand $agent): RedirectResponse
    {
    ...
    public function previewPost(Request $request): JsonResponse
    {
        ...
        $tempAgent = new TitanCommand($request->all());
    ...
    public function getPosts(Request $request): JsonResponse
    {
        ...
        $baseQuery = fn () => tap(TitanCommandPost::query(), function ($query) use ($userId, $filters, $filterHandlers, $startDate, $endDate, $dateColumn, $search) {
    ...
    public function storePost(Request $request): JsonResponse
    {
        ...
        $this->validate($request, [
            ...
            'agent_id'             => 'required|exists:ext_titancommands,id',
        ...
        $platform = CommandPlatform::query()
        ...
        $status = TitanCommandPost::STATUS_SCHEDULED;
        ...
        if ($status === TitanCommandPost::STATUS_SCHEDULED) {
            $this->syncTitanCommandPost($post);
        }
     ...
     */
    public function generatePosts(TitanCommand $agent): RedirectResponse
    {
        $this->authorize('update', $agent);
     ...
     */
    public function approvePost(Request $request, TitanCommandPost $post): JsonResponse
    {
        ...
        try {
            ...
            // Create in TitanCommand extension's table
            $socialMediaPost = TitanCommandPost::create([
     ...
     */
    public function approveBulk(Request $request, TitanCommand $agent): JsonResponse
    {
        $this->authorize('update', $agent);

        $request->validate([
            'post_ids'   => 'required|array',
            'post_ids.*' => 'exists:ext_titancommand_posts,id',
        ]);
        ...
        foreach ($request->input('post_ids') as $postId) {
            $post = TitanCommandPost::find($postId);
     ...
     */
    public function rejectPost(TitanCommandPost $post): JsonResponse
    {
        if (Helper::appIsDemo()) {
            return response()->json([
                'type'    => 'error',
     ...
     * Duplicate a post
     */
    public function duplicatePost(TitanCommandPost $post): JsonResponse
    {
        $this->authorize('update', $post->agent);
        ...
        $duplicate->status = TitanCommandPost::STATUS_DRAFT;
     ...
     * Update a post
     */
    public function updatePost(Request $request, TitanCommandPost $post): JsonResponse
    {
        $this->authorize('update', $post->agent);

        if ($post->status === TitanCommandPost::STATUS_PUBLISHED) {
            return response()->json([
                'success' => false,
        ...
        if (isset($validated['platform_id'])) {
            $platformBelongsToUser = CommandPlatform::query()
        ...
        if (! empty($validated['scheduled_at'])) {
            ...
            $validated['status'] = TitanCommandPost::STATUS_SCHEDULED;
        }
        ...
        if ($post->status === TitanCommandPost::STATUS_SCHEDULED) {
    ...
    protected function resolveAgentForPlatform(int $platformId): ?TitanCommand
    {
        return TitanCommand::query()
            ->where('company_id', $companyId)->where('user_id', $userId)
            ->whereJsonContains('platform_ids', $platformId)
            ->first();
    }

    protected function queuePostGeneration(TitanCommand $agent): void
    {
        TitanCommandGenerationCache::forgetForUser($agent->user_id);

        $stats = TitanCommandGenerationCache::computePostStats($agent);
        ...
        TitanCommandGenerationCache::mark($agent, 'queued', [
    ...
    {
        $userId = Auth::id();
        $status = TitanCommandGenerationCache::getForUser($userId);
     ...
     */
    protected function generateInitialPosts(TitanCommand $agent, int $count = 7): int
    {
        $postService = new PostGenerationService;
        $generated = 0;
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($post['success']) {
                    ...
                    TitanCommandPost::create([
                        ...
                        'status'           => TitanCommandPost::STATUS_DRAFT,
                        'scheduled_at'     => $scheduledAt,
                    ]);
    ...
    protected function syncTitanCommandPost(TitanCommandPost $post): void
    {
        ...
        $payload = [
            ...
            'scheduled_at'               => $post->scheduled_at,
            'hashtags'                 	 => $post->hashtags,
            'titancommand_post_id' => $post->id,
        ];

        $socialMediaPost = null;

        if ($post->platform_post_id) {
            $socialMediaPost = TitanCommandPost::find($post->platform_post_id);
        }

        if ($socialMediaPost) {
            $socialMediaPost->update($payload);
        } else {
            $socialMediaPost = TitanCommandPost::create($payload);
        }
     ...
     */
    protected function calculateNextScheduleTime(TitanCommand $agent, int $offset = 0): DateTime
    {
        $now = now();
    ...
    public function falWebhook(Request $request): JsonResponse
    {
        try {
            ...
            $post = TitanCommandPost::where('image_request_id', $requestId)->first();
            ...
            if ($status === 'COMPLETED') {
                ...
                if ($imageUrl) {
                    ...
                    $imageService = new \App\Extensions\TitanCommand\System\Services\ImageGenerationService;
    ...
    protected function downloadAndStoreImage(string $url): string
    {
        ...
        $filename = 'titancommand/' . uniqid('post_', true) . '.png';
    ...
    private function estimatePlannedGenerationCount(TitanCommand $agent): int
    {
    ...
    private function estimateTargetPostsPerPlatform(TitanCommand $agent): int
    {
    ...
    private function estimatePostsNeededByPlatform(TitanCommand $agent, int $targetPerPlatform): array
    {
        $needs = [];
    ...
    private function countExistingFuturePosts(TitanCommand $agent, int $platformId): int
    {
        return $agent->posts()
            ->where('platform_id', $platformId)
            ->whereIn('status', [
                TitanCommandPost::STATUS_DRAFT,
                TitanCommandPost::STATUS_SCHEDULED,
            ])
    ...
    {
        $limit = $this->getTitanCommandLimitValue('agents');

        if ($limit === 0) {
            return $this->titanCommandAgentLimitErrorResponse(__('Your current plan does not allow creating AI Titan Command Agents.'));
        }

        if ($limit > 0) {
            $agentCount = TitanCommand::query()
                ->where('company_id', $companyId)->where('user_id', $userId)
                ->count();

            if ($agentCount >= $limit) {
                return $this->titanCommandAgentLimitErrorResponse(__('You have reached the maximum number of agents included in your plan.'));
            }
        }
    ...
    {
        $limit = $this->getTitanCommandLimitValue('monthly_posts');

        if ($limit === 0) {
            return $this->titanCommandAgentLimitErrorResponse(__('Your current plan does not allow creating posts with the AI Titan Command Agent.'));
        }

        if ($limit > 0) {
            $now = Carbon::now();
            $postsThisMonth = TitanCommandPost::query()
                ->whereHas('agent', fn ($query) => $query->where('company_id', $companyId)->where('user_id', $userId))
                ->whereBetween('created_at', [$now->copy()->startOfMonth(), $now->copy()->endOfMonth()])
                ->count();

            if ($postsThisMonth >= $limit) {
                return $this->titanCommandAgentLimitErrorResponse(__('You have reached your monthly Titan Command Agent post limit.'));
            }
        }
    ...
    private function getTitanCommandLimitValue(string $key): int
    {
        $plan = Auth::user()?->relationPlan;
        $limits = (array) ($plan?->titancommand_limits ?? []);
        $value = $limits[$key] ?? -1;

        return is_numeric($value) ? (int) $value : -1;
    }

    private function titanCommandAgentLimitErrorResponse(string $message): JsonResponse
    {
        return response()->json([
            'success' => false,
            'message' => $message,
