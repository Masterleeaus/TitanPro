...
namespace App\Extensions\TitanCommand\System\Services\Analysis;
...
{
    public function buildMetricsSnapshot(TitanCommand $agent, int $lookbackDays = 14): array
    {
        ...
        $recentPosts = $posts->filter(fn (TitanCommandPost $post) => $post->posted_at && $post->posted_at->greaterThanOrEqualTo($recentStart));
        $previousPosts = $posts->filter(fn (TitanCommandPost $post) => $post->posted_at && $post->posted_at->lessThan($recentStart));
    ...
    public function buildProfileSummary(TitanCommand $agent): array
    {
    ...
    {
        $values = $posts
            ->map(fn (TitanCommandPost $post) => $post->{$attribute})
    ...
    {
        return $posts
            ->groupBy(fn (TitanCommandPost $post) => $this->platformValue($post))
    ...
    private function platformValue(TitanCommandPost $post): string
    {
    ...
    private function summarizePost(?TitanCommandPost $post): ?array
    {
        if (! $post) {
            return null;
        }
    ...
    private function topPost(Collection $posts): ?TitanCommandPost
    {
        return $posts->sortByDesc(fn (TitanCommandPost $post) => (float) ($post->post_engagement_rate ?? 0))->first();
    }

    private function bottomPost(Collection $posts): ?TitanCommandPost
    {
        return $posts->sortBy(fn (TitanCommandPost $post) => (float) ($post->post_engagement_rate ?? 0))->first();
    }
}
