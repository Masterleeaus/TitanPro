...
namespace App\Extensions\TitanCommand\System\Support;
...
class TitanCommandGenerationCache
{
    private const CACHE_PREFIX = 'titancommand:generation:';
    ...
    public static function mark(TitanCommand $agent, string $status, array $payload = []): array
    {
    ...
    public static function currentStatus(TitanCommand $agent): array
    {
    ...
    public static function computePostStats(TitanCommand $agent): array
    {
        ...
        $query = TitanCommandPost::query()
            ->whereIn('agent_id', $agentIds);

        $pending = (clone $query)->where('status', TitanCommandPost::STATUS_DRAFT)->count();
        $scheduled = (clone $query)->where('status', TitanCommandPost::STATUS_SCHEDULED)->count();
        $total = $query->count();
