...
namespace App\Extensions\TitanCommand\System\Database\Seeders;
...
class TitanCommandDemoSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $platforms = CommandPlatform::where('user_id', $user->id)->get();
     ...
     */
    protected function createDemoPosts(TitanCommand $agent, $platforms): void
    {
        ...
        for ($i = 0; $i < $postCount; $i++) {
            ...
            $statusDistribution = [
                TitanCommandPost::STATUS_DRAFT     => 50, // 50%
                TitanCommandPost::STATUS_SCHEDULED => 30, // 30%
                TitanCommandPost::STATUS_PUBLISHED => 20, // 20%
            ];
            $status = $this->faker->randomElement(array_merge(
                array_fill(0, $statusDistribution[TitanCommandPost::STATUS_DRAFT], TitanCommandPost::STATUS_DRAFT),
                array_fill(0, $statusDistribution[TitanCommandPost::STATUS_SCHEDULED], TitanCommandPost::STATUS_SCHEDULED),
                array_fill(0, $statusDistribution[TitanCommandPost::STATUS_PUBLISHED], TitanCommandPost::STATUS_PUBLISHED)
            ));
    ...
    protected function generateDummyImage(string $postType, int $index, int $imageIndex = 0): ?string
    {
        try {
            ...
            if ($response->successful()) {
                $filename = 'titancommand/demo/' . uniqid('post_', true) . '.jpg';
