@php
    $banner_bg_light = asset('vendor/command-agent/images/img-4.jpg');
    $banner_bg_dark = asset('vendor/command-agent/images/img-4-dark.png');
@endphp

@push('css')
    <style>
        .command-agent-banner {
            background-image: url({{ $banner_bg_light }});
        }

        .theme-dark .command-agent-banner {
...
<div
    class="command-agent-banner mb-10 flex flex-wrap items-center rounded-2xl bg-[url('{{ $banner_bg_light }}')] bg-cover bg-top px-5 dark:bg-white/[3%] dark:bg-[url('{{ $banner_bg_dark }}')] lg:px-12"
>
        ...
        >
            ...
            >
                <x-number-counter
                    ...
                    id="command-agent-pending-sub-items-counter"
            ...
            <span class="flex grow items-center justify-between gap-3">
                <span class="m-0 flex items-center gap-3">
                    ...
                    <template x-if="!showReadyProgressText">
                        <span>{{ __('Your new sub-item are ready for review.') }}</span>
                    </template>
                </span>
            </span>
        </a>
    ...
    <div class="flex w-full justify-center max-lg:order-first lg:w-1/2">
        <video
            ...
            width="1200"
            height="1050"
            src="{{ asset('vendor/command-agent/videos/banner-video.mp4') }}"
            autoplay
            playsinline
            muted
            loop
        ...
        <video
            ...
            width="1200"
            height="1050"
            src="{{ asset('vendor/command-agent/videos/banner-video-dark.mp4') }}"
            autoplay
            playsinline
            muted
            loop
