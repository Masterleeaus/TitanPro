...
<tr
    data-name="{{ data_get($container['credentials'], 'name') }}"
    data-container="{{ $container->container }}"
    x-show="!searchString || ($el.getAttribute('data-name').toLowerCase().includes(searchString) || $el.getAttribute('data-container').toLowerCase().includes(searchString))"
    x-transition
>
    <td>
        <div class="m-0 flex items-center gap-3">
            <figure class="inline-grid size-9 place-items-center overflow-hidden rounded-full">
                @if (data_get($container['credentials'], 'picture') && data_get($container['credentials'], 'picture') !== 'test')
                    <img
                        class="size-full object-cover object-center"
                        src="{{ data_get($container['credentials'], 'picture') }}"
                    >
                @else
                    <svg
                        class="size-7"
            ...
            <p class="m-0 font-medium">
                <span class="block text-xs">
                    {{ data_get($container['credentials'], 'name') }}
                </span>
    ...
    <td class="text-2xs">
        {{ date('M j Y', strtotime($container->connected_at)) }}
    </td>
    ...
    <td>
        <p class="m-0 flex items-center gap-3 text-2xs font-medium">
            <span @class([
                ...
                'bg-green-500' => $container->isConnected(),
                'bg-foreground/50' => !$container->isConnected(),
            ])></span>
            {{ $container->isConnected() ? __('Active') : __('Inactive') }}
        </p>
    </td>
    ...
    <td>
        <div class="flex items-center justify-end gap-2 font-normal">
            <x-button
                ...
                title="{{ __('View') }}"
                href="{{ route('social-media.oauth.connect.' . $container->container) . '?platform_id=' . $container->getKey() }}"
                onclick="return confirm('{{ __('Are you sure? This is permanent..') }}')"
            >

                @if ($container->expires_at < now()->subMinutes(10))
            ...
            <x-button
                ...
                variant="ghost-shadow"
                title="{{ __('View') }}"
                href="{{ route('dashboard.user.social-media.containers.disconnect', $container->getKey()) }}"
                onclick="return confirm('{{ __('Are you sure? This is permanent..') }}')"
            >
