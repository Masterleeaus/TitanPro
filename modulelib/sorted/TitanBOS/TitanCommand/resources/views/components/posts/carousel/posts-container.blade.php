...
<div
    id="command-agent-sub-items-carousel"
    @class([
        '-mx-5 overflow-hidden [&_.flickity-viewport]:w-full [&_.flickity-slider]:w-full' => filled(
            $sub-items),
        'flex relative [&.flickity-resize_.command-agent-sub-item-item]:min-h-full',
    ])
    :class="{ 'animate-pulse': currentTasks.has('fetchingPosts') }"
    x-ref="postsCarousel"
>
    @include('command-agent::components.sub-items.carousel.sub-item-items', ['items' => $sub-items])

    @if ($sub-items->hasMorePages())
        <div
            class="command-agent-sub-items-carousel-load-more-wrap command-agent-sub-item-item flex min-h-full w-full flex-shrink-0 flex-grow-0 basis-auto flex-col px-5 py-2 md:w-1/2 lg:w-1/4">
            <div class="flex min-h-full grow items-center justify-center p-3.5">
                <a
                    class="command-agent-sub-items-carousel-load-more group inline-flex w-full flex-col items-center gap-2 p-10 text-center text-sm font-medium"
                    href="{{ route('dashboard.user.social-media.agent.sub-item-items', ['page' => 2, 'post_style' => 'carousel']) }}"
                ...
                >
                    <span x-text="allPostsLoaded ? '{{ __('All sub-items loaded') }}' : loadingMore ? '{{ __('Loading...') }}' : '{{ __('Load more') }}'">
                        @lang('Load more')
                    </span>
