...
@section('title', __('Create Channels Agent'))
@section('titlebar_subtitle', __('Automate your channels posting with AI-powered agents'))
            ...
            <div class="mx-auto lg:max-w-[500px]">
                ...
                >
                    <template
                        ...
                        :key="'hidden-container-' + platformId"
                    >
                        <input
                            type="hidden"
                            name="platform_ids[]"
                    ...
                    @include('command-agent::create.create-step-0')
                    @include('command-agent::create.create-step-1')
                    @include('command-agent::create.create-step-2')
                    @include('command-agent::create.create-step-3')
                    @include('command-agent::create.create-step-4')
                    @include('command-agent::create.create-step-5')
                    @include('command-agent::create.create-step-6', ['days_of_week' => $days_of_week])
                    @include('command-agent::create.create-step-7', ['time_slots' => $time_slots])
                    @include('command-agent::create.create-step-8')
                    @include('command-agent::create.create-step-9')
                </form>
            </div>
        </div>
    </div>
            ...
            Alpine.data('agentWizard', () => ({
                    ...
                    switch (this.currentStep) {
                        case 1:
                            if (!this.formData.platform_ids.length) {
                                this.stepsErrors.get(this.currentStep).push('{{ __('Please select a container.') }}');
                            }
                        ...
                        case 4:
                            if (!this.formData.post_types.length) {
                                this.stepsErrors.get(this.currentStep).push('{{ __('Please select sub-item types.') }}');
                            }
                ...
                async nextStep(errorsArray) {
                    ...
                    if (this.currentStep === 2 && this.shouldForceManualDescription()) {
                        window.dispatchEvent(new CustomEvent('command-agent-force-description'));
                    }
                ...
                notifyTimeSlotLimit(limit) {
                    if (window?.toastr) {
                        const message = limit === 1 ?
                            ...
                            '{{ __('Increase the number of sub-items per day before selecting more time slots.') }}';
