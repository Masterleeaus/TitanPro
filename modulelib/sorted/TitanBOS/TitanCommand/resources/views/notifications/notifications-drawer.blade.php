@php
    $button_classname = @twMerge('lqd-command-agent-notification-trigger size-6 !p-0 max-lg:hidden max-lg:size-10 flex items-center justify-center hover:bg-transparent max-lg:rounded-full max-lg:border max-lg:dark:bg-white/[3%] relative', $attributes->get('class'));
@endphp

<div
    class="inline-flex items-center"
    x-data="Command AgentNotifications"
>
    <x-button
        class="{{ $button_classname }}"
        size="none"
        href="#"
        title="{{ __('Channels Notifications') }}"
        variant="link"
    ...
    <x-sidedrawer id="command-agent-notifications-sidedrawer">
...
@pushOnce('script')
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('Command AgentNotifications', () => ({
                notifications: [],
                unreadsCount: 0,
                ...
                async init() {
                    this.notificationsSidedrawer = document.querySelector('#command-agent-notifications-sidedrawer');
