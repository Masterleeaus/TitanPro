...
@section('title', trans('Channels Agent Chat'))
...
@push('css')
    ...
    <style>
        .chat-content-container:has(.command-agent-chat-sub-item-card:only-child) {
            width: 100%;
        }

        .command-agent-chat-sub-item-card {
            width: clamp(70%, 440px, 100%);
        }

        .command-agent-chat-sub-item-card>p:first-of-type {
            display: none;
        }
    </style>
@endpush
...
@section('content')
                        ...
                        @else
                            <x-button
                                ...
                                onclick="{!! $disable_actions
                                    ...
                                    : (auth()->check()
                                        ? 'return startNewChat(\'{{ $category->id }}\', \'{{ LaravelLocalization::getCurrentLocale() }}\', \'command-agent\')'
            ...
            <div class="grow px-2 md:px-5 lg:px-0">
                <div @class([
                    ...
                    <div class="hidden w-5/12 items-center gap-4 lg:flex">
                        ...
                        @include('command-agent::chat.components.chat_category_dropdown')
                    ...
                    >
                        ...
                        >
                            ...
                            @if ($chat != null)
                                ...
                                @else
                                    @include('command-agent::chat.includes.chat_area_container')
                                @endif
                            @else
                                <div
    ...
    @include('command-agent::chat.components.chat-sub-item-card-template')
    ...
    <script>
        (() => {
            document.addEventListener('alpine:init', () => {
                ...
                Alpine.data('Command AgentChatPostCard', () => ({
                    sub-item: null,
                    contentTemplate: document.querySelector('#command-agent-chat-sub-item-card-template'),
                    scheduleButton: null,
                    ...
                    async init() {
                        ...
                        const data = await this.fetchPost({
                            query: `id=${this.$el.getAttribute('data-sub-item-id')}`
                        });
                        const sub-item = data.sub-items.data[0];

                        if (!sub-item) {
                            return toastr.error('{{ __('Failed Fetching Sub-item.') }}')
                        }

                        this.sub-item = sub-item;
                    },
                    ...
                    onPostUpdated(event) {
                        ...
                        if (sub-item?.id !== this.sub-item.id) return;
                    ...
                    onPostRejected(event) {
                        ...
                        if (postId !== this.sub-item.id) return;
