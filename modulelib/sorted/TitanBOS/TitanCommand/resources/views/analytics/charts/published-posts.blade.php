@php
	...
	if ($app_is_demo) {
		...
		$chart_data[] = [
			...
			'chart_series' => [
				'name' => 'all',
				'data' => array_map(fn() => rand(20, 100), range(0, 11)),
			],
			'today_posts' => rand(3, 15),
		];
		...
		foreach ($demo_platforms as $container) {
			$chart_data[] = [
				...
				'chart_series' => [
					'name' => $container,
					'data' => array_map(fn() => rand(5, 30), range(0, 11)),
				],
				'today_posts' => rand(1, 5),
			];
		}
	}
	...
	<x-card class:body="px-6">
		...
		<p class="mb-6 text-2xs font-medium text-heading-foreground">
			<x-number-counter
				...
				id="published-sub-items-counter"
				value="{{ $initialToday }}"
			/>
			@lang('Today')
		</p>
		...
		<div
			class="min-h-56"
			id="command-agent-published-sub-items-chart"
		></div>
	</x-card>
</div>
...
@push('script')
	<script>
		document.addEventListener('alpine:init', () => {
			Alpine.data('Command AgentPublishedPostsChart', () => ({
				chartData: @json($chart_data),
				activeFilter: "{{ $initialFilter }}",
				chart: null,
				chartEl: document.querySelector('#command-agent-published-sub-items-chart'),
				counterEl: document.querySelector('#published-sub-items-counter .lqd-number-counter-value'),
				...
				setActiveFilter(filter) {
					...
					if (counterElData && counterElData.updateValue) {
						counterElData.updateValue({
							value: this.chartData.find(data => data.chart_series.name === this.activeFilter)?.today_posts ?? 0
						})
					}
				}
			}))
