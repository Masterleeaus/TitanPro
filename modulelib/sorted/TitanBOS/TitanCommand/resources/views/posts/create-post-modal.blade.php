@php
    $compose_titles = collect($containers)
        ->mapWithKeys(function ($container) {
            $platformId = data_get($container, 'id');

            if ($platformId === null) {
                return [];
            }

            $platformSlug = (string) data_get($container, 'container', '');
            $platformName = $platformSlug !== '' ? Str::headline($platformSlug) : __('Container');
            $firstCharacter = Str::lower(Str::substr($platformName, 0, 1));
            $article = in_array($firstCharacter, ['a', 'e', 'i', 'o', 'u']) ? 'an' : 'a';

            return [
                $platformId => __('Compose :article :container sub-item', [
                    'article' => $article,
    ...
    $default_platform_id = data_get($containers, '0.id');
    $default_agent_id = data_get($agent_options, '0.id');
@endphp

<template x-teleport="body">
    <div
        class="command-agent-create-sub-item-modal fixed start-0 top-0 z-10 h-screen w-screen overflow-y-auto overscroll-contain bg-background"
        x-cloak
        x-show="createPostModalShow"
        x-transition.opacity
        x-data="Command AgentCreatePostModal"
        @create-sub-item-modal-show.window="createPostModalShow = {show: $event.detail.show, platformId: ($event.detail.platformId ?? containers.at(0)?.id ?? 1)}"
    >
                            ...
                            >
                                @foreach ($containers as $container)
                                    @php
                                        ...
                                        $image_dark_version = 'vendor/social-media/icons/' . $container->container . '-mono-light.svg';
                                    ...
                                    <div
                                        ...
                                        @click.prevent="formInputs.platform_id = {{ $container->id }}; dropdownOpen = false; ensureAgentMatchesPlatform(formInputs.platform_id);"
                                        x-show="formInputs.platform_id != {{ $container->id }}"
                                    >
                                        <figure class="w-5 transition-all group-hover:scale-125">
                                            <img
...
@pushOnce('script')
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('Command AgentCreatePostModal', () => ({
                _showModal: false,
                ...
                getPlatformById(platformId) {
                    return this.containers.find(container => container.id === platformId);
                },
                ...
                async onCreatePostFormSubmit() {
                    ...
                    if (!this.formInputs.content.trim()) {
                        return toastr.error('{{ __('Please fill in the sub-item content.') }}');
                    }

                    this.creatingPost = true;

                    try {
                        const response = await fetch('{{ route('dashboard.user.social-media.agent.api.sub-items.store') }}', {
                            method: 'Sub-item',
                            headers: {
                        ...
                        if (!response.ok || data.success === false) {
                            throw new Error(data.message || '{{ __('Sub-item could not be created.') }}');
                        }

                        toastr.success(data.message || '{{ __('Sub-item created.') }}');
                        this.resetForm();
                        this.createPostModalShow = false;
                        window.dispatchEvent(new CustomEvent('command-agent:sub-item-created'));
                    } catch (error) {
                ...
                async generateAIImage() {
                    ...
                    try {
                        const response = await fetch('{{ route('dashboard.user.social-media.agent.api.sub-items.generate-image') }}', {
                            method: 'Sub-item',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json',
                ...
                async generateAIContent() {
                    ...
                    try {
                        const response = await fetch('{{ route('dashboard.user.social-media.agent.api.sub-items.generate-content') }}', {
                            method: 'Sub-item',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json',
