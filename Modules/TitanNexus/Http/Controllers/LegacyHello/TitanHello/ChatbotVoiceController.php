...
use Extensions\TitanNexus\Helpers\Requests\VoiceChatbotStoreRequest;
use Extensions\TitanNexus\Helpers\Requests\VoiceChatbotUpdateRequest;
...
class ChatbotVoiceController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $since = now()->subDay();
        $recentCalls = TitanNexusCallSession::query()
            ->where('created_at', '>=', $since)
            ->orderByDesc('created_at')
            ->limit(20)
            ->get();

        $missedCalls = TitanNexusCallSession::query()
            ->where('created_at', '>=', $since)
