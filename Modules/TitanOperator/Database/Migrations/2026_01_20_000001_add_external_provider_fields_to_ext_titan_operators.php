...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('tz_portal_operator_bots', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('tz_portal_operator_bots', 'provider_type')) {
                $table->string('provider_type', 32)->nullable()->after('human_agent_conditions');
            }
            if (!Schema::hasColumn('tz_portal_operator_bots', 'external_endpoint_url')) {
                $table->text('external_endpoint_url')->nullable()->after('provider_type');
            }
            if (!Schema::hasColumn('tz_portal_operator_bots', 'external_auth_type')) {
                $table->string('external_auth_type', 32)->nullable()->after('external_endpoint_url');
            }
            if (!Schema::hasColumn('tz_portal_operator_bots', 'external_auth_token')) {
                $table->text('external_auth_token')->nullable()->after('external_auth_type');
            }
            if (!Schema::hasColumn('tz_portal_operator_bots', 'external_signing_secret')) {
                $table->text('external_signing_secret')->nullable()->after('external_auth_token');
            }
            if (!Schema::hasColumn('tz_portal_operator_bots', 'external_timeout_ms')) {
    ...
    public function down(): void
    {
        Schema::table('tz_portal_operator_bots', function (Blueprint $table) {
            foreach (['provider_type','external_endpoint_url','external_auth_type','external_auth_token','external_signing_secret','external_timeout_ms'] as $col) {
                if (Schema::hasColumn('tz_portal_operator_bots', $col)) {
