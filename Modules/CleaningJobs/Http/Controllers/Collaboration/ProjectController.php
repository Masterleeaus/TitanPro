...
namespace Modules\CleaningJobs\Http\Controllers\JobBoard;
...
use Modules\CleaningJobs\Models\JobBoard\ActivityLog;
use Modules\CleaningJobs\Models\JobBoard\BugComment;
use Modules\CleaningJobs\Models\JobBoard\BugFile;
use Modules\CleaningJobs\Models\JobBoard\BugReport;
use Modules\CleaningJobs\Models\JobBoard\BugStage;
use Modules\CleaningJobs\Models\JobBoard\ClientProject;
use Modules\CleaningJobs\Models\JobBoard\Comment;
use Modules\CleaningJobs\Models\JobBoard\Milestone;
use Modules\CleaningJobs\Models\JobBoard\Project;
use Modules\CleaningJobs\Models\JobBoard\ProjectFile;
use Modules\CleaningJobs\Models\JobBoard\Stage;
use Modules\CleaningJobs\Models\JobBoard\SubTask;
use Modules\CleaningJobs\Models\JobBoard\Task;
use Modules\CleaningJobs\Models\JobBoard\TaskFile;
use Modules\CleaningJobs\Models\JobBoard\UserProject;
use Modules\CleaningJobs\Models\JobBoard\VenderProject;
use Workdo\TimeTracker\Entities\TimeTracker;
use Modules\CleaningJobs\Events\JobBoard\CreateBug;
use Modules\CleaningJobs\Events\JobBoard\CreateMilestone;
use Modules\CleaningJobs\Events\JobBoard\CreateProject;
use Modules\CleaningJobs\Events\JobBoard\CreateTask;
use Modules\CleaningJobs\Events\JobBoard\CreateTaskComment;
use Modules\CleaningJobs\Events\JobBoard\DestroyBug;
use Modules\CleaningJobs\Events\JobBoard\DestroyMilestone;
use Modules\CleaningJobs\Events\JobBoard\DestroyProject;
use Modules\CleaningJobs\Events\JobBoard\DestroyTask;
use Modules\CleaningJobs\Events\JobBoard\DestroyTaskComment;
use Modules\CleaningJobs\Events\JobBoard\UpdateBug;
use Modules\CleaningJobs\Events\JobBoard\UpdateMilestone;
use Modules\CleaningJobs\Events\JobBoard\UpdateProject;
use Modules\CleaningJobs\Events\JobBoard\UpdateTask;
use Modules\CleaningJobs\Events\JobBoard\UpdateTaskStage;
use Modules\CleaningJobs\Events\JobBoard\ProjectInviteUser;
use Modules\CleaningJobs\Events\JobBoard\ProjectShareToClient;
use Modules\CleaningJobs\Events\JobBoard\ProjectUploadFiles;
use Modules\CleaningJobs\Events\JobBoard\UpdateBugStage;
use Workdo\Account\Entities\Bill;
use Workdo\Documents\Entities\Document;
use Workdo\Retainer\Entities\Retainer;
use App\Models\Proposal;
use App\Models\Purchase;
use Workdo\JobBoard\DataTables\ProjectBugDatatable;
use Workdo\JobBoard\DataTables\ProjectDatatable;
use Workdo\JobBoard\DataTables\ProjectTaskDatatable;
...
class ProjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth::user()->isAbleTo('project manage')) {
            ...
            return view('jobboard::projects.index', compact('projects'));
        } else {
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('project create')) {
            if (module_is_active('CustomField')) {
                $customFields =  \Workdo\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'jobboard')->where('sub_module', 'projects')->get();
            } else {
                $customFields = null;
            }
            $workspace_users  = User::where('created_by', creatorId())->emp()->where('workspace_id', getActiveWorkSpace())->orWhere('id', Auth::user()->id)->get();
            return view('jobboard::projects.create', compact('customFields', 'workspace_users'));
        } else {
    ...
    public function show($id)
    {
        if (\Auth::user()->isAbleTo('project show')) {
            ...
            if (module_is_active('CustomField')) {
                $project->customField = \Workdo\CustomField\Entities\CustomField::getData($project,'jobboard', 'projects');
                $customFields = \Workdo\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace($project->created_by))->where('module', '=', 'jobboard')->where('sub_module', 'projects')->get();
            } else {
                $customFields = null;
            }
            ...
            if ($project) {
                ...
                return view('jobboard::projects.show', compact('project', 'daysleft', 'chartData','customFields'));
            }
    ...
    public function edit(Project $project)
    {
        if (Auth::user()->isAbleTo('project edit')) {
            ...
            return view('jobboard::projects.edit', compact('project', 'customFields'));
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    ...
    public function destroy($projectID)
    {
        ...
        if ($project->created_by == $objUser->id || $objUser->type == 'company') {
            ...
            if ($task == 0 && $bug == 0) {
                ...
                if (module_is_active('CustomField')) {
                    $customFields = \Workdo\CustomField\Entities\CustomField::where('module', 'jobboard')->where('sub_module', 'projects')->get();
    ...
    public function milestone($projectID)
    {
        $currentWorkspace = getActiveWorkSpace();
        $project          = Project::find($projectID);
        return view('jobboard::projects.milestone', compact('currentWorkspace', 'project'));
    }
    ...
    public function milestoneEdit($milestoneID)
    {
        ...
        return view('jobboard::projects.milestoneEdit', compact('milestone', 'currentWorkspace'));
    }
    ...
    public function milestoneShow($milestoneID)
    {
        ...
        return view('jobboard::projects.milestoneShow', compact('currentWorkspace', 'milestone'));
    }
    ...
    public function popup($projectID)
    {
        ...
        return view('jobboard::projects.invite', compact('currentWorkspace', 'project', 'workspace_users', 'workspace_clients'));
    }
    ...
    public function sharePopup($projectID)
    {
        ...
        if($project){
            ...
            return view('jobboard::projects.share', compact('currentWorkspace', 'project', 'clients'));
        }else{
    ...
    public function sharePopupVender($projectID)
    {
        ...
        return view('jobboard::projects.share_vender', compact('currentWorkspace', 'project', 'venders'));
    }
    ...
    public function gantt($projectID, $duration = 'Week')
    {
        if (\Auth::user()->isAbleTo('sub-task manage')) {
            ...
            return view('jobboard::projects.gantt', compact('currentWorkspace', 'project', 'tasks', 'duration', 'is_client'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function taskBoard($projectID)
    {
        if (\Auth::user()->isAbleTo('task manage')) {
            ...
            if ($project) {
                ...
                return view('jobboard::projects.taskboard', compact('currentWorkspace', 'project', 'stages', 'statusClass'));
            } else {
    ...
    public function taskCreate($projectID)
    {
        ...
        $currentWorkspace = getActiveWorkSpace();
        if (module_is_active('CustomField')) {
            $customFields =  \Workdo\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'jobboard')->where('sub_module', 'tasks')->get();
        } else {
            $customFields = null;
        }
        if ($objUser->hasRole('client')) {
        ...
        return view('jobboard::projects.taskCreate', compact('currentWorkspace', 'customFields', 'project', 'projects', 'users'));
    }
    ...
    public function taskShow($projectID, $taskID)
    {
        ...
        if (module_is_active('CustomField')) {
            $task->customField = \Workdo\CustomField\Entities\CustomField::getData($task,'jobboard', 'tasks');
            $customFields = \Workdo\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace($task->created_by))->where('module', '=', 'jobboard')->where('sub_module', 'tasks')->get();
        } else {
            $customFields = null;
        }

        return view('jobboard::projects.taskShow', compact('currentWorkspace','users', 'task', 'clientID','projectID' ,'assign_to','customFields'));
    }
    ...
    public function taskEdit($projectID, $taskId)
    {
        ...
        return view('jobboard::projects.taskEdit', compact('currentWorkspace', 'project', 'projects', 'users', 'task', 'customFields','stages'));
    }
    ...
    public function taskDestroy($projectID, $taskID)
    {
        ...
        if (module_is_active('CustomField')) {
            $customFields = \Workdo\CustomField\Entities\CustomField::where('module', 'jobboard')->where('sub_module', 'tasks')->get();
            foreach ($customFields as $customField) {
    ...
    public function bugReport($project_id)
    {
        if (\Auth::user()->isAbleTo('bug manage')) {
            ...
            if(!empty($project)){
                ...
                return view('jobboard::projects.bug_report', compact('currentWorkspace', 'project', 'stages', 'statusClass'));
            }else{
    ...
    public function bugReportCreate($project_id)
    {
        ...
        if (module_is_active('CustomField')) {
            $customFields =  \Workdo\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'jobboard')->where('sub_module', 'bugs')->get();
        } else {
            $customFields = null;
        }
        if ($objUser->hasRole('client')) {
        ...
        return view('jobboard::projects.bug_report_create', compact('currentWorkspace', 'project', 'users', 'arrStatus', 'customFields'));
    }
    ...
    public function bugReportEdit($project_id, $bug_id)
    {
        ...
        if (module_is_active('CustomField')) {
            $bug->customField = \Workdo\CustomField\Entities\CustomField::getData($bug, 'jobboard', 'bugs');
            $customFields             = \Workdo\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'jobboard')->where('sub_module', 'bugs')->get();
        } else {
            $customFields = null;
        }
        ...
        return view('jobboard::projects.bug_report_edit', compact('currentWorkspace', 'project', 'users', 'bug', 'arrStatus', 'customFields'));
    }
    ...
    public function bugReportDestroy($project_id, $bug_id)
    {
        ...
        if (module_is_active('CustomField')) {
            $customFields = \Workdo\CustomField\Entities\CustomField::where('module', 'jobboard')->where('sub_module', 'bugs')->get();
            foreach ($customFields as $customField) {
    ...
    public function bugReportShow($project_id, $bug_id)
    {
        ...
        if (module_is_active('CustomField')) {
            $bug->customField = \Workdo\CustomField\Entities\CustomField::getData($bug,'jobboard', 'bugs');
            $customFields = \Workdo\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace($bug->created_by))->where('module', '=', 'jobboard')->where('sub_module', 'bugs')->get();
        } else {
            $customFields = null;
        }

        return view('jobboard::projects.bug_report_show', compact('currentWorkspace', 'bug', 'clientID','customFields'));
    }
    ...
    public function tracker($id)
    {
        if (Auth::user()->isAbleTo('time tracker manage')) {
            ...
            return view('jobboard::time_trackers.index', compact('currentWorkspace', 'treckers', 'id'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function List(ProjectDatatable $dataTable)
    {
        if (Auth::user()->isAbleTo('project manage')) {
            ...
            return $dataTable->render('jobboard::projects.list');
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function TaskList(ProjectTaskDatatable $dataTable,$projectID)
    {
        if (\Auth::user()->isAbleTo('task manage')) {
            ...
            if ($project) {
                ...
                return $dataTable->render('jobboard::projects.tasklist',compact('currentWorkspace','project'));
            } else {
    ...
    public function BugList(ProjectBugDatatable $dataTable,$project_id)
    {
        if (\Auth::user()->isAbleTo('bug manage'))
        {
            ...
            return $dataTable->render('jobboard::projects.bug_report_list',compact('currentWorkspace', 'project'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function copyproject($id)
    {
        if (Auth::user()->isAbleTo('project create')) {
            $project = Project::find($id);
            return view('jobboard::projects.copy', compact('project'));
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    ...
    public function fileImportExport()
    {
        if (Auth::user()->isAbleTo('project import')) {
            return view('jobboard::projects.import');
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    ...
    {
        if (Auth::user()->isAbleTo('project import')) {
            return view('jobboard::projects.import_modal');
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    ...
    public function CopylinkSetting($id)
    {
        if (Auth::user()->isAbleTo('project setting')) {
            $project = Project::find($id);
            return view('jobboard::projects.copylink_setting', compact('project'));
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    ...
    public function ProjectSharedLink($id = null, $lang = null)
    {
        ...
        if (!empty($project->copylinksetting) && json_decode($project->copylinksetting)->password_protected == 'on' && $project_id != $id_de) {
            return view('jobboard::projects.password_check', compact('company_id', 'workspace_id', 'id', 'lang'));
        }
        if ($project) {
            $bills = [];
            if (module_is_active('Account')) {
                $bills = Bill::where('workspace', '=', getActiveWorkSpace($company_id))->where('bill_module', '=', 'jobboard')->where('category_id', '=', $project->id)->get();
            }
            ...
            $invoices = Invoice::where('workspace', '=', $workspace_id)->where('invoice_module', '=', 'jobboard')->where('category_id', $project_id)->get();
            $retainers =[];
            ...
            return view('jobboard::projects.sharedlink', compact('company_id', 'workspace_id', 'project', 'id', 'lang', 'chartData', 'daysleft', 'stages', 'bug_stages', 'invoices', 'bills', 'documents','retainers','proposals'));
    ...
    public function ProjectLinkTaskShow($projectID, $taskID)
    {
        if ($taskID) {
            $task    = Task::find($taskID);
            $project = Project::find($projectID);
            return view('jobboard::projects.linktaskShow', compact('task', 'project'));
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    public function ProjectLinkbugReportShow($projectID, $bug_id)
    {
        if (!empty($projectID) && !empty($bug_id)) {
            $project = Project::find($projectID);
            $bug     = BugReport::find($bug_id);
            return view('jobboard::projects.link_bug_report_show', compact('bug', 'project'));
        } else {
            return response()->json(['error' => __('Permission denied.')], 401);
        }
    }
    ...
    public function calendar(Request $request,$id){
        ...
        return view('jobboard::projects.calendar', compact('current_month_task','arrTask' ,'id'));
    }
    ...
    public function finance(Request $request ,$id){
        ...
        return view('jobboard::projects.proposal' ,compact('project' ,'id' ,'proposals'));
    ...
    public function proposal(Request $request ,$id){
        ...
        return view('jobboard::projects.proposal', compact( 'id','project','proposals'));
    ...
    public function purchases(Request $request ,$id){
        ...
        return view('jobboard::projects.purchases', compact( 'id'));
    }


    public function invoice(Request $request ,$id){
        $project = Project::find($id);
        $query = Invoice::where('workspace', getActiveWorkSpace())->where('account_type','JobBoard')->where('category_id',$id);
        $invoices = $query->with('customers')->orderBy('id', 'desc')->get();
        return view('jobboard::projects.invoice', compact( 'project' ,'id' ,'invoices'));
    }
