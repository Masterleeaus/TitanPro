...
namespace Workdo\JobBoard\Http\Controllers;
...
use Workdo\JobBoard\Entities\Project;
use Workdo\JobBoard\Entities\Stage;
use Workdo\JobBoard\Entities\Task;
use Workdo\JobBoard\Entities\Milestone;
use Workdo\Account\Entities\Bill;
use Workdo\Account\Entities\BillPayment;
use Workdo\JobBoard\DataTables\ProjectReportDatatable;
...
class ProjectReportController extends Controller
{
    ...
    public function index(ProjectReportDatatable $dataTable)
    {
        ...
        return $dataTable->render('jobboard::project_report.index',compact('dataTable','users'));
    }
    ...
    public function show(Request $request,$id)
    {
            ...
            $arrProcess_Label_priority = [];
                foreach ($project_priority_task as $lable => $process) {
                 ...
                 $arrProcessClass = [
                  ...
                  $chartData = app('Workdo\JobBoard\Http\Controllers\ProjectController')->getProjectChart([
                    'workspace_id' => getActiveWorkSpace(),
                    'duration' => 'week',
                ]);
        ...
        ->join('invoices', 'invoice_payments.invoice_id', '=', 'invoices.id')->where('invoices.account_type' ,'JobBoard')->where('invoices.category_id' , $id)
    ...
    public function quarterlyCashflow(Request $request ,$id){
      ...
      $data['netProfitArray'] = $netProfit;
        ...
        $returnHTML = view('jobboard::project_report.cashflow', compact('id','filter','data','year'))->render();
        $response = [
            'is_success' => true,
            'message' => '',
            'html' => $returnHTML,
