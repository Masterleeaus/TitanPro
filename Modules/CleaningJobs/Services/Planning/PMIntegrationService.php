...
namespace Modules\CleaningJobs\Services\PlanningCore;
...
class PMIntegrationService
{
    ...
    public function canViewProject($projectId, $user = null): bool
    {
        ...
        return \Modules\CleaningJobs\Models\PlanningCore\ProjectMember::where('project_id', $projectId)
    ...
    {
        $stats = [
            'total_projects' => \Modules\CleaningJobs\Models\PlanningCore\Project::count(),
            'active_projects' => \Modules\CleaningJobs\Models\PlanningCore\Project::where('status', \Modules\CleaningJobs\Enums\PlanningCore\ProjectStatus::IN_PROGRESS->value)->count(),
            'completed_projects' => \Modules\CleaningJobs\Models\PlanningCore\Project::where('status', \Modules\CleaningJobs\Enums\PlanningCore\ProjectStatus::COMPLETED->value)->count(),
            'overdue_projects' => \Modules\CleaningJobs\Models\PlanningCore\Project::where('end_date', '<', now())
                ->whereNotIn('status', [\Modules\CleaningJobs\Enums\PlanningCore\ProjectStatus::COMPLETED->value, \Modules\CleaningJobs\Enums\PlanningCore\ProjectStatus::CANCELLED->value])
                ->count(),
        ];
    ...
    {
        return [
            'statuses' => \Modules\CleaningJobs\Enums\PlanningCore\ProjectStatus::cases(),
            'types' => \Modules\CleaningJobs\Enums\PlanningCore\ProjectType::cases(),
            'priorities' => \Modules\CleaningJobs\Enums\PlanningCore\ProjectPriority::cases(),
