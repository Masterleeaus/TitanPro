...
use Modules\CleaningJobs\Models\JobBoard\Timesheet;
...
class Project extends Model
{
    ...
    {
        return \Workdo\JobBoard\Database\factories\ProjectFactory::new();
    }
    ...
    {
        return $this->hasMany('Modules\CleaningJobs\Models\JobBoard\Task', 'project_id', 'id');
    }
    ...
    {
        return $this->hasMany('Modules\CleaningJobs\Models\JobBoard\Milestone', 'project_id', 'id');
    }

    public function files()
    {
        return $this->hasMany('Modules\CleaningJobs\Models\JobBoard\ProjectFile', 'project_id', 'id');
    }

    public function activities()
    {
        return $this->hasMany('Modules\CleaningJobs\Models\JobBoard\ActivityLog', 'project_id', 'id')->orderBy('id', 'desc');
    }
    ...
    {
        $stage    = \Modules\CleaningJobs\Models\JobBoard\Stage::where('workspace_id', '=', getActiveWorkSpace())->where('complete', 1)->first();
        if (!empty($stage)) {
            $status = $stage->id;
        } else {
            $status = 0;
