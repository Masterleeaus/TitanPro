...
@section('page-script')
    <script>
        window.pageData = {
            urls: {
                projectData: @json(route('planning.projects.data')),
                projectStore: @json(route('planning.projects.store')),
                projectUpdate: @json(route('planning.projects.update', ['project' => '__ID__'])),
                projectDelete: @json(route('planning.projects.destroy', ['project' => '__ID__'])),
                projectShow: @json(route('planning.projects.show', ['project' => '__ID__'])),
                projectEdit: @json(route('planning.projects.edit', ['project' => '__ID__']))
            },
            labels: {
    ...
    @vite(['Modules/PlanningCore/resources/assets/js/project-list.js'])
@endsection
...
@include('planning::projects._partials._stats', ['stats' => $stats])

<!-- Projects Table -->
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">{{ __('Manage Projects') }}</h5>
        @can('planning.create-project')
        <a href="{{ route('planning.projects.create') }}" class="btn btn-primary">
