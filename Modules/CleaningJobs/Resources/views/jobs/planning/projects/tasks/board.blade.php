...
@section('page-script')
    @vite(['Modules/PlanningCore/resources/assets/js/task-form-handler.js'])
    @vite(['Modules/PlanningCore/resources/assets/js/project-tasks-board.js'])
@endsection
...
<x-breadcrumb
    ...
    :breadcrumbs="[
        ...
        ['name' => $project->name, 'url' => route('planning.projects.show', $project->id)],
...
@include('planning::projects.tasks._form', [
    'formId' => 'taskForm',
...
window.pageData = {
    ...
    users: @json($users),
    urls: {
        getDataAjax: @json(route('planning.projects.tasks.getDataAjax', $project->id)),
        store: @json(route('planning.projects.tasks.store', $project->id)),
        update: @json(route('planning.projects.tasks.update', [$project->id, '__ID__'])),
        destroy: @json(route('planning.projects.tasks.destroy', [$project->id, '__ID__'])),
        show: @json(route('planning.projects.tasks.show', [$project->id, '__ID__'])),
        complete: @json(route('planning.projects.tasks.complete', [$project->id, '__ID__'])),
        start: @json(route('planning.projects.tasks.start', [$project->id, '__ID__'])),
        stop: @json(route('planning.projects.tasks.stop', [$project->id, '__ID__'])),
        reorder: @json(route('planning.projects.tasks.reorder', $project->id)),
        // URLs for TaskFormHandler
        tasksStore: @json(route('planning.projects.tasks.store', $project->id)),
        tasksUpdate: @json(route('planning.projects.tasks.update', [$project->id, '__ID__'])),
        tasksShow: @json(route('planning.projects.tasks.show', [$project->id, '__ID__']))
    },
    labels: {
