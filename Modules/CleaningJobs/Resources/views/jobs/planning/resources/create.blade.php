...
@section('page-script')
    <script>
        window.pageData = {
            ...
            user: @json($user ?? null),
            urls: {
                store: @json(route('planning.resources.store')),
                index: @json(route('planning.resources.index')),
                projectSearch: @json(route('planning.projects.search')),
                userSearch: @json(route('planning.users.search')),
                availability: @json(route('planning.resources.availability')),
                projectTasks: '/planning/projects/:id/tasks'
            },
            labels: {
    ...
    @vite(['Modules/PlanningCore/resources/assets/js/resource-create.js'])
@endsection
            ...
            <div class="card-body">
                <form id="createAllocationForm">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('planning.resources.index') }}" class="btn btn-label-secondary">{{ __('Cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
