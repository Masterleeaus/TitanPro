...
class BookingController extends Controller
{
    ...
    public function __construct(Booking $booking, BookingRepeatDetails $bookingRepeatDetail, BookingRepeatHistory $bookingRepeatHistory, BookingRepeat $bookingRepeat, BookingDetail $bookingDetails, BookingStatusHistory $bookingStatusHistory, BookingScheduleHistory $bookingScheduleHistory, Category $category, Zone $zone, Serviceman $serviceman, UserAddress $userAddress, BookingAdditionalInformation $bookingAdditionalInformation)
    {
        ...
        $subscribedServiceClass = class_exists('\Modules\ProviderManagement\Entities\SubscribedService')
            ? '\Modules\ProviderManagement\Entities\SubscribedService'
            : null;
        if ($subscribedServiceClass) {
            try {
                $this->subscribedSubCategories = $subscribedServiceClass::where(['is_subscribed' => 1])->pluck('sub_category_id')->toArray();
            } catch (\Exception $exception) {
                $this->subscribedSubCategories = $subscribedServiceClass::pluck('sub_category_id')->toArray();
            }
        } else {
            $this->subscribedSubCategories = [];
        }
    }
