...
use Modules\BookingModule\Entities\Category;
...
class BookingController extends Controller
{
    ...
    public function __construct(Booking $booking, BookingRepeat $bookingRepeat, BookingRepeatDetails $bookingRepeatDetail, BookingRepeatHistory $bookingRepeatHistory, BookingIgnore $bookingIgnore, BookingStatusHistory $bookingStatusHistory, BookingScheduleHistory $bookingScheduleHistory, Category $category, Zone $zone, Serviceman $serviceman, User $user, UserAddress $userAddress, BookingDetail $bookingDetail)
    {
        ...
        $providerClass = class_exists('\Modules\ProviderManagement\Entities\Provider') ? '\Modules\ProviderManagement\Entities\Provider' : null;
        $subscribedServiceClass = class_exists('\Modules\ProviderManagement\Entities\SubscribedService') ? '\Modules\ProviderManagement\Entities\SubscribedService' : null;
        $this->provider = $providerClass ? app($providerClass) : null;
        $this->subscribed_service = $subscribedServiceClass ? app($subscribedServiceClass) : null;
        $this->user = $user;
        ...
        if ($subscribedServiceClass && optional(auth('api')->user())->provider?->id) {
            try {
                $this->subscribed_sub_categories = $subscribedServiceClass::where(['provider_id' => auth('api')->user()->provider->id])
                    ->where(['is_subscribed' => 1])->pluck('sub_category_id')->toArray();
            } catch (\Exception $exception) {
                $this->subscribed_sub_categories = $subscribedServiceClass::pluck('sub_category_id')->toArray();
            }
        } else {
            $this->subscribed_sub_categories = [];
        }
    }
