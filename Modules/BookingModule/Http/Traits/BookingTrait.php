...
trait BookingTrait
{
    ...
    {
        return class_exists('\Modules\ProviderManagement\Entities\Provider') ? '\Modules\ProviderManagement\Entities\Provider' : null;
    }

    protected function subscribedServiceClass(): ?string
    {
        return class_exists('\Modules\ProviderManagement\Entities\SubscribedService') ? '\Modules\ProviderManagement\Entities\SubscribedService' : null;
    }
                ...
                if ($booking->payment_method == 'cash_after_service') {
                    if ($maximumBookingAmount > 0 && $booking->total_booking_amount < $maximumBookingAmount) {
                        if (isset($booking->provider_id) && $booking->booking_status != 'pending') {
                            $provider = ($providerClass = $this->providerClass()) ? $providerClass::with('owner') : collect([])->whereId($booking->provider_id)->first();
                            ...
                            $providerIds = $subscribedServiceClass = $this->subscribedServiceClass();
                            if (!$subscribedServiceClass) { $providerIds = []; } else { $providerIds = $subscribedServiceClass::where('sub_category_id', $subCategory)->ofSubscription(1)->pluck('provider_id')->toArray(); }
                            if (business_config('suspend_on_exceed_cash_limit_provider', 'provider_config')->live_values) {
                                $providers = ($providerClass = $this->providerClass()) ? $providerClass::with('owner') : collect([])->whereIn('id', $providerIds)->where('zone_id', $booking?->zone_id)->where('is_suspended', 0)->get();
                            } else {
                                $providers = ($providerClass = $this->providerClass()) ? $providerClass::with('owner') : collect([])->whereIn('id', $providerIds)->where('zone_id', $booking?->zone_id)->get();
                            }
    ...
    public function calculateCommissionDetails($booking): array
    {
        ...
        $provider = ($providerClass = $this->providerClass()) ? $providerClass::find($booking['provider_id']) : null;
