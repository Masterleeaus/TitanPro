...
use Modules\BookingModule\Traits\HasUuid;
use Carbon\Carbon;
...
class Booking extends Model
{
    ...
    use HasFactory, HasUuid, BookingTrait, BookingScopes {
        BookingTrait::providerClass insteadof BookingScopes;
        BookingTrait::subscribedServiceClass insteadof BookingScopes;
    }
    ...
    {
        $providerClass = class_exists('\\Modules\\ProviderManagement\\Entities\\Provider') ? '\Modules\ProviderManagement\Entities\Provider' : User::class;
        return $this->belongsTo($providerClass, 'provider_id');
    }
    ...
    {
        return $this->belongsTo('Modules\\BidModule\\Entities\\Post', 'id', 'booking_id');
    }
            ...
            if (isset($booking_notification_status) && $booking_notification_status['push_notification_booking']) {
                foreach ($notifications ?? [] as $notification) {
                    ...
                    if ($settingsType == 'provider_notification') {
                        $suspendConfig = business_config('suspend_on_exceed_cash_limit_provider', 'provider_config');
                        $suspendEnabled = $suspendConfig && $suspendConfig->live_values;
                        $isSuspended = $model?->provider?->is_suspended == 1;

                        // Only notify if: suspension is disabled, OR provider is not suspended, AND booking is pending
                        if ((!$suspendEnabled || !$isSuspended) && $model->booking_status == 'pending') {
